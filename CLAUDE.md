...
| **Menores a cargo** / asignar una entrada a un menor | `docs/specs/menores-a-cargo.md` 🟦 **TANDA 1 EN EL ÁRBOL** (`#191`, 2026-08-27, carril A: la entidad `Dependent`, el registro con tope de servidor bajo lock, `GET|POST|DELETE /me/dependents` contra el contrato, `anonymize()`/export/purga de go-live, la poda y el tope en Ajustes → «Puerta»; 34 casos, 7 mutaciones que muerden, sonda HTTP sobre MySQL — **sin firmas de menor todavía**). ▶ **EMPIEZA POR §9 y, antes de escribir nada, §9.5: cinco decisiones del owner con número y coste** — ❗ **la 1.ª es NUC-3** (la cadena de hashes con firmas de menor: cadena por (titular, sujeto) recomendada, vs poda por prefijo) y bloquea la tanda 2; **la 2.ª es el techo del chunk** (0,16 KiB) y bloquea la tanda 3 · §9.2: en qué se apartó del cuerpo (solo menores al declarar · sin edición · la FK `subject_id → dependents` llega en la tanda 2 · el correo verificado NO se exige para declarar) · §9.6 trampas. Después el cuerpo: **§8.1 CORRIGE a §4.8: el mecanismo que hay que extender es la LISTA BLANCA de `cart.js::save()`, no `selection.js`** · **§8.2 subir `STORAGE_VERSION` purga TODAS las cestas vivas** · **§4.6: Booking NO puede mirar a Identity** · **§4.7: al elegir la cantidad NO hay sesión garantizada** · §4.1: la fila sobrevive a la minoría de edad. `DECISIONES #142` + `#156` + **`#191`** |
