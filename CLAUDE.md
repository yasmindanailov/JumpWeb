...
| **Desmontar `ViewOrder`** · el god-class del panel · tocar edición/reembolso/calendario de un pedido | `docs/specs/desmontar-view-order.md` 🟦 **EN EJECUCIÓN, queda SOLO la 4b** — ✅ owner + cinco tandas empujadas el 2026-08-26 (`#170` paso 0 · `#172` calendario→trait · `#173` presentación→trait · `#176` la oferta de re-programación a `Booking\Services\ItemRescheduleOffer` · `#177` el instrumento `panel-edit`, VISTO FALLAR): **`ViewOrder` en 3.907 líneas y ya no compone ninguna oferta** · ▶ **EMPIEZA POR §9.5 (el handoff de la 4b, sub-pasos A→H) y §4.3 (el mapa transaccional REAL: la txn contiene SOLO aforo, la secuencia financiera corre POST-COMMIT — no crees la envolvente que no existe)** · §8 es el registro de la revisión (`#167`/`#168`) · §5: once invariantes, `AFORO-01`/`AFORO-05` las más expuestas · `[DECIDIDO owner]` (`#181`): con la 4b el desmontaje TERMINA (§4.4 cerrada) · regla pagada §9.1: commitear en local ANTES de mutar · `docs/DEUDA.md` |
