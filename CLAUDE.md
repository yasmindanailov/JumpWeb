...
| Desglose de dinero que ve el CLIENTE (ledger, «Mis pedidos») | `docs/specs/desglose-dinero-cliente.md` 🟦 — **TANDAS A y B EJECUTADAS + los TRES defectos de lectura** (§15 el dominio dice la verdad · §16 el desglose ya es LEGIBLE · **§18: y ya es VERIFICABLE** · **§19: «Mis pedidos» es pantalla propia — y el suelo que la sostenía PERDÍA pedidos**) · marco en `DECISIONES #127`, lectura en `#128`, pantalla en `#129`, **segunda vuelta con el owner en `#130` (§20: «Mis reservas» sin dinero · el eje de caja solo si dice algo NUEVO · la línea que faltaba)**. **CERRADO.** **Empieza por §9, §10 y §18, no por el principio**: §9.2 son **CUATRO defectos de DOMINIO** que las dos auditorías anteriores no llegaron a construir —y **el peor, el pedido cancelado sin reembolsar, no lo caza ninguna identidad**— · §9.3, la proyección son **cuatro** defectos y no uno · **§10: los DOS EJES CERRADOS, verificados en 58 de 58 pedidos reales** · **§17: el pedido `R-L6UTIA`, que parece un fallo del desglose y es un DATO ROTO** (comprueba si el dato es real ANTES de buscar el fallo en el código) · §18.5 y §19.1, lo que enseñó la ejecución —**incluido que `GET /me/orders` perdía dos pedidos de 57, y que la guarda de conducta salía VERDE en SQLite**— · §9.4, las trampas de método · §4.quater, la receta y las CUATRO puertas de `OrderCreator` |
