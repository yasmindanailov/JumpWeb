...
| **Desmontar `ViewOrder`** · el god-class del panel · tocar edición/reembolso/calendario de un pedido | `docs/specs/desmontar-view-order.md` 🟦 — ⚠️⚠️ **EMPIEZA POR §8 (revisión adversarial, `#167`): el diagnóstico sobrevive pero TRES piezas del plan describían un código que no existe** — el paso 3 compara comprar-vs-mover y se re-diseña como EXTENSIÓN (`VERIFY_CONC` ya ahí) · **§8.4: la transacción de `executeItemEdit` contiene SOLO aforo; la secuencia financiera corre POST-COMMIT** (quien «preserve» la envolvente de §4.3 la CREARÍA) · §8.5: los verificadores de §6.4 no ejecutan lo mudado (`AFORO-05` sin instrumento) · §8.6: las «25 llamadas» son **8** · §8.7: dos de los tres métodos de disponibilidad están MUERTOS · Cuerpo: §1.3 es GRANDE pero NO enmarañado (5 propiedades, extracciones acotadas) · §4.1 cuatro extracciones por riesgo creciente, el dinero la última — **pendiente: incorporar §8.12 y el ✅ del owner; hasta entonces `app/` no se toca** · `docs/DEUDA.md` |
