...
| **Desmontar `ViewOrder`** · el god-class del panel · tocar edición/reembolso/calendario de un pedido | `docs/specs/desmontar-view-order.md` 🟦 — **revisada (`#167`) y CORREGIDA (`#168`): el cuerpo ya describe el código real; §8 es el registro de la revisión** — §1.3: es GRANDE pero NO enmarañado (5 propiedades, extracciones acotadas) · **§4.1: paso 0 (código muerto) + cuatro extracciones por riesgo creciente, el dinero la última; el calendario va como CONCERN** · ⚠️ **§4.2/§4.1·3: la disponibilidad del panel responde a MOVER, no a comprar — el paso 3 es una EXTENSIÓN del dominio y dispara `VERIFY_CONC`** · ⚠️⚠️ **§4.3: la transacción de `executeItemEdit` contiene SOLO aforo; la secuencia financiera corre POST-COMMIT — no crees la envolvente que no existe** · §5: once invariantes, `AFORO-01`/`AFORO-05` las más expuestas · §6·4: el paso 4 exige un escenario nuevo de edición concurrente, visto fallar — ❗ **pendiente SOLO el ✅ del owner; hasta entonces `app/` no se toca** · `docs/DEUDA.md` |
...
| Instalar un cliente nuevo (white-label) · **paquete de tema (`client.css`)** | `docs/INSTALACION-CLIENTE.md` (**§4: el tema son TRES mecanismos y cada cosa entra por el suyo** — ⚠️ el hueco de `client.css` son **tres piezas** y con dos parece que funciona: el orden, el `.gitignore` y la exclusión del `rsync --delete`) |
