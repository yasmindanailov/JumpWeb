...
| **Waiver** (firma, prueba, PDF, versiones del texto) | `docs/specs/waiver-probatorio.md` 🟦 (**§4.6: no se puede anonimizar un documento y que siga sirviendo de prueba** — es conservación restringida con plazo, y MODIFICA `RGPD-01` · §4.2: el idioma es parte de la prueba · §4.8: exigir la versión vigente en la puerta crea una cola) · `DECISIONES #142` |
| **Menores a cargo** / asignar una entrada a un menor | `docs/specs/menores-a-cargo.md` 🟦 (**§4.6: Booking NO puede mirar a Identity** — la asignación la posee Identity y referencia el ítem por su id entero · **§4.7: al elegir la cantidad NO hay sesión garantizada**, identificarse es el paso 5 · §4.1: la fila sobrevive a la minoría de edad) · `DECISIONES #142` |
| **Carné QR** del cliente · pantalla de puerta / escaneo | `docs/specs/identidad-qr-puerta.md` 🟦 (**§4.4: un carné es la siguiente credencial que `RGPD-06` existe para no perder** · §3: una URL sube el QR de versión 2 a 4, medido · **§4.8: un temporizador de navegador NO es garantía** — la ficha caduca en servidor, `SEC-04` aplicado al tiempo) · `DECISIONES #142` |
| **JumpPoints** / vales / lealtad | `docs/specs/lealtad-jumppoints.md` 🟦 (**§4.2: los puntos se abren DESPUÉS de la visita** y eso mata por construcción el agujero comprar→canjear→reembolsar · **§4.5: no es dinero pero se protege como si lo fuera** — el canje entra en el `CRITICAL_RE` · §4.6: la caducidad es lo único que depende del cron) · `DECISIONES #142` |
| CMS público (servicios, ofertas) | `docs/sistemas/SERVICIOS-CMS.md` · `docs/sistemas/OFERTAS-WIDGET.md` |
| **Reseñas / valoración de Google** · prueba social en la landing | `docs/specs/google-reviews.md` ⬜ **BORRADOR, pendiente del owner** — ⚠️⚠️ **§1.3: CINCO restricciones DURAS** verificadas contra la doc oficial, y tres chocan con invariantes ya endurecidos (`PERF-02`, `SEC-07`, `RGPD-05`). **§1.4: con Google como única fuente el parque NO elige qué sale en su portada.** **§3 son tres decisiones del owner** —y la de la caché es **cumplimiento, no ingeniería**—. ❗ **No se escribe código hasta que §3 esté respondida** |
