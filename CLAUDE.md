...
| Landing / tema visual (tokens CSS) · CMS de contenido · white-label por cliente | **`docs/specs/landing-white-label.md`** 🟦 (`#136`) — **la línea es: data-driven el DATO, no la PÁGINA.** ⚠️ **Empieza por §1**: tres de los cinco problemas enunciados NO eran lo que parecían · §4.5: el tema son **TRES** mecanismos (valores→tokens · ficheros→assets · dibujos→el set de iconos, que **ya existe y ya tiene guarda**) · §4.6: el icono por producto es el hueco real · ⚠️ **§4.4/§7: los servicios como producto real tocan AFORO y PAY — spec propia, no se empieza desde ahí** · `docs/ARQUITECTURA.md` (white-label) · `docs/MAPA-PAGINAS.md` |
