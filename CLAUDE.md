...
| Desglose de dinero que ve el CLIENTE (ledger, «Mis pedidos») | `docs/specs/desglose-dinero-cliente.md` 🟦 — **TANDAS A y B EJECUTADAS + los TRES defectos de lectura** (§15 el dominio dice la verdad · §16 el desglose ya es LEGIBLE · **§18: y ya es VERIFICABLE — el ancla de caja se ve siempre que haya habido un cobro, con su método y su fecha**; **lo siguiente es la tanda C, la pantalla**) · marco en `DECISIONES #127`, lectura en **`#128`**. **Empieza por §9, §10 y §18, no por el principio**: §9.2 son **CUATRO defectos de DOMINIO** que las dos auditorías anteriores no llegaron a construir —y **el peor, el pedido cancelado sin reembolsar, no lo caza ninguna identidad**— · §9.3, la proyección son **cuatro** defectos y no uno · **§10: los DOS EJES CERRADOS, verificados en 58 de 58 pedidos reales** · **§17: el pedido `R-L6UTIA`, que parece un fallo del desglose y es un DATO ROTO** (comprueba si el dato es real ANTES de buscar el fallo en el código) · §18.5, las cuatro cosas que enseñó la ejecución · §9.4, las trampas de método · §4.quater, la receta y las CUATRO puertas de `OrderCreator` |
| «Mis reservas» por RESERVA · historial aparte · orden y paginación | `docs/specs/mis-reservas-por-reserva.md` ✅ (**§3.4: los dos ámbitos son UN predicado con dos lados** — si se separan, una reserva puede no salir en ninguna de las dos pantallas y eso no falla ni avisa · §4.2: el ledger NO viaja con la tarjeta, se pide con `GET /orders/{code}`) · `DECISIONES #126` |
