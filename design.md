...
| **Interacción** *(futuro)* | `--interactive` · `--interactive-hover`, por superficie | El color de lo que responde al ratón y al teclado fuera de una zona (enlace en texto, pregunta abierta, pestaña activa, icono de enlace). **Producto por defecto: tinta** (`--fg`), como el foco. El paquete puede fijarlo por superficie con el contraste medido (PlayJump: Azul Muro en papel, cian en tinta — su propia tabla de roles). `[PENDIENTE: owner]` D1 |
| **Semánticos** | `--ok` · `--err` · `--warn` · `--attn` · `--refund` (+ `-bg`, `-border`, `-hover`) | Estado, nunca decoración (`--attn` ≠ `--zone-2` aunque hoy valgan igual: `#138`). *(futuro)* `--on-ok` · `--on-err` · `--on-warn` derivados por luminancia como `--on-brand`: hoy el texto sobre estos rellenos es `#fff` a mano (auditoría C3, 2,95 : 1) |
