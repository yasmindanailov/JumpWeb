...
| **Semánticos** | `--ok` · `--err` · `--warn` · `--attn` · `--refund` (+ `-bg`, `-border`, `-hover`) | Estado, nunca decoración (`--attn` ≠ `--zone-2` aunque hoy valgan igual: `#138`). **`--on-ok` · `--on-err` · `--on-warn`** (`#434`): el texto sobre esos rellenos; **`--ok-text` · `--err-text` · `--warn-text`** (`#450`): el color COMO texto sobre papel (con el verde claro de PlayJump, `--ok` como texto da 2,95). **No se derivan** —el servidor no ve `--ok`, que lo declara el paquete, y CSS no tiene luminancia—: el par lo declara quien declara el color (el producto para sus verde y rojo oscuros, el paquete para los suyos). Un `#fff` junto a un relleno semántico lo caza `SemanticFillTextTest` |
...
Nacida con la tanda A del cajón (`#450`): `DrawerControlsTest` (dentro del cajón ningún texto lee
`--zone-*` ni un semántico a secas; doce familias de control leen `--tap-min`; el CTA del pie no parte; la
página pausa sus bucles tras el velo).
