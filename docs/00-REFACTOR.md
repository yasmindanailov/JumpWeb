...
> Última actualización: **2026-09-21** · Leyenda: ⬜ pendiente · 🟦 en curso · ✅ hecho · ❗ bloqueado.
...
- [ ] F5 · instancia PlayJump y la landing fuera, v2.0.0 · **ABIERTA el 19-09 por el CENSO** (`specs/instancia-y-landing-fuera.md` ✅, `#639`). **T1 · el MENÚ, SERVIDO** (`#640`→`#646`, contrato **1.11.0**): los siete platos, uno a uno en su spec §4.1. **T2a ✅** (`specs/paquete-de-instancia.md`, `#647`): el namespace `instancia::`, el invariante **`SEC-12`**, la `plantilla/` y el repo de instancia con remoto privado. **T2b ✅** (`#649`→`#666`; su historia entera en §4.5.bis→§4.7.ter): **las NUEVE vistas de la landing salen del producto** con su material (37 ficheros, 9,2 MB), nace `CONTRATO_DE_VISTAS` y la suite corre SIN paquete. Huella **0/38** en todas, sitemap 11=11, arneses 144/144. ❗ `#664` midió 161 rojos con un anfitrión de nueve líneas y salieron **16**: *la cifra mide el anfitrión, no la página*. **T2c ✅** (`#667`, §4.6.bis): de 208 huérfanas solo **17** eran reales, y **128 no las pintaba nadie**: podadas 119 (−20,3 KB), con **trinquete**. **T3·1 ✅** (`#668`, §4.6.ter): fuera los dos recursos con CERO uso y **el contrato de instancia sube a 2**, la primera vez; los otros cuatro esperan a tener plato en el menú. **T4 ✅** (`#669`, §4.3): `zones` pierde sus cuatro columnas muertas. ▶ Queda el resto de la T3 —bloqueado: el menú no tiene plato para esos cuatro recursos— y la **T5** (v2.0.0)
...
- [ ] Google Business Profile (`#524`, lo lleva SPA): **T1 CERRADA** (`#720`→`#726`) y **T2·1→T2·6 EN EL ÁRBOL** (`#727`→`#732`): las tablas con sus plazos, la pasada diaria —**se escribe lo visto, se borra solo con una coherente**—, las **imágenes desde nuestro servidor**, **«Ocultar»** y la **fuente nueva con cascada de TRES**. Detalle en §4.1. ⚠️ **Places NO se retira** (`[owner]` 21-09): `img-src` sigue nombrando a Google; **la portada real vive en la instancia**. ▶ Faltan **respuesta/fotos/anónima en la tarjeta**, el botón del panel, el texto de privacidad y retirar Places; más JumpSystem y los 60 días.
