...
- [ ] F4 · cajón empaquetable y login por token, contrato 1.1.0 · ABIERTA el 2026-09-18, medida: **el token ✅ HECHO** (`specs/token-bearer.md`, `#630`: `POST /auth/tokens` y `/rotate`, contrato 1.1.0, ability `api-v1` exigida en toda ruta autenticada, tope de 10; 19 tests, mutación 14/14, `curl` en local) · **el cajón, por tandas y en este carril** (`specs/cajon-empaquetable.md`, `#631`→`#633`): **T1 ✅ el arranque** (`Http\Sidebar\SidebarBoot` + `GET /sidebar/boot` y `/sidebar/session`, contrato 1.2.0; `data-boot` idéntico byte a byte en 7 contextos, 9 tests, mutación 8/8) · **T2 ✅ la apertura sin Alpine** (`resources/js/cajon/controller.js`, `window.JumpWeb.cajon`, `data-jw-*`, eventos `jw:cajon:*`; sonda en navegador 17/17 y vista en rojo sin el proxy, 20 tests JS, mutación 13/13) · **T3a ✅ la carcasa con un solo dueño** (`cajon/shell.js`: el marcado pierde sus 8 atributos de Alpine y `a11yPanel` se retira; `/entradas` idéntica píxel a píxel, sonda 23/23, mutación 20/20; dos defectos del foco heredados encontrados en navegador) · **T3b ✅ el paquete se monta en una página AJENA** (`installCajon()` + `cajon/standalone.js` diferido: pide el arranque a la API y CONSTRUYE la carcasa; `jw:cajon:purchased`; sonda 31/31 con una página sin carcasa, sin `data-boot` y sin Alpine que compra; mutación 34/34) · falta T4 hoja propia (huella 24/24) · T5 la salida
