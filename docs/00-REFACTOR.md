...
> Última actualización: **2026-09-20** · Leyenda: ⬜ pendiente · 🟦 en curso · ✅ hecho · ❗ bloqueado.
...
- [ ] F5 · instancia PlayJump y la landing fuera, v2.0.0 · **ABIERTA el 19-09 por el CENSO** (`specs/instancia-y-landing-fuera.md` ✅, `#639`). **T1 · el MENÚ, SERVIDO** (`#640`→`#646`; hoy contrato **1.11.0**): los siete platos. **T2a ✅** (`specs/paquete-de-instancia.md`, `#647`): el namespace `instancia::`, el invariante **`SEC-12`**, la `plantilla/` y el repo de instancia. **T2b en curso** (`#649`→`#663`): el contrato producto↔instancia son los DATOS de la vista, no su HTML; nace el contrato de vista y **las OCHO de `pages/` MUDADAS** —la carpeta ya no existe— (anfitriones mínimos, suite SIN paquete, huella 0/38 salvo el dinero de `#660`). **Barrido de `home` HECHO y sus CUATRO reglas FUERA** (`#661`→`#663`): el euro recogido de SEIS escrituras, el horario, la escala de estrellas y **el material del cliente fuera de `main`** —37 ficheros y 9,2 MB, con exclusión del `rsync`, lista blanca de la guarda 9 y `git rm` en el MISMO commit—; y cuatro guardas nuevas. **La mudanza de `home`, MEDIDA antes de tocarla** (`#664`: 161 rojos de 682, y ~44 cambian de sujeto porque su sujeto es un componente) y **nace la T2c** (`#665`, `[DECIDIDO owner]`): el CSS se muda aparte, porque el marcado se verifica con huella y las reglas con cascada. ▶ Queda mudar `home` (la piden **682 casos en 60 ficheros**); después T3–T5
...
- [ ] Google Business Profile (`#524`): **T1 (la conexión) CERRADA en código** el 20-09 (`#720`→`#726`, la lleva el SPA) y la **T2 empezada**: **T2·1** (el cimiento: tablas, los dos plazos y la guarda de la imagen, `#727`) y **T2·2** (traer las reseñas: paginación con tope, filtro de candidatas, el analizador que falla cerrado y el `coherent()` de la pasada, `#728`); falta persistir (T2·3), imágenes, «Ocultar», la sección y retirar Places, más el proyecto de Google de JumpSystem y los 60 días de la ficha. · las bandas · los tres ejes de la pasada de vestido.
