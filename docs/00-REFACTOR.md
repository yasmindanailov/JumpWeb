...
- [ ] F5 · instancia PlayJump y la landing fuera, v2.0.0 · **ABIERTA el 19-09 por el CENSO** (`specs/instancia-y-landing-fuera.md` ✅, `#639`: tres decisiones del owner contestadas; el censo, en su §1). **T1 · el MENÚ, SERVIDO** (`#640`→`#646`, contrato **1.10.0**, arnés 44/44): `site`, horario, normas, legales, precios, la ficha de producto y zona, y la cifra de prueba social —sin reseñas: esperan a su fuente—. **T2a ✅** (`specs/paquete-de-instancia.md`, `#647`): el namespace `instancia::`, el invariante **`SEC-12`**, la `plantilla/` y el repo de instancia. **T2b en curso** (`#649`→`#651`): el contrato producto↔instancia son los DATOS de la vista, no su HTML; nace el contrato de vista y el barrido ya cazó un defecto vivo. ▶ Siguen el barrido, mudar las vistas, y T3–T5
...
- [ ] ▶ **La invitación, COMPLETA EN CÓDIGO.** Falta el **ojo del owner**, **desplegar** (T5, T6 y T7 en el árbol), **encender** (dato del owner) y el borde §7.1·5.
