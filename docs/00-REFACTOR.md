...
- [ ] F5 · instancia PlayJump y la landing fuera, v2.0.0 · **ABIERTA el 19-09 por el CENSO** (`specs/instancia-y-landing-fuera.md` ✅, `#639`). **T1 · el MENÚ, SERVIDO** (`#640`→`#646`; hoy contrato **1.11.0**): los siete platos. **T2a ✅** (`specs/paquete-de-instancia.md`, `#647`): el namespace `instancia::`, el invariante **`SEC-12`**, la `plantilla/` y el repo de instancia. **T2b en curso** (`#649`→`#661`): el contrato producto↔instancia son los DATOS de la vista, no su HTML; nace el contrato de vista y **las OCHO de `pages/` MUDADAS** —la carpeta ya no existe— (anfitriones mínimos, suite SIN paquete, huella 0/38 salvo el dinero DECIDIDO en `#660`). **Barrido de `home` HECHO y TRES de sus cuatro reglas FUERA** (`#661`, `#662`): el euro recogido de SEIS escrituras, el servicio de horario y la escala de estrellas; tres variables muertas y tres guardas nuevas. ▶ Queda el material del cliente fuera de `main` (`#663`, `[DECIDIDO owner]`) y mudar `home` (la piden **682 casos en 60 ficheros**); después T3–T5
