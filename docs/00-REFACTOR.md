...
- [ ] F5 · instancia PlayJump y la landing fuera, v2.0.0 · **ABIERTA el 19-09 por el CENSO** (`specs/instancia-y-landing-fuera.md` ✅, `#639`: tres decisiones del owner contestadas; el censo, en su §1). **El MENÚ, cinco platos servidos** (`#640`→`#644`, contrato **1.7.0**, arnés 24/24): `/site` con lista blanca —y un secreto no sale ni declarándolo—, el horario en dos rutas por cómo se cachean, las normas, los legales interpolados y los precios por tarifa. ▶ Siguen la ficha de producto y zona, y la prueba social
...
- [x] T5 · la página pública de la invitación (§4.6), **las cinco unidades en el árbol y sin desplegar**: **T5·1 la página y sus tres temas** (`#701`), **T5·2 contestar con su aviso de privacidad** (`#702`) y **T5·3 el recibo de 2 h con G2/G3** (`#703`) hechas y vistas por el owner. **T5·5 · el flujo firmar↔invitación** (`#704`, contrato `SignedInvitationReplies`, arnés 7/7; el CUARTO defecto, en §10.6·E). **T5·4 · compartir y calendario** (`#705`: `CalendarFile` en Platform, el `.ics` con `TZID`, arnés 8/8; ⚠️ sin probar en un teléfono real). **§10.6·A cerrada por el owner** (`#706`): nombre y apellidos siguen siendo DOS campos (`#236` en pie, sin migración) y **el prerrelleno del menor se retira**. Sus dos alternativas, medidas y descartadas en §10.6·A.
- [ ] T6 de la celebración, el aterrizaje (§10.8–§10.10): **T6·1 ✅ el bloque** (`#708`, POST propio y el testigo no se mueve) · **T6·2 ✅ propuestas y adopción** (`#709`, el caso intercalado) · **T6·3 ✅ «no vienen», «no caben» y «no lo apuntes»** (`#710`, el suelo medido, sin tocar `GuestCountAdjuster`) · **T6·4 ✅ puerta y hoja de sala** (`#711`, los tres estados y «8 de 12», por lotes dentro del techo de 28 consultas); arneses 8/8 · 6/6 · 8/8 · 9/9. Siguen T6·5, T6·6 y la T7.
