...
- [x] F4 · cajón empaquetable y login por token, contrato 1.1.0 · **CERRADA el 2026-09-19**: una página que no es del producto monta el cajón, abre y COMPRA (criterio de salida de §2·1), con la landing del producto idéntica píxel a píxel. ⚠️ El **anfitrión mínimo** de §4.4 es de F5 por diseño (`#631`), no un pendiente; y la compra de la T5 está medida en navegador pero **sin ojo del owner** todavía. Medida: **el token ✅ HECHO** (`specs/token-bearer.md`, `#630`: `POST /auth/tokens` y `/rotate`, contrato 1.1.0, ability `api-v1` exigida en toda ruta autenticada, tope de 10; 19 tests, mutación 14/14, `curl` en local) · **el cajón, por tandas y en este carril** (`specs/cajon-empaquetable.md` §4, `#631`→`#635`): **T1 ✅** el arranque (`SidebarBoot` + `/sidebar/boot` y `/sidebar/session`, contrato 1.2.0) · **T2 ✅** la apertura sin Alpine (`cajon/controller.js`, `window.JumpWeb.cajon`, `data-jw-*`) · **T3a ✅** la carcasa con un solo dueño (`cajon/shell.js`) · **T3b ✅** el paquete se monta en una página AJENA (`installCajon()` + `cajon/standalone.js`) · **T4 ✅** la hoja propia (`public/css/cajon.css` GENERADA por `scripts/hoja-del-cajon.py`, sin tocar `site.css`; tokens en `:where(.sidecart)` y el tema declara en `:root, .sidecart`, `#635`) · **T5 ✅** el cargador con entrada propia y ruta estable que redirige (`#636`: 25,3 → 6,1 KiB para una página ajena) y una COMPRA entera desde ella. Medido por tanda en la spec; en navegador: sonda **42/42** (201 + salto firmado a la pasarela), juez de maquetación 0 diferencias sobre 1.640 nodos, mutaciones 8/8 y 5/5
...
- [x] Desplegada la T3 en **v1.1.0** (18-09, noveno despliegue): la barra de firmar del justificante ya no está rota en producción. La casilla siguió pidiendo el despliegue un día después de hacerlo.
...
- [x] T5 · la página pública de la invitación (§4.6), **las cinco unidades en el árbol y sin desplegar**: **T5·1 la página y sus tres temas** (`#701`), **T5·2 contestar con su aviso de privacidad** (`#702`) y **T5·3 el recibo de 2 h con G2/G3** (`#703`) hechas y vistas por el owner. **T5·5 · el flujo firmar↔invitación: C, D y un CUARTO defecto que no estaba escrito —la vuelta de un formulario rechazado perdía la atadura y cobraba plaza— hechos** (`#704`, contrato `SignedInvitationReplies`, 5 casos + 1 de frontera, arnés 7/7). **T5·4 · compartir y calendario hecha** (`#705`: `CalendarFile` en Platform, el `.ics` con `TZID` y su `VTIMEZONE`, la vista previa con solo nombre/edad/día/hora/negocio; 12 casos, arnés 8/8; ⚠️ sin probar en un teléfono real). **§10.6·A cerrada por el owner** (`#706`): nombre y apellidos siguen siendo DOS campos (`#236` en pie, sin migración) y **el prerrelleno del menor se retira** — la hoja le enseña lo que escribió y lo reparte él. Medidas y descartadas sus dos alternativas: partir el nombre recorta «María del Carmen» a «María» en un documento firmado, y pedir solo el nombre en la invitación colapsa a dos «Martina» de una clase en una sola (el anfitrión ve «vienen 1»).
