...
- [ ] F4 · cajón empaquetable y login por token, contrato 1.1.0 · ABIERTA el 2026-09-18, medida: **el token ✅ HECHO** (`specs/token-bearer.md`, `#630`: `POST /auth/tokens` y `/rotate`, contrato 1.1.0, ability `api-v1` exigida en toda ruta autenticada, tope de 10; 19 tests, mutación 14/14, `curl` en local) · **el cajón, por tandas y en este carril** (`specs/cajon-empaquetable.md`, `#631`→`#633`): **T1 ✅ el arranque** (`Http\Sidebar\SidebarBoot` + `GET /sidebar/boot` y `/sidebar/session`, contrato 1.2.0; `data-boot` idéntico byte a byte en 7 contextos, 9 tests, mutación 8/8) · **T2 ✅ la apertura sin Alpine** (`resources/js/cajon/controller.js`, `window.JumpWeb.cajon`, `data-jw-*`, eventos `jw:cajon:*`; sonda en navegador 17/17 y vista en rojo sin el proxy, 20 tests JS, mutación 13/13) · falta T3 carcasa en el paquete · T4 hoja propia (huella 24/24) · T5 página ajena
