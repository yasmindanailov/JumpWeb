...
- [ ] F5 · instancia PlayJump y la landing fuera, v2.0.0 · **ABIERTA el 19-09 por el CENSO** (`specs/instancia-y-landing-fuera.md` ✅, `#639`). **T1 · el MENÚ, SERVIDO** (`#640`→`#646`, contrato **1.11.0**): los siete platos, uno a uno en su spec §4.1. **T2a ✅** (`specs/paquete-de-instancia.md`, `#647`): el namespace `instancia::`, el invariante **`SEC-12`**, la `plantilla/` y el repo de instancia con remoto privado. **T2b ✅** (`#649`→`#666`; su historia entera en §4.5.bis→§4.7.ter, que es donde no caduca): el contrato producto↔instancia son los DATOS de la vista, no su HTML — nace `CONTRATO_DE_VISTAS`, **las NUEVE vistas de la landing salen del producto** (`pages/` ya no existe y `home` tampoco), el material del cliente sale de `main` (37 ficheros, 9,2 MB) y la suite corre SIN paquete. Medido en todas: huella **0/38**, mismo DOM en es/en/fr, sitemap 11=11; arneses 144/144. ❗ `#664` midió 161 rojos con un anfitrión de nueve líneas y salieron **16**: *la cifra mide el anfitrión, no la página*, y un anfitrión tiene que **consumir el contrato entero**. ▶ Queda la **T2c**, el CSS (`#665`, `[DECIDIDO owner]`); después T3–T5
