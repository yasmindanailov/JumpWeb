...
- [ ] F5 · instancia PlayJump y la landing fuera, v2.0.0 · **ABIERTA el 19-09 por el CENSO** (`specs/instancia-y-landing-fuera.md` ✅, `#639`). **T1 · el MENÚ, SERVIDO** (`#640`→`#646`, contrato **1.11.0**): los siete platos, uno a uno en su spec §4.1. **T2a ✅** (`specs/paquete-de-instancia.md`, `#647`): el namespace `instancia::`, el invariante **`SEC-12`**, la `plantilla/` y el repo de instancia con remoto privado. **T2b ✅** (`#649`→`#666`; su historia entera en §4.5.bis→§4.7.ter): **las NUEVE vistas de la landing salen del producto** con su material (37 ficheros, 9,2 MB), nace `CONTRATO_DE_VISTAS` y la suite corre SIN paquete. Huella **0/38** en todas, sitemap 11=11, arneses 144/144. ❗ `#664` midió 161 rojos con un anfitrión de nueve líneas y salieron **16**: *la cifra mide el anfitrión, no la página*. **T2c ✅** (`#667`, §4.6.bis): las 208 huérfanas eran **17** y **128 no las pintaba nadie**; podadas 119 (−20,3 KB, huella 0/38) y nace el **trinquete** de CSS huérfano. **T3·1 ✅** (`#668`, §4.6.ter): salen los dos recursos con CERO uso —el widget de ofertas y el complemento por atracción— y **el contrato de instancia sube a 2**, la primera vez; los otros cuatro esperan y «las secciones de texto» no existen (medido). Sigue el resto de la T3, T4 y T5
