...
- [ ] F5 · instancia PlayJump y la landing fuera, v2.0.0 · **ABIERTA el 19-09 por el CENSO** (`specs/instancia-y-landing-fuera.md` ✅, `#639`: tres decisiones del owner contestadas; el censo, en su §1). **El MENÚ, SEIS platos servidos** (`#640`→`#645`, contrato **1.8.0**, arnés 34/34): `/site` con lista blanca —y un secreto no sale ni declarándolo—, el horario en dos rutas por cómo se cachean, las normas, los legales interpolados, los precios por tarifa y la ficha de producto y zona —cuya foto se SUBE al hueco de la instalación (`#645`), no se escribe como ruta al repo—. ▶ Sigue la prueba social sin avatares
