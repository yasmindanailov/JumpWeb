...
- [ ] F5 · instancia PlayJump y la landing fuera, v2.0.0 · **ABIERTA el 19-09 por el CENSO** (`specs/instancia-y-landing-fuera.md` ✅, `#639`: tres decisiones del owner contestadas; el censo, en su §1). **T1 ✅** (`#640`): `PublicFacts` con lista blanca por recurso —y un secreto no sale ni declarándolo— y `GET /api/v1/site`. **T2 ✅** (`#641`): el horario en dos rutas por cómo se cachean (`/schedule` y `/schedule/now`) y el «abierto ahora» calculado UNA vez, que ahora comparte con el chip del hero. **T3 ✅** (`#642`): las normas activas, traducidas por el servidor y en el orden del recorrido de una visita. Contrato 1.5.0; mutación 17/17. ▶ Siguen legales, precios, fichas y prueba social
