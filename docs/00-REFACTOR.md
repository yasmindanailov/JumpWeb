...
- [ ] F5 · instancia PlayJump y la landing fuera, v2.0.0 · **ABIERTA el 19-09 por el CENSO** (`specs/instancia-y-landing-fuera.md` ✅, `#639`). **T1 · el MENÚ, SERVIDO** (`#640`→`#646`, contrato **1.11.0**): los siete platos, uno a uno en su spec §4.1. **T2a ✅** (`specs/paquete-de-instancia.md`, `#647`): el namespace `instancia::`, el invariante **`SEC-12`**, la `plantilla/` y el repo de instancia con remoto privado. **T2b ✅** (`#649`→`#666`; su historia entera en §4.5.bis→§4.7.ter, que es donde no caduca): el contrato producto↔instancia son los DATOS de la vista, no su HTML — nace `CONTRATO_DE_VISTAS`, **las NUEVE vistas de la landing salen del producto** (`pages/` ya no existe y `home` tampoco), el material del cliente sale de `main` (37 ficheros, 9,2 MB) y la suite corre SIN paquete. Medido en todas: huella **0/38**, mismo DOM en es/en/fr, sitemap 11=11; arneses 144/144. ❗ `#664` midió 161 rojos con un anfitrión de nueve líneas y salieron **16**: *la cifra mide el anfitrión, no la página*, y un anfitrión tiene que **consumir el contrato entero**. **T2c ✅** (`#667`, detalle en §4.6.bis): las 208 huérfanas eran **17** y **128 no las pintaba nadie**; podadas 119 (−20,3 KB) con huella 0/38, y nace el **trinquete** de CSS huérfano sobre TODAS las hojas (deuda 15, solo encoge). ▶ Sigue la **T3**, T4 y T5
...
- [ ] Google Business Profile (`#524`, lo lleva el SPA): **T1 CERRADA** (`#720`→`#726`) y **T2·1→T2·5 EN EL ÁRBOL** (`#727`→`#731`): las tablas con sus dos plazos, el recorrido con su `coherent()`, la pasada diaria —**se escribe lo que se vio, se borra solo con una pasada coherente**—, las **imágenes desde nuestro servidor** —que quita la dependencia del consentimiento— y **«Ocultar»**. El detalle, §4.1. ▶ Faltan la sección con su contrato, el botón del panel y retirar Places; más el proyecto de Google de JumpSystem y los 60 días de la ficha. · las bandas · los tres ejes de la pasada de vestido.
