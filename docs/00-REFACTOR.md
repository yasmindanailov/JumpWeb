...
- [ ] F5 · instancia PlayJump y la landing fuera, v2.0.0 · **ABIERTA el 19-09 por el CENSO** (`specs/instancia-y-landing-fuera.md` ✅, `#639`: tres decisiones del owner contestadas). **T1 · el MENÚ, SERVIDO** (`#640`→`#646`; hoy contrato **1.11.0**): `site`, horario, normas, legales, precios, la ficha de producto y zona, y la cifra de prueba social. **T2a ✅** (`specs/paquete-de-instancia.md`, `#647`): el namespace `instancia::`, el invariante **`SEC-12`**, la `plantilla/` y el repo de instancia. **T2b en curso** (`#649`→`#653`): el contrato producto↔instancia son los DATOS de la vista, no su HTML; nace el contrato de vista y tres barridos bajan tres reglas al producto (dirección, números, resumen). ▶ Siguen `/contacto`, mudar las vistas, y T3–T5
...
- [x] T5 · la página pública (§4.6 y §10.5–§10.6), **cinco unidades, en el árbol y sin desplegar** (`#701`→`#706`, arneses 8/8 y 7/7): la página y sus temas, contestar, el recibo de 2 h, el flujo firmar↔invitación y el calendario.
...
- [x] T7 · los correos, **CERRADA en tres** (§10.14–§10.17, `#714`→`#717`; cruzó al carril de correos, avisado antes): el correo del post-form · el lector de «qué queda por hacer» · el aviso de la víspera a las **18:00 del día antes** (`[DECIDIDO owner]`). Arneses 8/8 · 10/10 · 12/12 +1 declarado. Inventario **26**. ❗ Enseñó que **el constructor de consultas de Eloquent SÍ toca `updated_at`**: la spec lo afirmaba al revés desde el diseño.
- [ ] ▶ **La invitación: código completo y ✅ del owner en vivo** (20-09; los tres huecos que ese ✅ no cubre, en el §0 de la spec). Falta **desplegar** (T5–T7 + migración), **encender** (dato del owner) y el **borde §7.1·5, ANTES de encender**: apagada no muerde; encendida, un «sí» adoptado se cae al bajar invitados.
