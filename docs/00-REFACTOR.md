...
- [ ] F5 · instancia PlayJump y la landing fuera, v2.0.0 · **ABIERTA el 19-09 por el CENSO** (`specs/instancia-y-landing-fuera.md` ✅, `#639`). **T1 · el MENÚ, SERVIDO** (`#640`→`#646`; hoy contrato **1.11.0**): los siete platos. **T2a ✅** (`specs/paquete-de-instancia.md`, `#647`): el namespace `instancia::`, el invariante **`SEC-12`**, la `plantilla/` y el repo de instancia. **T2b en curso** (`#649`→`#657`): el contrato producto↔instancia son los DATOS de la vista, no su HTML; nace el contrato de vista, cinco reglas bajan al producto y **`/contacto`, `/normas`, `/bar`, los legales y `/atracciones` están MUDADAS** (`#654`→`#657`: anfitriones mínimos, suite SIN paquete, huella 0/38). ▶ Siguen `precios`, `cumpleanos`, `servicios`, `home` y T3–T5
