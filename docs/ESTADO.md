...
> ❗❗❗ **ENCARGO ABIERTO, DIAGNOSTICADO Y SIN CONSTRUIR — EL SELLO DEL MODO DE UN COMPLEMENTO** (`[owner, 2026-09-08]`: *«deberíamos hacerlo»*; **se retoma en otro chat**). ▶ **El síntoma**: el owner intentó poner la hora extra «por invitado» en producción y el panel lo rechazó. **No es un fallo, es el cerrojo de `#443`** — pero el owner vio lo que el diseño no: **esa ventana NO SE ABRE NUNCA**. Con venta continua siempre hay una fiesta viva por delante, así que el «espera al 22/09» es un espejismo que se cierra en cuanto el mostrador vende la siguiente. ▶ **LA RAÍZ, medida**: `order_items.quantity` de una línea hija **significa cosas distintas según el modo** —con `fixed` son BLOQUES de tiempo, con `per_guest` son PERSONAS— y **el modo solo vive en la configuración del enganche**, así que cambiarlo **reinterpreta datos ya guardados**. ⚠️⚠️ **La comparación que lo explica entero, y es del owner**: *el PRECIO ya está sellado, y lo está porque vive en la línea* (`order_items.unit_price`), así que tocar el catálogo no mueve lo vendido; **el MODO no se guarda en ninguna parte de la línea, y por eso se escapa**. ▶ **EL ARREGLO ES `PAY-19`, que YA es la ley aquí** —*una fiesta conserva las condiciones con las que se vendió*— con el precedente exacto de `#288` (`order_items.age_family_seal`): **guardar el modo en la línea al nacer**. Con él, el cerrojo `ProductAddon::hasEditableSoldLines()` **SOBRA y se retira**. ⚠️ **Y el relleno de lo existente es correcto POR CONSTRUCCIÓN en los extensores, precisamente porque el cerrojo impidió cambiar el modo desde `#443`**: la configuración de hoy *es* la que se vendió — *el cerrojo deja el terreno limpio para retirarlo*. En los complementos que NO alargan la estancia el relleno sería **la mejor conjetura**, porque antes de `#443` sí se podía cambiar sin rastro; se dice así, no se disfraza. ▶ **Las dos reglas derivadas, que NO son excepciones nuevas sino las ya decididas**: cambiar el PRODUCTO re-sella (es otro producto, otros enganches); mover la FECHA re-precia y **conserva** el sello (`PAY-18` + Q2 de `#288`). ❗ **DOS COSAS SIN DECIDIR, y son del owner**: **(1) el ALCANCE** — sellar solo los extensores o **todos** los complementos (recomendado: todos, con la conjetura del relleno declarada); **(2) spec primero o diseño dentro del commit** (la casa manda spec cuando se toca dinero). ⚠️ **Es una TANDA COMPLETA, no media hora**: migración · escribir el sello en las TRES puertas por las que nace una línea · leerlo en el editor · retirar el cerrojo · relleno · y **`VERIFY_CONC=1`, porque toca dinero**. ▶ **Contexto que dio el owner y cambia el riesgo**: las dos reservas que hoy sostienen el cerrojo (**#39** JUMP 21/09 18:00 y **#24** KIDS 21/09 17:30) se crearon **a mano en el parque**, con el empleado explicando precios, **y se liquidan allí al precio pactado** — así que el daño que el cerrojo evita no les afecta. Por eso **hoy no se tocó nada**: con la venta online cerrada no se pierde un euro esperando a tener el sello bien hecho, en vez de meter un segundo producto que luego habría que retirar.
...
> ▶ **Último usado: `#447`** (sub-banda `#440`–`#449`, la de este carril; **quedan dos: `#448`–`#449`**). Otras sub-bandas vivas: `#450`–`#459` (diseño, último `#452`) y
