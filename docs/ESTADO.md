...
> - **F0 en el árbol**: la spec, las siete decisiones, la fila y las reglas 8 y 9 del enrutador. **Sigue F1, la doc
>   caliente**, que reescribe `CLAUDE.md`, este fichero y el tracker. ⚠️ **El carril del SPA se PARA durante F1** y
>   hace `git pull --rebase` al volver: el otro ordenador estaba apagado al abrir F0.
> - ⚠️ Docker Desktop estaba apagado en esta máquina al abrir F0: el `pre-push` exige el stack arriba.
...
> |---|---|---|---|
> | 🏗️ **Plataforma · producto e instancias · este ordenador** | **610–639** (último `#616`) | **`specs/producto-e-instancias.md`** §0 y §4.9 | ▶ **F0 en el árbol** (2026-09-16: spec, `#610`–`#616`, reglas 8 y 9 de `CLAUDE.md`) · **sigue F1**, la doc caliente; el carril del SPA en pausa mientras dure |
