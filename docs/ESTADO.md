...
> ❗❗❗ **POR DÓNDE SE RETOMA — LEE ESTO Y NADA MÁS DE ESTE BLOQUE.** 🚀 **LOS DOS ENCARGOS DEL OWNER ESTÁN EN PRODUCCIÓN Y VERIFICADOS** (`#446`, 2026-09-08, commit `200b019a`): la **hora extra de un pack se cobra por invitado** (`#443`) y **el cliente sube y baja sus invitados desde el post-form** (`#444`). ▶ **QUEDAN DOS COSAS Y LAS DOS SON DEL OWNER**, ninguna es código: **(1)** configurar en el panel el modo «Se cobra por invitado» y su precio — ⚠️⚠️ **el CANDADO lo bloquea hasta el 22/09**, medido en producción con el método ya desplegado: los enganches `#121` (JUMP) y `#122` (KIDS) tienen **una fiesta viva cada uno el 21/09** (18:00 y 17:30), y cambiar el modo con líneas vendidas editables las re-preciaría en la primera edición de cantidad; las salidas son esperar, crear un producto nuevo (`#427`) o tocar esas reservas —lo último desaconsejado, son de cliente—; **(2)** la **pasada de OJO por navegador**, que tiene guion cerrado en `VERIFICACION-E2E-CAJON.md` §5.octies y cubre las dos features con su escenario sembrado (A: sembrar · B1–B5 la hora extra · C1–C7 el número de invitados). **Hasta esa pasada las dos siguen 🟦 y no ✅** (`/dod`). ⚠️ **Producción va en `200b019a` y `main` en `be2d91b9`, y eso NO es un desfase que haya que desplegar**: lo que va detrás son `#447` (un arreglo de ARNÉS, solo `tests/`) y doc, y la doc ni siquiera viaja por rsync. **No hay nada pendiente de desplegar.**

> 🚀 **EL SELLO DEL MODO ESTÁ EN PRODUCCIÓN Y VERIFICADO** (`#448`/`#449`, 2026-09-08, commit `e76d6f2a`; sexto despliegue, `ENTORNOS.md` §6). ▶ **EL ENCARGO DEL OWNER QUEDA DESBLOQUEADO**: los enganches `#60` (Hora extra JUMP) y `#61` (KIDS) están **LIBRES** — ya se les puede poner «Se cobra por invitado» desde el panel, que es lo que el candado impedía y cuya ventana «no se abría nunca». ▶ **Medido tras migrar y coincide con la simulación al dígito**: 3 filas selladas · 25 hijas vivas sin sello (lo que el candado sigue protegiendo) · candado 9 → 7 · salud 9/9 · ni una línea en el log. ❗ **LO ÚNICO QUE QUEDA ES TUYO: la pasada de OJO** (`VERIFICACION-E2E-CAJON.md` §5.nonies) **y configurar el modo**. ⚠️ Y antes de configurarlo, lee el aviso de §5.nonies·D2: los otros siete enganches siguen candados a propósito y se sueltan solos cuando pasen sus fiestas. — **Lo que fue:** ▶ **El guion del ojo está cerrado**: `VERIFICACION-E2E-CAJON.md` §5.nonies (A siembra · B el desbloqueo · C la hora extra · D los controles), y dice también lo que NO cubre. ⚠️⚠️ **NADA DE ESTO ESTÁ DESPLEGADO**: producción sigue en `200b019a`, y **la T2 sí cambia conducta** (a diferencia de la T1), así que el despliegue se decide a la vista y no de corrido. ▶ **Con la T3 el encargo del owner queda desbloqueado**: el candado pasa a preguntar «¿hay ventas vivas SIN SELLO?», las dos horas extra de sala se sueltan el día del despliegue (las sella el relleno) y el resto se suelta solo cuando pasen las fiestas vendidas antes — *muere por vaciamiento, no por decreto, y no hay ni un instante sin defensa*. ▶ **La T1 escribe el hecho y NO lo lee nadie**, y ése es su contrato: la suite entera salió verde **sin tocar un solo caso existente** (4.445 → 4.460), así que es segura de desplegar sola. Ejecución y sus dos lecciones en **§12.16**. ⚠️⚠️ **Y dejó un hallazgo que NO es de esta feature: 14 de los 22 arneses de mutación DEJAN EL ÁRBOL MUTADO si el proceso muere** (`trap … EXIT` no corre con SIGKILL) — pasó de verdad aquí, dos ficheros se quedaron mutados y lo cazó la suite con **1 fallo de 4.460**; `mutar-sello-modo.sh` queda endurecido y sirve de MOLDE (copia en ruta fija, reparación al arrancar, integridad al terminar), los otros 13 tienen ficha en `DEUDA.md`. — **Lo que sigue vigente del diseño:** Diseño completo en **`specs/hora-extra.md` §12**; empieza por **§12.12, que son las cinco decisiones ya tomadas** (`[DECIDIDO owner, 2026-09-08]`), y sigue por §12.2 (producción medida) y §12.4 (el censo, que es lo que decidió el alcance). ▶ **El síntoma**: el owner intentó poner la hora extra «por invitado» y el panel lo rechazó — **no es un fallo, es el candado de `#443`**, pero él vio lo que el diseño no: **esa ventana NO SE ABRE NUNCA**. Medido llamando al método real sobre producción: **bloquea 9 de 29 enganches**, y entre ellos los menús, que **toda fiesta vendida lleva** (`groupDefault()` siempre elige uno del grupo) — ese conjunto se rellena con cada venta. ⚠️⚠️ **La comparación que lo explica entero, y es del owner**: *el PRECIO ya está sellado porque vive en la línea*; **el MODO no se guarda en ninguna parte de la línea, y por eso se escapa**. ▶ **EL PLAN SON CUATRO TANDAS** (§12.14) y las tres primeras tocan `CRITICAL_RE` → **`VERIFY_CONC=1`**: T1 el hecho sin leerlo (migración + tres puertas, *la suite sale verde sin tocar un caso*) · T2 las tres lecturas + la regla de divergencia · T3 el candado re-apuntado y `ProductAddon` al gate · T4 doc y OJO del owner. ❗❗❗ **TRES CORRECCIONES AL DIAGNÓSTICO DEL 08 QUE VAN ANTES QUE AQUEL TEXTO**: **(a)** NO es solo dinero — **la otra mitad es AFORO** y no estaba escrita en ningún sitio (`blocksFor()` → `extra_minutes` del padre: **900 minutos de sala** con 15 invitados); **(b)** el disparador es más ancho que «editar la cantidad» — **mover el DÍA re-alarga la fiesta con el modo de hoy** (`OrderItemEditor::changeSlot()`), y en ese mismo `forceFill` la línea siguiente re-precia bien el sello de EDADES: *la doctrina ya está aplicada justo al lado de la línea que la ignora*; **(c)** la configuración vive en **DOS tablas** — `extends_parent_stay`, `duration_min` y `seats_per_unit` son de `ticket_types` y **este sello no las alcanza**. ❗❗❗ **Y EL DAÑO YA OCURRIÓ**: la línea #5 del pedido **`R-BOMAZH`** (Pack Jump, 17 invitados, fiesta 21/09 17:00) se vendió el **01/09** como 1 menú a 2,00 € y el enganche pasó a `per_guest` el **06/09** — la primera edición de cantidad la pondría en **34,00 €, +32,00 € que nadie vendió**. **Es la ÚNICA** (control: 1 descuadre sobre 27 hijas vivas) y se sella como `fixed` en la migración, nominada. ⚠️⚠️ **CADUCA §11.11·A3**, que decía «cero enganches `per_guest` en producción»: **hay cuatro**. ⚠️ **El relleno NO puede salir del pivote vivo** (convertiría un error hoy corregible en un hecho inmutable, y sobre `R-BOMAZH` escribiría una mentira): solo los **dos extensores**, y su prueba es el **rastro** —las 4 filas de `audit_logs` tocan `108` y `110`, nunca `121` ni `122`—, no el candado, que nació el 08. ⚠️ **El candado se RE-APUNTA, no se retira** («¿hay líneas vivas **sin sello**?», conjunto que solo encoge): las dos horas extra se desbloquean el día del despliegue y **no hay ni un instante sin defensa**. ▶ **Si el owner tiene prisa**: un **complemento nuevo** ya en `per_guest` no toca ninguna línea vendida (verificado) — ⚠️ sin desenganchar el viejo. ❗❗ **CONVIENE CERRAR ANTES `GuestCountAdjuster`** (§12.15·4): **no re-escala las hijas por-invitado** y `invitados-en-post-form.md` afirma dos veces que sí — hoy no muerde porque ningún extensor es `per_guest`, **y se activa exactamente con el cambio que el owner quiere hacer**.
