...
>
> ▶ **DÓNDE ESTÁ EXACTAMENTE.** La **Fase 1** (el sistema) tiene **cinco tandas dentro y verdes**: táctil **44→48** en el producto · radios colapsados a **0·10·16·999** en el paquete · aire de sección **240/160 → 144/96** con token nuevo · columna **1176 → 1120** con hueco nuevo por instalación · y la **escala tipográfica** (`#474`), los diez niveles con nombre. Dos más se cerraron **sin código** (`#473`): el movimiento se queda en **cuatro curvas** —`#262` sigue en pie— y el **punto de corte se APARCA hasta la Fase 2**. ⬜ **Queda SOLO la T1h, los 65 iconos.**
>
...
>
> ⚠️⚠️ **TRES COSAS QUE NO PUEDES NO SABER ANTES DE TOCAR NADA DE ESTO.** **(1)** El paquete del cliente **`public/css/client.css` está GITIGNORADO**, así que **los valores de PlayJump NO viajan en ningún commit**: son **paso de despliegue** y la lista viva está en la **§5.bis** de la spec. Si miras la web y no ves el cambio, mira si el paquete local lo tiene. **(2)** `Tests\Support\ReadsSiteStylesheets::siteSheets()` **EXCLUYE `client.css` a propósito** —lo dice su docblock—, así que **para aseverar algo del paquete NO se usa el trait**: se lee el fichero con `file_get_contents`. Dos guardas escritas hoy vigilaban una cadena vacía por creer lo contrario, y **lo cazó una mutación, no una relectura**. **(3)** **NO hay sonda de navegador, y ahora se sabe por qué**: comprobado el 2026-09-09, **no hay ningún navegador instalado ni en el contenedor ni en el host** (ni Playwright, ni Chromium; el host no tiene ni `node`). Se han movido táctil, aire y columna sin verlo renderizado, y `TouchTargetTest` no mide píxeles —lo dice su propio docblock—. La pasada está planificada **una sola vez al final de la Fase 1** (spec §5.ter) y **hay que instalar el navegador y reconstruir la sonda** —vivía en `/root/e2e/tap44.mjs`, fuera del repo— leyendo antes sus dos trampas medidas. ▶ Es lo que hace que `TypeScaleTest` sea **aritmética**: afirma lo único sostenible sin verlo pintado. **(4)** **Los tokens del canvas van por v1.10, no v1.9** — cambiaron de versión en el mismo día: **se releen antes de cada tanda**, no una vez por carril.
>
> ▶ **LO SIGUIENTE, EN UNA LÍNEA: los 65 iconos (T1h)**, que cierra la Fase 1. ⚠️ **Va a traer decisiones del owner**, y el protocolo de este carril (`#469`) es **preguntárselas todas antes de construir**, aunque sean pequeñas — en la T1g fueron **cuatro** y la cuarta salió de que medir dejó sin sujeto a la respuesta de la tercera.
>
...
> Suite **4494 en verde** (27.656 aserciones, 1 skipped a propósito), medida el **2026-09-09** sobre el
> árbol con las cinco primeras tandas del carril de diseño (`#469`→`#474`, **+13**: la guarda de
> escalones inventados de `ShapeScaleTest`, los tres casos de `RhythmScaleTest`, el suelo táctil
> que un paquete no puede bajar, los **siete** de `TypeScaleTest` y el trinquete que obliga a
> `SidebarTokenBudgetTest::SIN_ESTRENAR` a encoger). ⚠️⚠️ **En un CLON LIMPIO el total NO cambia pero los saltados sí**:
