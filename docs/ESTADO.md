...
>
> ▶ **LA T2e ESTÁ EN EL ÁRBOL: la sección 04 «Cumpleaños»** (`#483` + **`#484`, el ojo del owner**). ❗❗❗ **Y `#484` VALE PARA TODO EL CARRIL, no solo para esta sección**: el owner vio «un recuadro sin border radius detrás de la card de Jump» y era **`[data-surface]` PINTANDO el contenedor** —esa regla no solo declara la superficie, hace `background: var(--bg)`—, así que con el atributo en el `<li>` ése se volvía un rectángulo de tinta pura con **radio 0** y la misma caja que la tarjeta. *Declarar una superficie no es solo cambiar tokens: es pintar.* ⚠️ Y antes de eso la pegatina de tinta **no se leía**: keyline, relleno y sombra casi el mismo color (contraste **1,19**). ▶ **Y la FOTO a sangre se retira** (`[DECIDIDO owner]` sobre tres opciones renderizadas): el problema no era cómo estaba puesta, es que `zones.image` para cumpleaños es **el comedor vacío** —el artboard lo tenía pendiente como «foto del cumple montado»—. La sección abre con `.sec-head`, la cabecera común de las ocho. Cabecera **sobre foto** —la única que la lleva—, el **reloj de las dos horas**, **dos tarjetas de pack** que se comparan al lado y el **bloque de complementos con el molde de las entradas**. Suite **4.554** · desborde 0 en las 13 vistas · la sección baja de **2.489 a 1.795 px** en móvil y la portada de **13,43 a 12,65 pantallas**. ❗❗❗ **LO QUE NO PUEDES NO SABER**: **por TERCERA vez en el carril, el acta del canvas describe piezas que su propio artboard tiene APAGADAS** —aquí el reloj y el aviso INFO, desde el recorte del 7 sep, igual que la chapa de zona de `#480` y la del «18 más» de `#482`—. *Esta fuente no se relee antes de cada tanda: se relee antes, mientras dura, y sin creerse el acta.* ⚠️ Y el artboard **se movió durante la tanda**: apareció un **turno 8** («Tu fiesta, tu manera») como propuesta abierta. ▶ **Cuatro decisiones del owner**: el **paso a paso SALE** de la portada (615 px, sigue entero en `/cumpleanos`) · las tarjetas **NAVEGAN**, no venden —*«en la landing va la promesa, no la lista»*— · el turno 8 **entra con el molde de las entradas** · y el **reloj entra en las DOS superficies**, contra el artboard. ▶ **El bloque de complementos pasa a `<x-site.addons-rail>`**, compartido por 02 y 04; la extracción se verificó **byte a byte**. ❗❗ **La edad que se publica es la del PACK** (4–7 y 8+), no la de su zona: es el campo que **cobra el suplemento mixto**, así que publicar otro tramo prometería un precio distinto del que se cobra. ❗❗ **El reloj NO reparte**: un diagrama de tramos promete horario aunque la letra diga lo contrario. ❗❗❗ **DOS DEFECTOS REPRODUCIDOS EN NAVEGADOR, con la suite en verde**: (1) la tarjeta oscura no declaraba su **superficie** —*pintarle el fondo no le cambia los tokens*— y su viñeta salía **tinta sobre tinta** (contraste 1,1); (2) ⚠️⚠️ **el hover de la pegatina estaba mal desde `#478` en TODA la portada**: el paquete no declaraba `--shadow-float-hover` / `-press`, así que la tarjeta reposaba con la sombra dura del cliente y **saltaba a la difusa del producto**. *Un comentario que describe un mecanismo no lo implementa.* Arreglado **también para la sección 01**, con guarda vista morder. ⚠️⚠️ **Y un localizador de guarda que acotaba mal, cazado de rebote**: `RateRailSectionTest::panel()` recortaba «hasta el siguiente panel» y para el último se llevaba el resto del documento — contó 6 fichas donde su panel pinta 2 (la lección de `#314` por otra puerta). ⚠️ **Una medición mía salió FALSA y la cacé por otra vía**: los packs daban 0,00 € porque leí `amount` en vez de `amount_cents`. ▶ **PASO DE DESPLIEGUE, y es un ARREGLO, no una mejora**: `--shadow-float-hover: 2px 2px 0 var(--paper-fg)` y `--shadow-float-press: 0 0 0 var(--paper-fg)` en el `client.css` de producción; sin ellos las secciones 01 y 04 conservan el defecto de la sombra. ▶ **LO SIGUIENTE: la T2f, sección 05 «Antes de venir»** (`Antes de Venir PJP` 2a + `Escritorio PJP` 3a). ⚠️ **Relee su artboard antes Y durante**: en las dos últimas tandas se movió mientras se construía.
>
