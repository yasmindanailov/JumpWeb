...
> ⚠️⚠️ **DOS INTERMITENTES ABIERTAS, LAS DOS SIN CAUSA ATRIBUIDA — y eso es deliberado, no pereza.** **(a)** `PackStayExtensionTest` se pone rojo bajo `--parallel` cada varias pasadas, **en dos métodos distintos**; verde en el fichero solo (3/3), en paralelo, en tres suites completas y en el método suelto. **Descartadas POR MEDIDA**: el día de la semana y el memo `once()` de `RateResolver::forDates()` (experimento controlado cebándolo antes de crear la tarifa `special`: siguió pasando). **(b)** `QrLogoTest`, el caso del rasterizador REAL: `rsvg-convert` tarda **34–125 ms** en reposo y **571 ms** en el peor de 40 muestras con la suite completa encima, contra un tope de **2,0 s** — o sea que el tope es **candidato, NO causa reproducida**. ▶ **Las dos tienen ya el INSTRUMENTO puesto**: sus asertos dicen el motivo, así que la próxima aparición se lee en una pasada en vez de reproducirse. Fichas en `DEUDA.md` con las cifras.
>
