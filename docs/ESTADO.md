...
> ❗❗❗ **POR DÓNDE SE RETOMA — LEE ESTO Y NADA MÁS DE ESTE BLOQUE.** ✅ **LA HORA EXTRA DE UN PACK YA SE PUEDE COBRAR POR INVITADO** (`#443`, 2026-09-07, `[DECIDIDO owner]` la opción A de `hora-extra.md` §10.2: *«la hora extra para todos los invitados, pero se cobra por invitado»*). **Sin migración y sin tocar el núcleo del dinero**: la regla vive en UN sitio (`AddonOccupancy::blocksFor()`) y dice que los **BLOQUES** salen del enganche y no de la cantidad — por-invitado, la cantidad son PERSONAS y el bloque es **1**. Suite **4.414** · **12/12 mutaciones** nuevas y **22/22** del arnés anterior · **los NUEVE escenarios** de `purchase:verify-oversell` sobre InnoDB, con el nuevo (`stay-extension-per-guest`) **visto FALLAR** sin la derivación (0 de 12 compras). ▶ **LO QUE QUEDA ES DEL OWNER y es CONFIGURACIÓN**: poner el modo «Se cobra por invitado» y el precio por invitado. ⚠️⚠️ **Y hay un CANDADO que lo va a frenar hasta el 21/09**, a propósito: cambiar el modo de un enganche con reservas vivas todavía editables **re-preciaría esas fiestas** en la primera edición de cantidad (medido: +35,00 € y +56,00 € sobre las dos horas extra reales de producción), así que el dominio lo rechaza; las dos fiestas con hora extra son del **21/09** y hasta que pasen el modo está bloqueado. **La salida, si lo quiere antes: un producto nuevo** (`#427`). ▶ ❗❗ **Y se midió en producción algo que NO es defecto sino DECISIÓN**: la hora extra de sala **no se vende viernes, sábado ni domingo** —el 06-09 a las 15:21 se borró su precio de la tarifa `special` desde el panel, y un complemento sin precio ese día no se ofrece—; `[DECIDIDO owner, 2026-09-07]` **se queda así**. `ENTORNOS.md` §6 llevaba la nota contraria y está corregida.
>
> ✅ **Y EL OTRO ENCARGO TAMBIÉN ESTÁ CONSTRUIDO: el cliente ya puede subir y bajar sus invitados desde el post-form** (`#444`, spec `invitados-en-post-form.md` §8). **Sin migración**; el único dato nuevo es un ajuste del parque (`packs.guest_count_cutoff_hours`, vacío = 24 h). ▶ **29 casos · 16/16 mutaciones · un escenario NUEVO de concurrencia (`guest-count`) visto FALLAR**: doce clientes subiendo +6 a la vez sobre un cupo de 30 dejaron **96 invitados vivos en la franja** sin la revalidación, y **30** con ella. ▶ **Lo que queda es el OJO del owner, y tiene guion: `VERIFICACION-E2E-CAJON.md` §5.octies**, que cubre las DOS features (`#443` y `#444`) con su escenario sembrado — incluidas las TRES filas que comprueban que los tres rechazos dicen cosas distintas, que es el corazón de la pasada. ⚠️⚠️ **Es la PRIMERA puerta por la que el cliente mueve AFORO**, que es justo lo que `complementos-post-reserva.md` §4.3·5 evitó a propósito: va con el MISMO `ZoneDaySlotLock` que la compra, como primera sentencia, y el dinero POST-COMMIT. ⚠️ **El SUELO son DOS motivos y no se funden** (el mínimo del pack se resuelve llamando; «ya has asignado más plazas» se resuelve quitando a alguien de la lista), y con eso se cierra por su lado la ficha de la hoja de sala que imprimía **plazas negativas**. ⚠️ Y `Booking` no puede mirar a `Identity`, así que lo que ya tiene dueño llega por un contrato nuevo (`ReservationPlacesTaken`) con el binding en el composition root. — ▶ **El diagnóstico original, para el registro:** Reproducido: manda 12 fichas para una línea de 10 y **se guardan 10, las dos de más se descartan en silencio** (`sanitizeGuestData()` recorta a `quantity` y la vista pinta exactamente `quantity` fichas, sin botón de añadir). ⚠️ **Lo que se resolvió en `#413` fueron los COMPLEMENTOS, no los invitados.** ▶ **`[DECIDIDO owner, 2026-09-07]` las cuatro de producto**: se puede **subir Y bajar** (*«no hay arbitraje, no es un negocio de este estilo… aparte lo ve el operador»*) · plazo **hasta un día antes como mínimo** · techo el **`max_qty` del pack (20)** · **basta con el libro**, sin correo al operador. ⚠️⚠️ **Es la primera vez que el cliente movería AFORO**, que es justo lo que `complementos-post-reserva.md` §4.3·5 evitó a propósito: va con lock de zona/día y con su escenario de `purchase:verify-oversell` visto fallar. ▶ **Medido en producción**: 16 fiestas vivas, 8–20 invitados (mediana 13), **2 ya en el techo**, **0 ediciones de cantidad** en seis días de operación real y 3 de 16 post-forms completos — el valor está en cuando se abra la venta online, y el riesgo es de hoy.
>
> **A.00 · LO ANTERIOR DE ESTA JORNADA.** 🚀 **EL 2026-09-07 SE DESPLEGÓ LA SESIÓN DE HOY Y ESTÁ VERIFICADO EN PRODUCCIÓN** (`#440` + `#441`, commit `b442168`; copia de la BD previa en `~/backups/playjump2_main-20260907-182903.sql.gz`, 318 K, gzip íntegro): el mostrador ya pide el teléfono que falta y **declarar un menor y aceptar su exención son un solo gesto**. ▶ **LO QUE QUEDA ES DEL OWNER, y son TRES cosas**: **(1)** su **✅ por navegador** de las dos features en `playjump.es` —el paso 1 del pedido manual con un cliente sin teléfono, y el alta de un menor con su casilla—; **(2)** las **tres pasadas de la rejilla de media hora** que siguen pendientes desde el 06-09 (`PANEL-ADMIN.md` §4.1); **(3)** confirmar que las dos líneas del cron del panel siguen activas (`#115`). ▶ ❗❗❗ **Y HAY UN ENCARGO ABIERTO, DIAGNOSTICADO Y SIN CONSTRUIR: el cliente NO puede añadir invitados desde el post-form.** Reproducido: manda 12 fichas para una línea de 10 y **se guardan 10, las dos de más se descartan en silencio** (`sanitizeGuestData()` recorta a `quantity` y la vista pinta exactamente `quantity` fichas, sin botón de añadir). ⚠️ **Lo que se resolvió en `#413` fueron los COMPLEMENTOS, no los invitados.** ⚠️⚠️ **Y no hay ni una frase que le diga qué hacer**: el post-form tiene «Llámanos» para CINCO situaciones (las tres de edad sin producto, el extra fuera de plazo y el que no se pudo cambiar) y ninguna para ésta. ▶ El camino existe pero pasa por el operador (subir la cantidad desde «Gestionar producto», que re-tarifica por `PAY-18` y revalida aforo). **`[owner, 2026-09-07]`: se itera el DISEÑO en otro chat**; ⚠️ ojo, la versión cara **es DINERO y AFORO** y choca con `#244` («cualquier gestión de dinero post-reserva ya cobrada se hace en las instalaciones»), así que es spec propia, no una pantalla.
>
...
> Suite **4443 en verde** (27.359 aserciones, 1 skipped a propósito), medida el **2026-09-07** sobre el
