...
> 🏗️ **SI ENTRAS NUEVO · carril PLATAFORMA (F0 abierta el 2026-09-16, este ordenador): `#610` → `#616`**
> - El programa «producto e instancias» vive en **`specs/producto-e-instancias.md`**: lee su **§0** y nada más para
>   saber qué cambia; las fases F0 → F6 están en su §4.9. Las decisiones son `#610` → `#616`, banda **610–639**.
> - **F0 cerrada**: la spec **✅ aprobada por el owner el 2026-09-16**, las siete decisiones, la fila y las reglas 8 y 9
>   del enrutador, empujadas con el gate en verde. **Sigue F1, la doc caliente**, que reescribe `CLAUDE.md`, este
>   fichero y el tracker: empieza por la spec §4.8 y §4.10. ⚠️ **El carril del SPA se PARA durante F1** y hace
>   `git pull --rebase` al volver: el otro ordenador estaba apagado al abrir F0.
> - Queda del owner: la pila de la app (F6), las herramientas de análisis estático y el modo de permisos del harness.
>
...
>   (el campo vivía fuera del `<form>`) — arreglado en el árbol, **pide despliegue**;
> - ⚠️ el despliegue lleva **cuatro líneas nuevas en el `client.css` de producción** (`--done`,
...
>     REAL del scheduler;
>   - coste de unos 11 USD al mes (Place Details Enterprise + Atmosphere: 25 USD por 1.000, 1.000 gratis);
>   - tope diario de la consola: 100;
>   - la IP del servidor (51.38.54.41) ya está admitida en la clave: 4,9 · 50 reseñas.
> - **`#592` · sin cookies de Google la sección ya no desaparece**:
>   - queda la nota y un aviso, «Elegir cookies», que abre el panel y recarga al conceder;
>   - banner, panel y política dicen **«Mapa y reseñas (Google)»**, y `POLICY_VERSION = 2026-09-13`
>     **vuelve a pedir el permiso a todos**;
>   - `/cookies` vive en la BD: la migración sustituye lo intacto del producto, y el párrafo reescrito de esta
>     instalación fue por el script gitignorado `aplicar-produccion-592.php`.
> - **`#593` · titulares de la portada**: cambian tres —«Una zona para cada edad», «Llega con tu QR y a
>   saltar», «Horarios y cómo llegar»— y se quedan cinco, con su motivo en `DECISIONES`.
> - **`#594` · ❗❗ REDSYS EN `live` DESDE EL 2026-09-13** (lo activó el owner en el panel):
>   - TPV real probado de punta a punta: `R-VPCOHW` se cobró y se devolvió por REST;
>   - la GUARDA 1 del despliegue admite `live` solo en producción;
>   - el despliegue de `#593` dejó **3 minutos de 503**, y por eso **`[DECIDIDO owner]` los despliegues a
>     producción se hacen DE NOCHE o con el parque cerrado**.
> - ⚠️ **Trampas nuevas**:
>   - un `artisan cache:clear` en producción también borra las reseñas, así que después va
>     `social-proof:refresh`;
>   - un `*/` dentro de un comentario PHP (p. ej. `lang/*/…`) lo cierra y da un error de sintaxis.
> - En local las reseñas son las 3 de ejemplo: Google rechaza la IP de casa, y el refresco no se programa solo.
>
> ▶ **Por dónde retomar, en orden**:
> 1. la **T6**, en/fr de lo que escribe el panel (dudas, normas, nota de acceso);
...
> 4. Google Business Profile (spec `#524` aprobada, código sin empezar).
...
> (su terminal no incluye datos en la redirección) — por eso **`redsys_merchant_url` en producción era
> paso obligatorio del go-live** (`sistemas/REDSYS.md` §14.bis) — ✅ puesta por el owner el 2026-09-13, con
> Redsys ya en `live` (`#594`). ⚠️ Su tarjeta «denegada»
...
> |---|---|---|---|
> | 🏗️ **Plataforma · producto e instancias · este ordenador** | **610–639** (último `#616`) | **`specs/producto-e-instancias.md`** §0 y §4.9 | ✅ **F0 cerrada** (2026-09-16: spec aprobada por el owner, `#610`–`#616`, reglas 8 y 9 de `CLAUDE.md`) · ▶ **sigue F1**, la doc caliente; el carril del SPA en pausa mientras dure |
> | 🎨 **Diseño de la web · este ordenador** | ~~470–499~~ · ~~520–549~~ → **580–609** (último `#594`) | **`specs/contenido-y-copys.md`** · `specs/rediseno-desde-canvas.md` §5.5 | ✅ **`#580`–`#585`**: la fachada en producción, iconos del kit, «Reservar» en cian, fuera el reloj, los complementos y los iconos de la web, el pie del cajón en primario y la etiqueta destacada en amarillo · ▶ **EN CURSO — CONTENIDO Y COPYS** (`#586`): decisiones del owner tomadas sobre los carteles del parque; **T1+T2 aplicadas en LOCAL** con el script de contenido (fuera del repo), **T3+T4 en el árbol** (`#587`), **`#589`** (los regalos, cada uno en su etiqueta amarilla —en un complemento, solo dentro de su «Más info»—; la escala de `/normas` con su regla y sin franjas pisadas; la entradilla de zonas y su «i»; el eslogan del hero; la pista de invitados del pack y el pie de `/servicios` para otros grupos) y **`#588`**: la edad del cumpleañero decide el pack (la web bloquea y recomienda, el panel avisa), un servicio vende varios productos (las excursiones salen de cumpleaños y `/servicios` lee la tabla de sus tramos y resume los cumpleaños), la nota de acceso bajo las zonas desde el panel, y **la T5 (textos del cajón) hecha aquí** con «Confirmado» — ✅ **EN PRODUCCIÓN desde el 2026-09-13** (`#590`, séptimo despliegue, `89e49ed0`: la web nueva con el contenido de la sesión y las divergencias de configuración decididas por el owner; compra online CERRADA hasta el TPV real — ⚠️ caducado: desde el 2026-09-13 Redsys está en `live`, `#594`); después, **`#591`–`#594` también en producción** (ver la cabecera de este documento); **queda la T6** (en/fr de lo que escribe el panel: dudas, normas, nota de acceso) y **el título de la pestaña de la portada, que dice «Murcia»** (`landing.footer.tag`) · ▶ **Para el agente del SPA**: `#584`/`#585` tocaron el cajón (`.bk-cta` en `--action`, la barra de fase en `--secondary`, `.catalog__badge` en `--marker`) y **`#588` también**: `TimeStep.vue` (tipo `celebrant_age` y el mínimo del pack), `CartStep.vue`, `PayStep.vue` (política y pago seguro), `ConfirmedStep.vue` (siguiente paso), `RegisterForm.vue` (pista del descargo), `line-problems.js` (+ test), la poda de `register` en `layout.blade.php` y textos de `tickets.php`/`account.php`/`auth.php` en los tres idiomas · y **`#589`**: `GiftList.vue` (nuevo) y `TimeStep.vue` (los regalos, SOLO dentro del «Más info» de un complemento —la tarjeta del catálogo no los lleva, `[DECIDIDO owner]`— y la pista de invitados del pack), `tickets.php` (`gifts_label`, `guests_change_later`) y `gifts` en cinco esquemas del contrato — **haz `git pull --rebase` antes de seguir** · antes: portada CERRADA · **Fase 3 EN CURSO**: el armazón de páginas — T3a·1 (destinos) ✅ `#521` · T3a·2 (el pie y las velas) ✅ `#522` · el ojo del owner (pie de la portada en papel, «Reservar» abierto) ✅ `#523` · la spec de Google Business Profile ✅ aprobada, **código no empezado** (`#524`) · T3a·3 (la cabecera de página) ✅ `#525` · T3a·4 (el cierre en las interiores) ↩️ construida en `#526` y **revertida por `#527`** (`[DECIDIDO owner]`: las interiores acaban en el pie, sin tarjeta) — **el armazón, COMPLETO** · las páginas: `/atracciones` ✅ `#481` · **`/cumpleanos` ✅ `#528`** (revisada en vivo por el owner; sin sus dos fotos hasta que las mande) · **el vídeo del hero, el nuevo** ✅ `#529` · **el segundo ordenador, montado** ✅ `#530` · **`/precios` ✅ `#531`** (dos columnas de precio entero, la hora extra como fila y los festivos) · **la FOTO de la zona en `/cumpleanos`** ✅ `#532` (`[DECIDIDO owner]` con el rechazo de `#484` delante; falta la de la mesa, que el parque no tiene) · **`/normas` ✅ `#533`** (el MOMENTO y el PORQUÉ entran como columnas —el hueco que midió §4.2—, la escala sale del dato y las edades las mandan el catálogo y la zona, no el artboard) · **`/contacto` ✅ `#535`** (los canales del dato, el formulario con su tema y su aviso, la chapa de atajos del inventario y la dirección escrita **sin mapa**) · **`/bar` ✅ `#536`** (la carta como IMAGEN subida en «Ajustes → El bar», su puerta de vuelta en la portada; **el bar se llama «Cantina»**, `[owner, 2026-09-12]` — puesto en LOCAL, y es **paso de despliegue** en producción: sin nombre la ruta da 404 y el destino no sale en ninguna superficie) — **las SIETE páginas del inventario, construidas**; queda `/servicios` (grupos, PAUSADA por el owner) · **las reseñas van DESPUÉS del diseño** (`[owner, 2026-09-11]`) |
...
> | 🏦 **Pasarela / producto (este ordenador)** | **4xx** (último `#454`; libres `#455`–`#459`) | `sistemas/REDSYS.md` §14.bis · `ENTORNOS.md` §1 | staging es el entorno de validación del banco · ✅ **go-live hecho el 2026-09-13**: producción en `live`, con cobro y devolución reales probados (`#594`) |
> | 🧩 **Diseño del SPA (el cajón) · el OTRO ordenador** | **550–579** (último `#571`) | **`CARRIL-SPA.md`** · `specs/rediseno-desde-canvas.md` §5 (Fase 4) · **`specs/celebracion-e-invitacion.md`** | ▶ **EN CURSO · el formulario de celebración, el justificante y la invitación digital** (spec `#569`, revisada dos veces) · ✅ **T1 `#570`**: la piel del formulario desde el canvas —texto bajo 15 px **164 → 20**, controles bajo 48 **3 → 0**, color de zona **6 → 0**— y el justificante hereda el molde. ⚠️⚠️ **`focused-layout` no cargaba `client.css` desde que existe** (las dos páginas iban con los colores del producto): arreglado. ⚠️ «Guardar» en el secundario (`#539`) y no en tinta: pendiente del ojo del owner · ✅ **T2 `#571`**: fichas pendientes arriba y listas plegadas, «Falta edad», barra de guardar pegada, extras en filas y el **pegado de la lista de nombres** (medido en navegador: 17 nombres, 20/20 en su sitio tras guardar, cero errores de JS). ⚠️⚠️ **Pintar las pendientes arriba reordenaba a los invitados al guardar**: se ordena por clave en el dominio. ❗❗ **DEFECTO EN PRODUCCIÓN DESDE `#444` (08-09), YA ARREGLADO EN EL ÁRBOL: el número de invitados del post-form NO SE ENVIABA** —el campo vive fuera del `<form>`—; con `form="gf-form"` y verificado en navegador (20 → 19). Pide despliegue · **lo siguiente: la T3** (la piel del justificante). ▶ **Para el agente de la web (compartido que tocó `#570`)**: `landing.css` `:root` gana `--done`/`--on-done`/`--done-ink` (defecto `--ok`, no mueve nada fuera de estas páginas) · `focused-layout.blade.php` carga `client.css` · este carril toma `.guardian__*`, `resources/views/reservation/**` y `lang/*/guestform.php`/`guardian.php` · **el ARMAZÓN COMPLETO y las SEIS PARADAS CERRADAS: las 25 pantallas del cajón están construidas** · ✅ **`#568`** (`[DECIDIDO owner]`, 2026-09-13): **el cajón abre EN el producto** desde la tarifa y los packs de `/servicios` (intención `{ type: 'product', id }`; la de sección ahora DESPLIEGA la tarjeta), CTA «Ver entradas / Ver grupos» en las tarjetas cerradas, Grupos «Servicio personalizado y merienda», avatar del artboard y **fuera «Cerrar sesión» del bloque**, `ui/pack` vigente del set y banderines en Grupos · ✅ **`#567`**: el mismo menor en dos entradas se compra (`distinct` con dos comodines era GLOBAL), el desenlace sin el bloque de cuenta con «Ir a mi cuenta», «¿Quiénes vienen?» en una fila · ✅ **T4·10 · LA PARADA 06** (`#566`, `[DECIDIDO owner]`): el suelo táctil del cajón, los antetítulos y el documento legal como control. ❗❗❗ **TRES censos del canvas se quedaban cortos y se midieron antes de tocar nada**: los antetítulos eran **CINCO** y no tres —su propio artboard dibuja cuatro—; la política la pintan **las DOS** altas y el `<summary>` del descargo está en **CINCO** sitios con los mismos 20 px; y la grieta 14 ya estaba cerrada con su segunda mitad **sin sujeto**. ⚠️⚠️ **Y una ficha PROPIA estaba mal**: `#563` acusaba a `.bk-back` de medir 20 contra 48 y **CUMPLE** —su pseudo le da 48—; aquella medición leyó la **CAJA** y no el **ÁREA**, la trampa de `#264`/`#307` por tercera vez. El que sí falla es el **aspa de cerrar**, y peor de lo fichado: **15×26**, o sea que tampoco llegaba de ancho. ⚠️ Pero `.bk-back` cumplía **por casualidad aritmética** (`inset: -14px` escrito cuando el control medía 17 y el suelo 44): hoy lee el token · ▶ **LA GUARDA VA PRIMERO** — `SidebarTouchTargetTest` es lo que `CARRIL-SPA.md` §7.8 pedía: censa los controles **desde el MARCADO** (una lista a mano se queda corta en silencio) y exige que cada uno DECLARE cómo llega al suelo; los **24** que hoy no lo hacen quedan nominados con su cifra en una lista que **solo encoge**. ⚠️⚠️ **Se asevera lo DECLARADO y no lo medido a propósito**: la sonda mide CAJAS y acusa a cuatro controles que cumplen por su pseudo — *un instrumento que acusa a lo que ya está bien no sirve para escribir una guarda* · ▶ **`WaiverDoc.vue`** recoge los CINCO `<details>` escritos idénticos, con el **chevron** (despliega aquí) frente a la **flecha** de la política (sale a otra página) · ❗❗ **DEFECTO VIVO que destapó buscarle sujeto a una regla**: `NoPasswordHint` pinta un `<button>` dentro de una pista **sin ninguna regla**, así que heredaba el gris del párrafo y salía sin subrayado — el defecto exacto que `#350` midió para el enlace de privacidad, una pantalla más allá y sin verlo nadie en tres meses. *Una regla que se queda sin sujeto no siempre sobra: a veces falta mirar quién más hacía lo mismo sin ella* · ⚠️ **El arnés dio 11/15 y las cuatro que no mordían eran debilidades MÍAS** (entre ellas, el chevron aseverado por SUBCADENA y un `__() !== ''` que **no podía fallar**, porque Laravel devuelve la clave); tras arreglarlas, **15/15** · ⚠️⚠️ **El techo del chunk NO sube y quedan 0,01 KiB** (283,99 de 284): la poda obvia —extraer la fila a un componente— **se midió y no paga** (284,17), y queda escrito en el ledger para que nadie la repita · **Medido**: aspa 15×26 → área **48×48** · las dos filas legales 20 → **48** · antetítulos **5 → 0** · selectores acusados 17 → 16 (134 → **126** apariciones) · ✅ **T4·9 · LA PARADA 05, las nueve pantallas de la cuenta** (`#564` el color · `#565` las piezas). ❗❗❗ **TRES badges FALLABAN el contraste HOY con el paquete del cliente puesto** —«Pagado» **2,64**, «Cancelado» **3,88** y «Gratis» **2,64**, éste también en la landing—: el paquete sobrescribe `--ok`/`--err` y **no sus fondos claros**, que eran siete hex del PRIMER cliente. Hoy se DERIVAN del color sólido (14 % fondo · 30 % borde · texto en tinta) **en el producto**, así que una instalación que declare su verde y su rojo obtiene sus avisos sin escribir nada. ⚠️⚠️ **Y el error que cometió la propia tanda solo lo vio la CAPTURA**: `--money` vale tinta por defecto, así que poner el rol en los importes PENDIENTES no cambia nada en la suite — y con el paquete dejó «A pagar en el parque» en color y «Pagado» en tinta, al revés que el artboard. El color se reserva a lo YA COBRADO · ✅ **las dos preguntas que las hacía el NAVEGADOR** —quitar un menor y borrar la cuenta— pasan dentro del cajón con `ConfirmInline`, que lleva el gesto entero; ⚠️ su primera versión dejaba fuera el disparador y **`PrivacyZone` se pasó de su techo de líneas**, que es lo que lo delató · ✅ el texto bajo el QR era **el de otra pantalla** (grieta 08), «Mis pagos», «Privacidad y datos» y «Justificantes de los invitados» · ⚠️ **el artboard estaba CADUCADO en tres de sus ocho puntos** (el color del área lo cerró `#551`, el táctil ya es 48 y el «Volver» ya estaba corregido) · ✅ **T4·8b · LOS CUATRO DESENLACES** (`#563`, `[DECIDIDO owner]`): **tres frases que el servidor ya tenía escritas y la pantalla no pedía** —`verify_hold` con CERO consumidores desde hace meses, a qué correo se ha escrito, y la HORA de caducidad, que el denegado resumía en «unos minutos» teniendo el dato en la misma respuesta que ya pedía—; la **puerta al CARNÉ** en la reserva creada (**el mismo código de siempre: no hay un QR por pedido**) y el denegado pierde «hacer otra reserva», que era el peor consejo con la plaza aún guardada. ⚠️⚠️ **HALLAZGO QUE NO ES DE LA TANDA: el armazón del cajón no llega al suelo táctil** — `.sidecart__close` mide **26 px** en las 32 pantallas medidas y `.bk-back` **20**, contra 48; son los dos controles que salen en TODAS y **no lo ve ninguna guarda** (`TouchTargetTest` recorre rutas públicas, la lección de `#557`). Ficha en `DEUDA.md`, tanda propia y del owner · ✅ **T4·8a · EL PASO DE PAGAR** (`#562`, `[DECIDIDO owner]`): el titular decía **cuatro veces la misma palabra** en 390 px y hoy dice el trabajo («Repasa tu reserva»); el **enlace de las condiciones sale de la casilla** a su propia fila de 48 (medido: inline eran 19), **compartiendo receta con «Ver más fechas»** en vez de copiarla. ⚠️⚠️ **Y el bloque que el cliente RELLENA no lo vigilaba ningún diff desde `#349`**: su caso de contrato nace sin sujeto —comprador con teléfono y sin condiciones publicadas— **y el renderizador SSR no componía `need`**, así que no había forma de que apareciera · ✅ **T4·7 · LA PARADA 03** (`#561`, `[DECIDIDO owner]`): el paso 05 —«donde se pierde gente»— con la pestaña del SISTEMA (era la de las ZONAS del parque), la entradilla que da un MOTIVO en vez de explicar el obstáculo, el descargo nombrado entero y la pista del teléfono en el alta. ⚠️⚠️ **Hallazgo: un arreglo A MEDIAS de `#551`** — el enlace de la política de privacidad seguía en `--zone-1` (**2,45** de contraste): aquella tanda arregló el enlace legal DENTRO de una casilla y no éste, que es el mismo FUERA de ella, y `form__*` está fuera de su censo · ✅ **T4·6 · LA CESTA** (`#558`): la línea cambia de ESTRUCTURA —azulejo · nombre+cuándo · precio · quitar— y **se deshacen dos tarjetas anidadas** (cada menor era una tarjeta con borde dentro de la tarjeta de la línea, lo que `#484` prohibió en la portada) · ✅ **`#559`**: la línea decía «+ 1× Calcetines» — **tres defectos en una frase**, y el artboard no los veía porque **transcribió nuestro propio texto** · ❗❗ ✅ **`#560` · EL ÚNICO DEFECTO DE DATOS**: el campo obligatorio de una línea restaurada **se borraba del DOM con la primera letra dentro** —emitía en cada pulsación y `pending` lista los VACÍOS—, así que el nombre del homenajeado se guardaba con un carácter; hoy hay borrador con «Guardar»/«Descartar» · ✅ **T4·5 · EL DÍA Y LA HORA** (`#557`, vestidas desde `Pasos Compra PJP` 4a): títulos como PREGUNTAS a 24, «Ver más fechas» como FILA con el chevron del set, el chip de día a 64×80 con su número en rótulo, y **el stepper de 32 redondo a 44/48 cuadrado** — ⚠️⚠️ **doce píxeles por debajo del suelo táctil en el control más pulsado del embudo, y ninguna guarda lo veía**: `TouchTargetTest` recorre RUTAS de la web pública y el cajón no es una ruta. ⚠️ **Dos cosas del artboard NO se copian**: el mes horizontal (su dibujo son 4 días de un mes; la tira lleva 182 que cruzan varios) y sus tallas de texto pequeñas (es anterior a la grieta 00 — del artboard, la GEOMETRÍA; las tallas, el suelo de `#550`) · ✅ **T4·4c · CON SESIÓN, «QUIÉN ERES» NO ES UNA FASE** (`#556`, **lo vio el owner**): con sesión el carrito manda DIRECTO al pago, así que esa pantalla no se visita nunca — y la banda **prometía dos donde quedaba una, el contador saltaba del 3 al 5, y la fase salía HECHA** sin que el cliente hubiera estado. Hoy cuenta cuatro. ⚠️ La señal es `auth()->id()` **al pintar la página**, no la sesión de ahora: con el estado vivo, quien se identifica por el camino vería desaparecer la fase al completarla. **16/16 mutaciones** · ✅ **T4·4b · LA BANDA DE CINCO FASES** (`#555`, `[DECIDIDO owner]`): del día al pago, **una fase por PANTALLA** —lo que hace honesto el contador, que antes repetía «Paso 3» en dos pantallas— y **un solo «Volver»** en el embudo, el de la banda (los pasos 4, 5 y 8 pierden el suyo). ⚠️⚠️ **Defecto que solo vio la sonda: «Your basket» se RECORTABA en inglés** (66 px en un carril de 64,4), y `scrollWidth` lo daba por bueno — se mide la TINTA con un `Range`. **13/13 mutaciones** · ✅ **T4·4a · EL PIE DEL ARMAZÓN** (`#554`, `[DECIDIDO owner]` sobre las dos formas **renderizadas**): el CTA a **fila completa** con el tamaño grande del sistema (56 · 18/34 · 800) y **el paso 08 ancla en LO QUE SE COBRA** — con señal el pie decía el total y a la tarjeta iba otra cifra; hoy la banda dice «Total» y «A pagar en el parque» y pegado al botón va lo que va a la tarjeta. ⚠️⚠️ **Tres cosas del canvas no se sostuvieron al medirlas**, y la que más cambia el plan: **la banda de fases NO es trabajo de servidor** (lo dice en tres sitios) — `Purchase.php` está retirado y hoy la compone el cliente. **17/17 mutaciones** · ✅ **T4·1 · la GRIETA 00** (`#550`): el cuerpo del cajón al suelo del sistema · ✅ **T4·2 · la GRIETA 01** (`#551`): el cajón deja de pintar la ACCIÓN con la marca — `.btn--zone` **retirada del producto** y sustituida por `.btn--ink`, «Pagar» como **único** relleno de acción del embudo, y los cinco textos que daban **2,21–2,70** de contraste a **5,81–18,50**; censo de `--zone-*` **29 → 13**, **17/17 mutaciones** · ✅ **T4·3 · la TARJETA GRANDE del catálogo** (`#552`): dos tarjetas con su franja y su frase, el cumpleaños en tinta, «Servicios» → **«Grupos»**, y el defecto medido del precio de un pack (**159 px de 324**, la descripción en 55 y cortada) → las siete filas iguales · ✅ **T4·3 bis · la PUERTA de categoría** (`#553`, **la propuso el owner**): las dos categorías salen CERRADAS y al pulsarlas se abren en la tarjeta grande — la bifurcación se ve **sin pantalla nueva**; puerta cerrada **154 px** y el panel **817** contra 1.033; **16/16 mutaciones** · **lo siguiente**: la **T4·4b, la banda de CINCO fases** (`[DECIDIDO owner]`), del día al pago — y con ella el «Volver» de carrito, identificarse y pagar, que hoy traen el suyo propio y con la banda serían dos · **el reparto por FICHERO con la web está en `CARRIL-SPA.md` §5 y §7**, y lo compartido se avisa AQUÍ antes de tocarlo |
>
> ▶ **Para el agente de la web** (lo COMPARTIDO que tocó `#566`, y es poco): **`layout.blade.php`**
gana **`urls.privacy`** —`route('legal.privacidad')`, que las dos altas del cajón necesitan suelta
desde que el enlace salió del texto— y el subgrupo `register` **pierde su `array_replace`**, que
existía solo para interpolar ese `:url` dentro de `privacy_notice`. De paso `eyebrow` sale de las
listas de `register` y de `verify`: las cinco pantallas de auth pierden su antetítulo.
> ⚠️ **`account.register.privacy_notice` ya NO lleva marcado** en los tres idiomas: si alguna vista
tuya lo pintara con `v-html` o `{!! !!}`, hoy es texto plano. (Verificado: cero consumidores fuera del
cajón.) ⚠️ Y **`account.reset.eyebrow` y `account.orders.eyebrow` NO se han tocado a propósito** —los
pinta la WEB con `<x-site.page-head>`, en restablecer contraseña y en reintentar el pago—: hay un caso
que impide que alguien «termine la limpieza» y te deje esos dos rótulos en blanco.
> ⚠️ En `site.css` lo único fuera de los bloques del cajón es **`.form__hint a` → `.form__hint a,
.form__hint button`**: solo AÑADE un selector, así que ninguna regla tuya cambia. Entra porque
`NoPasswordHint` pintaba un botón dentro de una pista sin regla ninguna.
> ⚠️ **No toqué ninguna lista global de guardas** (`MotionBudgetTest`, `ShapeScaleTest`,
`TouchTargetTest`, `InteractionColourIsNotAZoneTest`, `ActionFillTest`): verificado fichero a fichero.
▶ La guarda nueva del suelo táctil del cajón (`SidebarTouchTargetTest`) es **hermana** de la tuya y no
la toca: aquélla recorre RUTAS públicas, ésta censa el marcado del cajón.
>
> ▶ **Para el agente de la web** (lo que tocó `#564`, y es UNA regla tuya): **`.addons-mini__badge--free`**
—el distintivo «Gratis» de los complementos en la landing— deja de pintar `color: var(--ok)` sobre
`var(--ok-bg)` y pasa a tinta. ⚠️ **No es estética: era un contraste de 2,64** con el paquete del
cliente puesto, porque el paquete sobrescribe `--ok` y **no** su fondo claro. ▶ Con ella, **los siete
colores heredados del PRIMER cliente salen del producto**: `--ok-bg`, `--ok-border`, `--err-bg`,
`--err-border` y `--err-hover` se DERIVAN ahora de su color sólido con `color-mix` (14 % / 30 % / el
88 % de `actionHover()`), nacen `--attn-bg` y `--attn-border`, y se RETIRAN `--warn`, `--warn-hover`,
`--err-strong`, `--refund`, `--refund-bg` y `--on-warn`. ⚠️ Si alguna vista tuya los nombraba, hoy no
existen —verificado: cero usos fuera de `site.css`—; el aviso es `--attn`, el texto sobre un tinte es
`--fg` y cualquier cifra es `--money`. ⚠️⚠️ **`SemanticFillTextTest` encoge su lista de `--on-*` a dos**
y gana tres casos; recorre las DOS hojas, así que si estrenas un aviso en la landing te hablará.
>
> ▶ **Para el agente de la web** (lo compartido que tocó `#562`, y es poco): **`layout.blade.php`
gana `urls.terms`** —`route('legal.condiciones')`, que el paso de pagar necesita suelta desde que el
enlace salió del texto de la casilla— y **`due_terms` sale de su `array_replace`**, porque ya es texto
plano y interpolarlo sería pasarle un `:url` que nadie sustituye. `terms_link`, el párrafo de quien ya
aceptó, **sigue viajando interpolado y con su `<a>` dentro**. ⚠️ **No toqué ninguna lista global de
guardas** (`MotionBudgetTest`, `ShapeScaleTest`, `TouchTargetTest`, `InteractionColourIsNotAZoneTest`,
`ActionFillTest`): verificado fichero a fichero. ⚠️ Y en `site.css` lo único fuera de los bloques del
cajón es **`.sidecart__panel .check input`**, que está acotado al panel a propósito: la casilla sube a
24 px **solo dentro del cajón**, y ese bloque ENCOGE el día que vistas el formulario de contacto, el
post-form o el justificante, que emiten la misma clase.
>
> ▶ **Para el agente de la web** (lo que el cajón EMPEZÓ A USAR en `#561`, y no cambié): la barra de
«Entrar / Crear cuenta» del cajón adopta **`.tabset` / `.tabset__tab`**, tu pestaña del sistema —la de
`/precios`—, en lugar de `.zone-tabs` / `.zone-tab`. **No he tocado ni una línea de `landing.css`**,
pero desde hoy el cajón depende de dos cosas suyas: que `.tabset` declare **`display: flex`** de forma
incondicional (el panel es estrecho SIEMPRE, y sin eso las pestañas comparten línea con el «Volver»)
y que `.tabset__tab` conserve su **`flex: 1`** (o los dos botones se dimensionan por la longitud de su
rótulo: medido en `#125`, 103 px y 156 px, y «Entrar» se lee como menos que «Crear cuenta»). Las dos
las asevera ahora `SidebarTokenBudgetTest::the_drawer_tabs_do_not_depend_on_the_viewport`, así que si
las mueves te saldrá en rojo con el motivo escrito. ⚠️ Y `.zone-tab` **sigue viva y sin tocar**: la
...
>
> ═══════ 🎨 CIERRE DEL CARRIL DE DISEÑO · 2026-09-12, MADRUGADA (`#543`→`#549`) ═══════
>
> ▶ **LA PASADA DE VESTIDO, SEGUNDA TANDA: LAS PIEZAS DE FACHADA SOBRE LA PORTADA — y el detalle que
el ojo del owner fue puliendo.** `#543` las **bandas de enlace** (una familia para lo que había cuatro
veces) · `#544` las tres **manchas** detrás de los titulares de sección · `#545` el **laboratorio**
(`/_diseno/splash` y `/_diseno/siluetas`, solo en `local`: siete colocaciones de cada familia con lo
que cuesta cada una) · `#546` las **dos variantes sobre la portada real** (`?fachada=1|2`) · `#547` la
variante 2 pulida con seis puntos del owner, más el **arco `F3`** y el **trío `G4`**, que el producto
COMPONE a partir de poses sueltas · `#548` las tres colocaciones de **MÓVIL** · `#549` el detalle: el
**sello de zona con edad y altura**, el **«Más info»** de la ficha de complemento, las **flechas
laterales** del carril y las **opiniones que se deslizan en todos los anchos**.
>
> ❗❗❗ **LO QUE NO PUEDES NO SABER SI TOCAS UN CARRIL O UNA FICHA DE COMPLEMENTO:**
> · **La PILA de opiniones de `#490` ya no existe.** La sección 06 es un **carril con `scroll-snap`** en
todos los anchos (`[owner]`: «que se pueda hacer slide en todo tipo de dispositivo»), y con ella se
fueron el `visibility` de interruptor y la clase `is-on`. Los puntos **mueven** el carril y el carril
**dice** en cuál estás: son dos direcciones del mismo estado.
> · **Quién enseña las flechas es el CSS, no el servidor**, a partir del `data-rail-scroll` que publica
`ui/rail-sails.js`. Sin JavaScript no hay atributo y no hay flechas: el defecto cae del lado seguro.
> · **`setPointerCapture()` en un `pointerdown` MATA todos los clics de dentro** —el `click` se dispara
sobre quien capturó—, y la suite lo ve verde. La captura del arrastre se pide en el primer
`pointermove` que pasa del umbral. Lo cazó el **control** de la sonda, no una relectura.
> · **El «cómo» se mueve un carril vive en la hoja** (`scroll-behavior` + su excepción de
`prefers-reduced-motion`): un `behavior: 'smooth'` en la llamada de JS gana a la hoja y se la salta.
> · **Un `max-content` se ensancha con lo que despliegas dentro**: la ficha de complemento lo evita con
`width: 0` + `min-width: 100%` en su panel. Medido: abre y cierra sin moverse a lo ancho.
> · **El tinte de identidad de la ficha de complemento se RETIRÓ** (`[owner]`), y al re-apuntar su
guarda apareció un **incumplimiento de la R2 de `#537` en `/contacto`**: dos textos en Humo sobre tinte,
...
18 ranuras de laboratorio siguen declaradas en `IllustrationKit::SLOTS` marcadas como temporales**.
>
...
> `rediseno-desde-canvas.md` §5 planifica cinco (sistema · portada · páginas · SPA · post-form) y el
> canvas ha dibujado **cuatro áreas más** que no están en ese plan. Verificado el 12-09 leyendo
> `doc/pendiente.md` del canvas, que es su lista viva de encargos:
>
> | Área nueva | Artboards | Estado |
> |---|---|---|
> | **El PANEL** | `Puerta` · `Calendario` · `Cliente` · `Escritorio` · `Pedido` Panel PJP | dibujado · **~25 decisiones del owner abiertas** (54–80) |
> | **La APP móvil** | `Mapa App PJP` · `App PJP` · `Armazon App PJP` (JumpPoints + Ruleta, 39 pantallas) | dibujado · decisiones 27–35 · **no existe en el producto** |
> | **Las INVITACIONES** | `Invitaciones PJP` · `Invitaciones Temas PJP` (6 temas) | dibujado · decisiones 36–41 · **no existe en el producto** |
> | **Las BANDAS** | `Bandas PJP` (la de aviso arriba + las de enlace) | dibujado **y decidido por el owner** · decisiones 42–46 |
>
> ⚠️ **Ninguna tiene carril asignado.** No es una propuesta de construirlas: es que quien lea «quedan
> las cinco fases» va a creer que falta menos de lo que falta.
>
> ▶ **DE ESTE CARRIL (diseño web), lo que queda:**
>
...
>    lista de encargos escrita en `doc/pendiente.md`. **Nadie las ha construido.** Es la única fase de
>    la spec sin empezar que **no depende de nadie más**.
> 4. **Las bandas** — el owner ya decidió en su formulario: la banda de arriba anuncia y **la caja de
>    ofertas se retira**. Sin construir.
>
> ▶ **RECOMENDACIÓN, con grupos pausado: la FASE 5.** Está dibujada, está decidida y no bloquea en
> nadie; y hoy el post-form es una página que se rellena de pie y con prisa, sin vestir. La siguiente
> candidata son **las bandas**: pequeñas y ya decididas.
>
> ⚠️⚠️ **LA COPIA LOCAL DEL CANVAS ES DEL 9-SEP Y LE FALTAN ~50 ARTBOARDS** (medido: 23 ficheros en
> `mockup_playjumppark_v2/` contra los ~75 del canvas; no están `Layout Paginas`, `Bar`, `Contacto`,
> `Normas`, `Servicios`, `Atracciones`, `Precios Pagina`, `Formulario Post Reserva`, `Justificante
> Invitado`…). ▶ **Pero `DesignSync` FUNCIONA** —usado cuatro veces el 12-09 sin autorización extra—,
> así que **se lee del canvas directamente** y la D7 de la pasada de vestido ya no bloquea como su
> texto supone. *No te fíes de la copia local: está tres días y cincuenta artboards por detrás.*
>
> ▶ **LO QUE BLOQUEA Y ES DEL OWNER** (datos, no diseño): la **foto de la mesa** de `/cumpleanos` · la
> **carta** y la **foto del local** de Cantina · el **aparcamiento** (la sección 07 y la web publicada
> dicen cosas distintas) · los **festivos de Lorca** · el **horario en conflicto** (publicado L–V
> 16:30 · S–D 11:00 contra el cerrado L–J 16:30 · V–D 11:30) · y **qué pasa con `/entradas`**, que
> existe y no está en el inventario del canvas.
>
> ▶ **DEUDA ALTA VIVA**: **las reseñas de Google se ven el 17 % del tiempo** (caché de 30 min contra
> refresco de 3 h) y en local la clave está **bloqueada por IP**. La salida estándar está investigada
> y **aprobada** —`#524`, la API de Business Profile— con **código no empezado**. `[owner]`: las
> reseñas van DESPUÉS del diseño.
>
> ▶ **Un instrumento que NO se versionó y conviene versionar**: las capturas de página completa. Hoy
> encontraron **tres** defectos que ninguna sonda vio (la foto del bar a 900 px de alto, la chapa
> despegada 272 px y el enlace de privacidad invisible), y el script vive en `storage/app/captura.mjs`,
> que está **gitignorado** — la lección de `#475`: *un instrumento que no viaja en el repo se vuelve a
> escribir, y al reescribirlo se vuelven a pagar sus trampas*. `medir-seccion.mjs` mide altos y
> `sonda-geometria.mjs` táctil y desborde; **capturas no las saca ninguno**.
>
> ───────────────────────────────────────────────────────────────────────────────────────────
>
...
**tira la premisa con la que el artboard justificaba la página** —él dice que lo que la salva de ser
«un trozo de la portada con otro título» es que los dos horarios son distintos—. Lo que la separa de
«Visítanos» es *qué se hace*: allí se consulta, aquí se escribe. ▶ **Consecuencia: NO hay campo de
horario de atención y no se añade** — el plazo se deriva de `heroStatus` (cero consultas nuevas) y sin
horario publicado no se promete nada. ⚠️ **Fuera el mapa de `/contacto`**, con guarda INVERTIDA (no
basta con que esté bien gateado: es que no esté), y con él se retiran `.map-card`, las cuatro
`.visit-*` que sobrevivían por esta página y la niebla `.spray` con su token. ⚠️⚠️ **Y `#533` había
dejado escrita en `TouchTargetTest` una afirmación FALSA** —«las legales ya no pintan `page__back`»—:
las cuatro lo pintan, verificado con `curl`; la guarda vuelve a `/privacidad`. ▶ **Queda del OWNER y
nada bloquea**: el ojo en vivo y **dos datos del panel** — el teléfono FIJO (hoy solo hay móvil, así
que salen dos canales y no los tres del artboard) y quién lee el WhatsApp; si es el mismo número que
el móvil, se rellena igual y el producto funde las dos tarjetas en una.
>
> ▶ **Cierre del carril de diseño (12-09, mañana)**: `/cumpleanos` gana su foto (`#532`), **`/normas` rehecha
y empujada** (`#533`) y la **página de GRUPOS investigada y PAUSADA** (`#534`). ▶ **Por dónde
retomar**: el owner está iterando la presentación de grupos **en Claude Design**; cuando traiga el
artboard nuevo, la lógica ya está medida y escrita en `#534` — **no hay que volver a investigarla**.
Lo que hay que saber antes de tocar una línea: el producto de excursiones **existe en producción y ya
...
que se verificó decodificándolo entero con `ffmpeg`, `#529`) · la **foto de la MESA** de `/cumpleanos`
—la de la zona ya entró en `#532` con la que el parque tiene— · y
...
>
> ✅ **T4·4b · LA BANDA DE CINCO FASES** (`#555`, `[DECIDIDO owner]`): **con ella el armazón queda
> COMPLETO**. La banda vive en las cinco pantallas del camino —día · hora · carrito · quién eres ·
> pagar— y **el «Volver» del embudo es UNO**: los pasos 4, 5 y 8 pierden el suyo propio, que existía
> solo porque no tenían banda.
>
> ❗❗ **UNA FASE POR PANTALLA, y el argumento es técnico**: la tercera se encendía DENTRO de la
> pantalla de la hora, así que el mismo «Paso 3 de N» salía en **dos pantallas distintas** — y una
> banda que existe para decir cuánto queda no puede repetir su número. El rótulo de la tercera lo
> eligió el owner entre tres, y **es el nombre de la pantalla a la que lleva** («Tu carrito»).
>
> ⚠️⚠️ **EL DEFECTO QUE SOLO VIO LA SONDA, y por poco pasa**: con cinco fases el carril mide **64,4 px**
> a 390, y **«Your basket» pedía 66** — se recortaba en INGLÉS, y en la captura no se nota. ▶ **`scrollWidth`
> LO DABA POR BUENO**: se capa al `clientWidth` cuando el texto cabe, así que devuelve el mismo número
> para un rótulo holgado y para uno al borde (los cinco daban «64/64,4»). La tinta se mide con un
> **`Range`** sobre el nodo de texto. ▶ La causa era decoración —MAYÚSCULAS y `letter-spacing`, ~9 px
> por rótulo— que **el artboard ya no dibuja**: retiradas, los tres idiomas pasan con 6,4 · 8,4 · 6,4.
>
> ⚠️⚠️ **Y JUNTAR EL RÓTULO CON SU DESTINO NO BASTÓ, lo demostró el arnés**: la mutación que separaba
> el destino del rótulo —«Volver al carrito» llevando al catálogo— **no la cazaba ninguna de las doce
> guardas**. La misma fila los mantiene juntos pero no impide que se contradigan dentro de ella; lo
> cierra una **EQUIVALENCIA** (dice «al carrito» si y solo si va al carrito). *Dos listas paralelas
> habrían dejado el mismo hueco.*
>
> ✅ **T4·5 · EL DÍA Y LA HORA** (`#557`): parada 02 del canvas, las dos pantallas por las que pasa
> todo el mundo. ⚠️⚠️ **El hallazgo: el stepper llevaba 32×32** —doce por debajo del suelo táctil del
> producto— **en el control más pulsado del embudo, y ninguna guarda lo veía**: `TouchTargetTest`
> recorre RUTAS de la web pública y el cajón no es una ruta suya. ▶ *Una guarda que recorre páginas no
> vigila lo que no es una página.*
>
> ⚠️⚠️ **Y UNA TRAMPA QUE POR POCO PASA EN VERDE**: envolver el título con su entradilla rompía los dos
> casos del contrato de árbol de ese paso —anclan en `wiz__title` y el diff recorre hermanos
> SIGUIENTES, así que el paso entero (sesenta nodos) se quedaba fuera **y el manifiesto regenerado lo
> daba por bueno**—. Lo cazó **revisar el diff del manifiesto línea a línea**, no la suite. *Un ancla
> se cae cuando cambia lo que tiene ENCIMA, y lo hace sin fallar* — segunda vez en tres tandas.
>
> ✅ **T4·6 · LA CESTA, y con ella la PARADA 02 CERRADA** (`#558`→`#560`): las cuatro pantallas del
> camino vestidas. ⚠️⚠️ **Vestir destapó tres cosas que no eran de diseño**: dos tarjetas ANIDADAS con
> el mismo canto —lo que `#484` prohibió en la portada, vivo aquí—; una línea que decía «+ 1× …» con
> **tres defectos en una frase** y que el artboard no veía porque **transcribió nuestro propio texto**;
> y, el serio, **el campo obligatorio que se borraba del DOM con la primera letra dentro** (`#560`):
> emitía en cada pulsación y `line.pending` lista los que siguen VACÍOS, así que el nombre del
> homenajeado **se guardaba con un carácter**. Reproducido llamando al módulo, no en navegador.
>
> ⚠️ **El ojo de estas cuatro tandas lo puso el OWNER en vivo**, que es como pidió trabajar el carril:
> las tres cosas de arriba las vio él mirando la pantalla, no una sonda.
>
> ✅ **T4·7 · LA PARADA 03, CERRADA** (`#561`, `[DECIDIDO owner]`). El paso 05: **la pestaña era la de
> las ZONAS del parque** —mayúsculas a 13 con espaciado, un rótulo de CATEGORÍA— usada para elegir
> entre entrar y registrarse; hoy es la del sistema. La entradilla **da un motivo** («Tu cuenta guarda
> tu código y tu firma…») en vez de explicar el obstáculo. El descargo **se nombra entero**. Y **la
> pista del teléfono entra en el alta** (grieta 14): era obligatorio sin decir para qué, mientras el
> paso de pagar sí lo hacía.
>
> ⚠️⚠️ **EL HALLAZGO: UN ARREGLO A MEDIAS DE `#551`.** `.form__hint a` —el enlace de la política de
> privacidad— seguía leyendo `var(--zone-1)`: **2,45** de contraste sobre papel. Aquella tanda arregló
> `.check span a`, el enlace legal DENTRO de una casilla, **y no éste, que es el mismo FUERA de ella**
> —justo el que `#350` había sacado—. Se quedó en el hueco entre las dos tandas, y **`form__*` está
> fuera de `PREFIJOS_DEL_CAJON`**, así que el censo no lo veía. ▶ *Una pieza corregida no corrige a sus
> hermanas, y menos si acaba de mudarse.*
>
> ⚠️⚠️ **LO QUE NO HAY QUE VOLVER A DESCUBRIR**: la barra de auth estaba escrita **dos veces** (el paso
> 5 y `AuthTabs.vue`) y hubo que sincronizarla **a mano**; hoy es `steps/AuthTabset.vue` con guarda ·
> **la poda NO podó, otra vez**: extraer ese componente parecía ahorro y medido SUBIÓ el chunk
> (280,08 → 280,17) —*un componente de Vue trae su propio envoltorio y pesa más que el marcado que deja
> de repetir*, la misma medición de `#553`— · y una guarda re-apuntada que quedaba MÁS DÉBIL: pedía
> `display:flex` + `width:100%`, y con la pieza nueva eso pasaba en verde aunque alguien volviera a
> `.zone-tabs`; hoy ata la PIEZA.
>
> ✅ **T4·8 · LA PARADA 04, CERRADA** (`#562` el paso de pagar · `#563` el salto al banco y los cuatro
> desenlaces). ❗❗❗ **LO QUE NO PUEDES NO SABER**: (1) **el bloque «lo que falta antes de pagar» no lo
> vigilaba ningún diff desde que existe** (`#349`) —su caso de contrato nace con un comprador que YA
> tiene teléfono y una instalación que NO publica condiciones, así que `need` salía todo `false` y el
> `.paydue` no se emitía; y aunque el caso lo hubiera pedido, **el renderizador SSR no componía
> `need`**—; (2) **tres frases que el servidor ya tenía escritas no las pedía ninguna pantalla**, y
> `verify_hold` llevaba meses en los tres idiomas con **cero** consumidores; (3) **la sonda del cajón
> llevaba rota desde `#553`** y no llegaba al paso de pagar (las categorías nacen cerradas y sus
> productos siguen en el DOM: el `waitForSelector` pasa en verde y el clic agota el tiempo) — hoy
> abre la categoría por `aria-expanded` y **mide la pantalla de pagar, que no medía nadie**.
>
> ❗❗❗ **Y UN HALLAZGO QUE NO ES DE LA TANDA Y ES DEL OWNER: el ARMAZÓN del cajón no llega al suelo
> táctil.** Medido con la sonda sobre **32 pantallas**: **`.sidecart__close` mide 26 px de alto en las
> 32** y **`.bk-back` 20 en 30**, contra los **48** que el producto declara desde `#470`. Son los dos
> controles que existen en **todas** las pantallas del cajón —cerrar y volver—, y con ellos
> `.pwd-input__toggle` (32) y los dos `.acct__btn` (45). ▶ **No lo ve ninguna guarda**: `TouchTargetTest`
> recorre RUTAS de la web pública y el cajón no es una ruta suya — la misma lección que `#557` pagó con
> el stepper. Ficha en `DEUDA.md` (Alta). **Es tanda propia**: toca el armazón (`#554`/`#555`) y por
> tanto las 25 pantallas a la vez, y lo primero que hace falta es **una guarda que vea el cajón**.
>
> ▶ **Cuatro `[DECIDIDO owner, 2026-09-12]` en esta parada**: el enlace de las condiciones **fuera de
> la casilla** como control de 48 · la reserva creada gana la **puerta al carné** con «hacer otra
> reserva» de fantasma · el denegado **pierde** «hacer otra reserva» · y `pending_at_park` pasa a
> **«A pagar en el parque»**. ⚠️ De este último salió una corrección al artboard: decía que el mismo
> euro se llama de TRES maneras y **dos tienen su motivo escrito desde `#554`** —el verbo entra donde
> cuelga de «Total», que no lo dice, y se queda fuera donde cuelga de «Pagas ahora», que ya lo lleva—.
> *El síntoma era cierto y el alcance no.*
>
> ⚠️ **Tres cosas del artboard NO se hicieron, con su motivo**: renombrar el post-form a «Su día
> especial» (**`#462` ya decidió que no se renombra** y el formulario tiene nombre de cara al cliente) ·
> el `<noscript>` del salto al banco en tinta (**`#551` lo dejó en acción con su motivo**: sin JS ese
> botón ES el de pagar) · y apagar el bloque de «registro del parque», que es **vaciar un ajuste de la
> instalación**, no tocar código — queda como paso de despliegue.
>
> ✅ **EL CAJÓN ABRE EN EL PRODUCTO, LAS TARJETAS INVITAN A ABRIRSE Y EL BLOQUE DE CUENTA SE ALIGERA**
> (`#568`, `[DECIDIDO owner, 2026-09-13]`, tanda corta y sin tests nuevos a petición suya).
> **(1) Del rediseño de `Navegacion Cuenta PJP` se hace SOLO el avatar y la salida**: círculo de tinta
> con la INICIAL (y la persona con «+» sin sesión) y **fuera «Cerrar sesión»**, que sigue en el índice
> de «Mi cuenta». **(2)** Las dos tarjetas del catálogo llevan **«Ver entradas» / «Ver grupos»** mientras
> están cerradas, y Grupos dice **«Servicio personalizado y merienda»** (la zona no es exclusiva).
> **(3)** Iconos: ENTRADAS con el **`ui/pack` vigente del set** (la entrada repetida) y GRUPOS con los
> **banderines**, no el regalo. ⚠️ La variante del set de hoy dibuja la guirnalda a 2,4 y la guarda de
> anatomía la rechaza: se usa la instalada. ⚠️ El set trae también `ui/entrada` y `ui/regalo` nuevos,
> sin tocar. **(4) Abrir el cajón EN el producto**: intención `{ type: 'product', id }` desde las tarifas
> y los packs de `/servicios` —aterriza en «¿Qué día venís?», medido a 390 y 1280—, y la de sección
> (`/cumpleanos`) ahora **despliega** la tarjeta en vez de desplazarse a un acordeón cerrado.
> ▶ Chunk **285,29 KiB → techo 286**; `PurchaseSection` 453 → 464.
>
> ✅ **TRES ARREGLOS DEL OWNER SOBRE EL CAJÓN CONSTRUIDO** (`#567`, `[DECIDIDO owner, 2026-09-12]`).
> ❗❗❗ **LO QUE NO PUEDES NO SABER**:
> **(1) `distinct` con DOS comodines es GLOBAL, y rechazaba compras legítimas**: `items.*.dependent_ids.*`
> compara contra los menores de TODAS las líneas, así que el mismo niño en dos entradas daba **422 antes
> de pagar**, con el texto de Laravel en inglés («The items.0.dependent_ids.0 field has a duplicate
> value»). El dominio lo permite —`check()` es por línea y el índice único también— y **ningún test
> mandaba dos líneas con el mismo menor**. Hoy «sin repetidos» es una regla sobre la LÍNEA
> (`api.dependents.repeated`) y el mismo menor puede ir en líneas distintas.
> **(2) El bloque de cuenta se oculta en las CINCO pantallas de `result`** —revierte lo que la guarda
> exigía, «en el desenlace debe volver»— y la salida principal de la reserva creada es **«Ir a mi
> cuenta» → el índice** (revierte la puerta al carné de `#563`). ⚠️ **Esa puerta no tenía guarda desde
> `#563`**: cambiar la zona o desconectar el evento no ponía nada en rojo.
> **(3) «¿Quiénes vienen?» en UNA fila con rótulos cortos**, y un **quinto** (`guardian_only`): el corto
> habría dicho «Menores a cargo» en el bloque que solo trae el justificante.
>
> ⚠️ **El arnés dio 14/16 y las dos que no mordían eran guardas mías**: `hideRule()` recortaba la regla
> DESDE su primer selector conocido, así que un modo añadido al principio de la lista escapaba; y nada
> vigilaba que `TimeStep` le pasara `offers` al módulo —sin él, el valor por defecto devuelve «menores»
> y la regla, sus casos y los textos siguen en verde—. Tras arreglarlas, **16/16**
> (`scripts/mutar-567.py`).
>
> ▶ **Medido en navegador con control** (390 y 1280): la cabecera con los textos viejos, 73 px en dos
> filas → con los nuevos, **45 px en una**, en los **15 rótulos × 3 idiomas**; la reserva creada,
> entrando por la vuelta del banco, con `.acct` oculto en `is-result` y **visible (209 px) en el
> catálogo**, «Ir a mi cuenta» y el índice con sus 8 tarjetas. ⚠️ Chunk **284,04 KiB → techo 285** tras
> una poda medida de 41 B; `TimeStep` 49 → 50 líneas.
> ❗ **Queda el OJO del owner** en su navegador.
>
> ✅ **T4·10 · LA PARADA 06, CERRADA — Y CON ELLA LAS 25 PANTALLAS** (`#566`, `[DECIDIDO owner]`).
> ❗❗❗ **LO QUE NO PUEDES NO SABER**:
> **(1) TRES censos del canvas se quedaban cortos, y se midieron antes de tocar nada**: los
> antetítulos eran **CINCO** y no tres (su propio artboard dibuja **cuatro**, con «Casi listo»
> colgando del mismo interruptor; el quinto, el alta con Google, ni lo dibuja) · la política la pintan
> **las DOS** altas, y el `<summary>` del descargo —que el canvas invoca como precedente YA resuelto—
> está en **CINCO** sitios midiendo los **mismos 20 px** que el enlace que venía a arreglar · y la
> grieta 14 ya estaba cerrada, con su segunda mitad sin sujeto desde `#350`.
> **(2) UNA FICHA PROPIA ESTABA MAL**: `#563` puso en `DEUDA.md` que `.bk-back` mide 20 contra 48, y
> **CUMPLE** —su pseudo absoluto le da 48 exactos—. Aquella medición leyó la **CAJA** y no el **ÁREA**:
> la trampa de `#264` y `#307`, por **tercera** vez. ⚠️ El que sí falla es el **aspa de cerrar**, y
> peor de lo fichado: **15 × 26**, o sea que tampoco llegaba de ancho, y esa columna no la miró nadie.
> ⚠️⚠️ Pero `.bk-back` cumplía **por casualidad aritmética** —`inset: -14px` escrito en `#237` cuando
> el control medía 17 y el suelo era 44— y el día que `--tap-min` suba se queda corto sin fallar nada.
> **(3) LA GUARDA VA PRIMERO, y es lo que `CARRIL-SPA.md` §7.8 pedía**: `SidebarTouchTargetTest` censa
> los controles **desde el MARCADO** —una lista a mano se queda corta en silencio, como le pasó a
> `SidebarBodySizeTest` con `qr-pass` y `dep-pick`— y exige que cada uno DECLARE cómo llega al suelo.
> Los **24** que hoy no lo hacen quedan **nominados con su cifra**, en una lista con trinquete que
> **solo encoge**. ⚠️⚠️ **Se asevera lo DECLARADO y no lo medido**: la sonda mide cajas y acusa a
> `.bk-back`, `.cart__remove`, `.cal__nav` y `.bk-foot__info-btn`, **y los cuatro cumplen**.
> **(4) UN DEFECTO VIVO que destapó buscarle sujeto a una regla**: al sacar el enlace de privacidad de
> su frase, `.form__hint a` se quedó sin `<a>` que vigilar — y antes de retirarla apareció quién más
> hacía lo mismo sin ella: **`NoPasswordHint` pinta un `<button>` dentro de una pista y NO tenía
> ninguna regla**, así que heredaba el gris del párrafo y salía sin subrayado. Es literalmente el
> defecto que `#350` midió para el enlace de privacidad, una pantalla más allá y sin verlo nadie.
>
> ⚠️ **El arnés dio 11/15 y las CUATRO que no mordían eran debilidades de mis guardas, no mutaciones
> flojas**: el pseudo se daba por bueno sin exigir `max()` ni `position` en el anfitrión · el `height`
> del PSEUDO satisfacía «declara el suelo» antes de llegar a la comprobación que iba a rechazarlo
> —*el alto de un pseudo no es el alto del control*— · los dos antetítulos de la web se comprobaban
> con `__() !== ''`, y Laravel devuelve **la clave** cuando falta, o sea una aserción que **no podía
> fallar** · y el chevron se aseveraba por **SUBCADENA**. Tras arreglarlas, **15/15**.
>
> ⚠️⚠️ **El techo del chunk NO sube y quedan 0,01 KiB** (medido 283,99 de 284). La poda obvia
> —extraer la fila legal a un `LegalRow.vue` compartido por las dos altas— **se intentó y se MIDIÓ**:
> subía a **284,17**, así que se revirtió. *Dos copias de diez líneas de marcado cuestan menos que el
> envoltorio de un componente*: el patrón de `#553`/`#561`/`#563`, no la excepción de `#565`. Queda en
> el ledger de `SidebarBundleBudgetTest` para que nadie la repita.
>
> ▶ **Medido**: aspa **15×26 de dibujo → 48×48 de área** (la cabecera no crece: crecerla de verdad la
> engordaba 25 px en las 32 pantallas) · «Volver» 66×**48**, ahora del token · las dos filas legales
> **20 → 48** · pantallas con antetítulo **5 → 0** · selectores que la sonda acusa **17 → 16** y sus
> apariciones **134 → 126** · cero recortes y cero desborde en las 32.
>
> ❗ **QUEDA el OJO del owner**, y es lo único. Con esta tanda **las 25 pantallas están construidas**;
> lo siguiente que el canvas tiene escrito es el **cuaderno de entrega** del cajón, como el de la
> portada. Capturas del antes y el después en `storage/app/audit/cajon-{antes,despues}-566/`.
>
> ✅ **T4·9 · LA PARADA 05, CERRADA** (`#564` el color · `#565` las piezas). ❗❗❗ **LO QUE NO PUEDES
> NO SABER**: (1) **tres badges fallaban el contraste HOY** con el paquete del cliente puesto —2,64 ·
> 3,88 · 2,64, y el tercero también en la landing—, porque el paquete sobrescribe `--ok`/`--err` y
> **no sus fondos claros**, que eran siete hex del PRIMER cliente; hoy se derivan del color sólido
> **en el producto**, no en `client.css`; (2) **el error que cometió la propia tanda solo lo vio la
> CAPTURA** — `--money` vale tinta por defecto, así que ponerlo en los importes PENDIENTES no cambia
> nada en la suite ni en un clon sin paquete, y con el del cliente dejó «A pagar en el parque» en
> COLOR y «Pagado» en tinta: *un token que por defecto vale tinta esconde su propio error hasta que
> alguien instala un paquete*; (3) **el artboard estaba caducado en TRES de sus ocho puntos** (el
> color del área lo cerró `#551`, el táctil ya es 48 desde `#470` y el «Volver» ya estaba corregido);
> y (4) **`ConfirmInline` es un componente por el FOCO, no por las líneas** — su primera versión dejó
> fuera el disparador, las tres zonas volvieron a escribir el estado y los dos focos, y `PrivacyZone`
> se pasó de su techo, que es lo que lo delató; con el gesto entero dentro el bundle **bajó**.
>
> ⚠️ **Y una trampa de instrumento nueva**: la captura enseñaba el «Cancelar» de borrar la cuenta gris
> y casi ilegible, como deshabilitado — y era la **TRANSICIÓN de salida del `:hover`**. El botón nace
> justo donde estaba el que se pulsó, así que **apartar el ratón no basta: hay que esperar a que la
> transición termine**. Es la trampa de `#554` por la otra puerta, anotada en la sonda.
>
...
>
> ✅ **HECHA en `#566`** — lo de abajo es el encargo tal como estaba escrito, y se conserva porque la
> parte que NO envejeció es dónde mirar. ⚠️ Lo que sí envejeció: sus grietas 12 y 13 alcanzaban a más
> pantallas de las que decía (cinco y dos, no tres y una), y la 14 ya estaba cerrada.
> ▶ **LO SIGUIENTE es el CUADERNO DE ENTREGA del cajón**, que es lo único que `doc/spa.md` deja
> escrito tras la parada 06: «las 25 pantallas están dibujadas, lo que queda es empaquetarlas como se
> hizo con la portada». Y antes de eso, el **OJO del owner en un teléfono de verdad**, que es lo único
> que no puede hacer un agente.
> ▶ **Al 2026-09-13, tras `#567` y `#568`, queda ABIERTO y dicho** (nada de esto bloquea):
> **(a)** una **pasada de iconos**: el set del canvas (`Iconos PJP`) trae `ui/entrada` y `ui/regalo`
> redibujados —la entrada con ventana; el regalo pasa a gorro de fiesta— y los componentes del producto
> son la versión anterior; afectan a los iconos de los PRODUCTOS del catálogo. ⚠️ Y los banderines del
> set de hoy (`ui/fiestas`) dibujan la guirnalda a **2,4**, que `IconSetAnatomyTest` rechaza: por eso
> Grupos lleva los instalados. **(b)** El botón «Reservar» de un pack en `/servicios` abre ese producto
> (lo cubre `ServicesPageTest`), pero **no se vio en navegador**: en local ningún servicio tiene pack
> vinculado. **(c)** **El botón del SISTEMA** (16/800 con borde) sigue aplazado: toca la familia `.btn`
> entera y se acuerda antes con el carril de la web. **(d)** Del artboard `Navegacion Cuenta PJP` el owner
> **solo quiso el avatar y quitar «Cerrar sesión»**: la fila nueva, el tercer estado «visita hoy» y la
> tira de avisos **se descartaron** — no los construyas sin preguntarle. ⚠️ Las sondas de estas dos
> tandas viven en `storage/app/audit/sonda-567*.mjs` y `sonda-568.mjs` (gitignoradas); la de `#567`
> entra en «reserva creada» sembrando un pase de vuelta del banco para `R-UNPIRD`
> (`RedsysReturnController::handoff()`), sin tocar datos del dominio.
>
> ❗❗ **EL ENCARGO, TAL COMO ESTABA ESCRITO: LA PARADA 06** —el historial y las tres pantallas de
> invitado— **y con ella las
...
> Suite **4871 en verde** (31.112 aserciones, 1 skipped), medida el **2026-09-13** con `#571` fusionado sobre `#594`
> (**+13 aserciones** en `#594`: la guarda de Redsys del despliegue se prueba EJECUTANDO su condición con
> doce valores; **+6 casos** en `#592`: la sección de reseñas con la nota y el aviso cuando solo falta el
> permiso, y la política de cookies que nombra las reseñas; antes, **+5** con `#591`: las reseñas de Google en el idioma de la
> instalación y la caché que dura más que el hueco entre dos refrescos; antes, **+22** sobre `#568` con `#588` y `#589`: la edad del cumpleañero, los servicios con
> varios productos, los regalos, la escala de altura sin franjas pisadas y el horario «sábado y domingo»). Verificadores de concurrencia en
> verde sobre InnoDB: `redsys` · los DIEZ escenarios de `purchase:verify-oversell` · `postform` `addons` y `cross`.
>
> Antes de `#588`, **4828 en verde** (30.680 aserciones, 1 skipped), medida el **2026-09-13** con `#568` en el árbol
> (mismo número de casos: las guardas se re-apuntaron, no se añadieron).
>
> Antes de `#568`, **4838 en verde** (30.759 aserciones, 1 skipped), medida el **2026-09-13** de madrugada con
> `#567` en el árbol (**+5**: 3 de `WhoBlockLabelTest`, el mismo menor en dos líneas y la puerta de la
> reserva creada en `AccountDoorWiringTest`). JS **980** en verde.
>
> Antes de `#567`, **4833 en verde** (30.723 aserciones, 1 skipped), medida el **2026-09-12** de madrugada sobre
...
> de cifra— · `#565`: **+1** de `SidebarDrawerPolishTest`, que ata las tres cosas que se pierden al
> copiar la pregunta).
> ⚠️ **Ninguna de las dos cifras que traía cada carril valía** —4799 se midió sin la parada 05 y 4796
> sin la pasada de vestido—: la buena salió de MEDIR tras rebasar, que es la regla escrita.
> ▶ Antes, con el **ARMAZÓN del cajón COMPLETO** del carril del SPA
