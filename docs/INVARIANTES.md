...
| **`TicketIssuer` es idempotente:** no-op si la Order ya tiene tickets (cinturón del anterior). | `TicketIssuer` | `RedsysReturnHandlerTest::test_ticket_issuer_is_idempotent` + `::test_second_authorized_payment_on_already_paid_order_does_not_duplicate` |
...
| **La clave secreta de Redsys vive en `config/services.php` (`redsys.secret_key` = `env(...)`), NUNCA `env()` en código de runtime** — `env()` devuelve `null` tras `config:cache` y caería a la clave sandbox pública (apagón de cobros en despliegue). `Redsys::config()` lee `config(...)`. | `config/services.php` + `Redsys::config()` | `RedsysSecretKeyConfigTest::test_reads_secret_key_from_config_so_it_survives_config_cache` + smoke post-`config:cache` en deploy |
...
| **Rutas `/pago/redsys/*` con `throttle:120,1`.** | `routes/web.php` | ⚠️ Sin test dedicado (verificado 2026-08-12): nada asevera el throttle de `payments.redsys.*` — candidato a test nuevo |
...
| **«Hoy operativo» usa `DisplayTime::today()/now()`** (timezone de display), NUNCA `Carbon::today()` crudo en el subsistema de venta, y NUNCA cambiar `config('app.timezone')` global (la BD persiste en UTC). | `DisplayTime` | `OrderCreatorTest::test_rejects_a_today_slot_whose_start_time_already_passed` + `SlotOfferTest::test_past_times_today_are_not_offered` + `DisplayTimeTest`; ⚠️ sin caso que cruce la frontera de fecha UTC↔Madrid |
| **El pedido pendiente nace auto-liberable:** el hold se pasa a `createPendingOrder` (no en un `save` posterior) — un crash no deja retención de aforo perpetua. | `Purchase::confirmReservation` → `createPendingOrder` | `OrderCreatorTest::test_can_create_a_provisional_held_order` + `RedsysIdaTest::test_order_expires_at_is_set_to_hold_window_when_redirecting` |
...
| Invariante (NO deshacer) | Código | Verificación |
|---|---|---|
| **`SecurityHeaders` cubre TAMBIÉN el panel admin** (está en el stack del `AdminPanelProvider`, no solo en el grupo `web`) — sin él, la superficie que gestiona PII/reembolsos/roles queda sin cabeceras ni CSP. | `AdminPanelProvider` (middleware) + `bootstrap/app.php` | `SecurityHeadersTest::test_admin_panel_login_also_sends_the_base_security_headers` |
...
| **Los secretos NO son legibles ni editables desde el panel** (la página Settings los excluye). | Página Settings | `SettingsPageTest::test_secrets_and_counter_are_untouched_on_save`; ⚠️ la NO-legibilidad (que el form no cargue el secreto) no tiene assert |
...
| Invariante (NO deshacer) | Código | Verificación |
|---|---|---|
| **`Setting::value()` memoiza POR PETICIÓN** (`self::$memo`, invalidado en `saved`/`deleted`): las múltiples lecturas de un request (CSP, tema, flags) colapsan en 1 query. ⚠️ Acarrea el invariante de suite: `flushMemo()` en `TestCase::setUp()` (ver §6). | `app/Models/Setting.php` | Suite + `HomePageTest::test_anonymous_home_get_stays_within_query_budget` |
...
| **Los guards `Schema::hasTable` del arranque solo se consultan FUERA de producción** (−4 queries a `information_schema` por request en el hot path). | `AppServiceProvider::tableExists()` | ⚠️ Sin test dedicado (verificado 2026-08-12): nada asevera el gate no-producción — candidato a test nuevo |
| **Sin N+1 en el listado de servicios:** `isSellablePackForLanding()` reutiliza la relación `prices` ya eager-loaded (0 queries); solo cae al `EXISTS` puntual si no está cargada. | `TicketType::hasPositivePrice()` | Indirecta: presupuesto de queries de la home (`HomePageTest`); sin assert directo del «0 queries con `prices` eager» |
