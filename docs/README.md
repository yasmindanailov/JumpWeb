...
| `specs/waiver-probatorio.md` | 🟦 **Subsistema B de Fase 6 — CÓDIGO COMPLETO** (`DECISIONES #142`; ejecución `#160` · `#161` · `#163` · `#166`, spec **§9**; **guion `VERIFICACION-E2E-CAJON.md` §5.nonies recorrido en headless y subsistema entero revisado de forma adversarial —spec §9.10 y §10, `#169`—**; sigue 🟦 hasta el OJO del owner en navegador, el texto definitivo, la retención y las decisiones de §10.11), y el primero por dependencia. El waiver deja de ser solo externo y pasa a tener valor de prueba: versiones inmutables de documento, snapshot en el instante de la firma y PDF compuesto del snapshot, **nunca del CMS**. ⚠️ **Modifica `INVARIANTES` §3 (RGPD-01)**: el registro se conserva al borrar la cuenta, bajo tratamiento restringido y con plazo `[PENDIENTE: owner]`. §4.7 explica por qué eIDAS hoy no, y **qué hay que dejar puesto hoy para poder anclar mañana**. |
...
| `specs/menores-a-cargo.md` | 🟦 **Subsistema C de Fase 6**. Nombre y fecha de nacimiento; la edad se **deriva y nunca se persiste**; «quitar» es desvincular si hay firma o historial detrás. ⚠️ **§4.6 es lo que hay que leer antes de tocar el modelo**: Booking **no puede mirar a Identity**, así que la asignación la posee Identity y referencia el ítem por su id entero. §4.7 mide por qué la asignación necesita **dos puertas** (no hay sesión garantizada al elegir la cantidad). |
...
| `MODELO-DATOS.md` | Mapa de BD **regenerado desde el código** (32 modelos · 78 migraciones), por dominios, con rarezas heredadas. |
