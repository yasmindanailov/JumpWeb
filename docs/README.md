...
| `specs/menores-a-cargo.md` | 🟦 **Subsistema C de Fase 6 — TANDA 1 EN EL ÁRBOL** (`DECISIONES #191`, 2026-08-27: `dependents` + `DependentRegistry` + `GET|POST|DELETE /me/dependents` + `anonymize()`/export/purga + el tope en Ajustes; **sin firmas de menor todavía**). ▶ **Empieza por §9** y por **§9.5: lo que la tanda 2 necesita del owner, con número y coste** —la primera es NUC-3, la cadena de hashes con firmas de menor—. Nombre y fecha de nacimiento; la edad se **deriva y nunca se persiste**; «quitar» es desvincular si hay firma detrás. ⚠️ **§4.6 antes de tocar el modelo**: Booking **no puede mirar a Identity**, la asignación la posee Identity por id entero. §4.7: **dos puertas** para asignar (no hay sesión al elegir cantidad). |
...
| `MODELO-DATOS.md` | Mapa de BD **regenerado desde el código** (33 modelos · 80 migraciones), por dominios, con rarezas heredadas. |
