...
| `specs/waiver-probatorio.md` | 🟦 **Subsistema B de Fase 6 — CÓDIGO COMPLETO** (`DECISIONES #142`; ejecución `#160` · `#161` · `#163` · `#166`, spec **§9**; sigue 🟦 hasta el ✅ del owner en navegador —guion `VERIFICACION-E2E-CAJON.md` §5.sexies—, el texto definitivo y la retención), y el primero por dependencia. El waiver deja de ser solo externo y pasa a tener valor de prueba: versiones inmutables de documento, snapshot en el instante de la firma y PDF compuesto del snapshot, **nunca del CMS**. ⚠️ **Modifica `INVARIANTES` §3 (RGPD-01)**: el registro se conserva al borrar la cuenta, bajo tratamiento restringido y con plazo `[PENDIENTE: owner]`. §4.7 explica por qué eIDAS hoy no, y **qué hay que dejar puesto hoy para poder anclar mañana**. |
