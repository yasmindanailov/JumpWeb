...
| `specs/waiver-probatorio.md` | 🟦 **Subsistema B de Fase 6** (`DECISIONES #142`), y el primero por dependencia. El waiver deja de ser solo externo y pasa a tener valor de prueba: versiones inmutables de documento, snapshot en el instante de la firma y PDF compuesto del snapshot, **nunca del CMS**. ⚠️ **Modifica `INVARIANTES` §3 (RGPD-01)**: el registro se conserva al borrar la cuenta, bajo tratamiento restringido y con plazo `[PENDIENTE: owner]`. §4.7 explica por qué eIDAS hoy no, y **qué hay que dejar puesto hoy para poder anclar mañana**. |
...
| `MODELO-DATOS.md` | Mapa de BD **regenerado desde el código** (32 modelos · 76 migraciones), por dominios, con rarezas heredadas. |
