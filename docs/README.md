...
| `specs/menores-a-cargo.md` | 🟦 **Subsistema C de Fase 6 — TANDAS 1 y 2 EN EL ÁRBOL** (`#191` el núcleo + la API; **`#198` la FIRMA DEL MENOR**: cadena de hashes por (titular, sujeto) —`#197`, owner—, identidad del menor en la firma (v3), FK RESTRICT, `POST /me/dependents/{id}/waiver`, PDF y panel con el nombre, retención desde los 18). ▶ **Empieza por §9**: §9.1/§9.7 qué existe, §9.5 las cinco decisiones TOMADAS, §9.7.4 lo que queda — la **tanda 3 (el cajón, midiendo el chunk)** y la 4 (el embudo). Nombre y fecha de nacimiento; la edad se **deriva y nunca se persiste**; «quitar» es desvincular si hay firma detrás. ⚠️ **§4.6**: Booking **no puede mirar a Identity**. §4.7: **dos puertas** para asignar. |
