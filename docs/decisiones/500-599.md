...
⚠️ **D2 modificada por `#520` (2026-09-18)**: la lista completa ya NO rechaza un «sí» — era un oráculo
de pertenencia. El resto de D1–D17 sigue vigente.
...
## #575 · 2026-09-17 · T4·3 · el interruptor de la invitación, su guard en el modelo y el embudo
Spec `celebracion-e-invitacion.md` §4.8 y §10.4.3. Entra «Invitación digital» en el catálogo, con el guard
de `TicketType::saving()` como **autoridad**: exige pack, columna de nombre en los datos por invitado y
justificante distinto de `required`. Nacen `funnelGuardianMode()` y `offersGuardianInFunnel()`, que leen
el cajón (`CatalogReader`) y el mostrador: con invitación **el embudo deja de enseñar la casilla del
justificante**. Y `product_addons.show_in_invitation` (D12) atraviesa las **cuatro** puertas del pivote.
**Por qué**: la combinación imposible tiene que pararse donde escriben TODOS —seeders, importaciones, un
`update()` a mano—, no en un formulario; y quien contesta la invitación ya dice si el niño va con un
adulto, así que preguntárselo además al anfitrión sería pedir el dato a quien no lo sabe.
**Descartado**: deshabilitarla en el formulario de forma reactiva (dos sitios que divergen) · tocar
`OrderCreator`, que está en el `CRITICAL_RE`: esto es una OFERTA, no un permiso (`#400`), y un caso fija
que `offersGuardianAuthorization()` no cambia.
**Consecuencia**: `InvitationCatalogTest` (11) + `AddonStageTest` (23); arnés **11/11** con control. ⚠️ El
guard puso en rojo un caso de la T4·2 que creaba esa fila con `save()`: se reescribe forzándola por
debajo del modelo. ▶ Siguen T4·4 (plazas y firmador), T4·5 (RGPD) y T4·6 (API).

## #576 · 2026-09-18 · T4·4 · un «sí» es una plaza con dueño, y la firma atada que no la cobra dos veces
Spec `celebracion-e-invitacion.md` §4.5·7/·8, §4.5·11 y §10.4.4. Nace el contrato `Booking\PartyGuests`
(aplazado desde la T4·1 hasta tener consumidor); `GuardianPlaces` gana un **cuarto sumando** —los «sí»
vivos sin justificante atado— y el firmador una **excepción**: una firma atada a un «sí» no descuenta
plaza, y el vínculo se escribe solo si el contrato confirma que esa respuesta es de esta reserva. Con
ellos, **anular el enlace** desde la ficha, con permiso re-exigido al ejecutar, bloqueo auditado y el
token fuera del rastro.
**Por qué**: sin el sumando el anfitrión puede bajar los invitados por debajo de quien ya le confirmó;
sin la excepción **el padre que dijo «sí» no podría firmar** —su propio aviso le cerraría la puerta—. Y
el enlace se reparte a un grupo de clase entero: hay que poder cerrarlo antes de que caduque, y
`RGPD-06` no lo alcanza.
**Descartado**: que el contrato devuelva un RECUENTO —restar los firmados obligaría a Booking a mirar
Identity; la resta va donde las dos mitades coexisten, como en `#401`— · que el firmador se crea el id
que le llega · emparejar por nombre el justificante suelto, que `#328` descartó: **cuenta dos veces,
declarado**, y el suelo alto es el lado seguro.
**Consecuencia**: `InvitationPlacesTest` (9), `InvitationLinkRotationTest` (4), un caso nuevo en
`ModuleContractsTest` y arnés **12/12** con control — uno de esos mutantes solo lo caza el DOBLE. ▶
Siguen T4·5 (RGPD) y T4·6 (API).

## #577 · 2026-09-18 · T4·5 · la supresión se lleva la invitación, y deja en pie la prueba que no es suya
Spec `celebracion-e-invitacion.md` §4.4 y §10.4.5; `RGPD-01` y `RGPD-06` al día. `User::anonymize()`
borra `invitation_replies` y `party_invitations` de las reservas del titular, **por tabla y en dos
consultas**. El justificante **se conserva** y solo pierde el puntero; la purga de go-live se las lleva
por cascada; el export del art. 20 no las publica.
**Por qué**: `anonymize()` llevaba desde la Fase 1 vaciando `guest_data`, y la invitación volvía a
guardar esa misma clase de dato —nombre de un menor, lo que contestó el padre de otro niño, sus
alergias— en dos tablas que la supresión no miraba. **Nada fallaba**: el art. 17 se incumple con una
tabla nueva que nadie recordó. Por tabla porque `User` solo puede importar `Order`/`OrderItem`/`Ticket`;
en dos consultas porque el art. 17 no puede depender de una FK **que por esta vía nadie dispara**.
**Descartado**: borrar también el justificante —no es del responsable, art. 17.3.e— · meter el vínculo
en el hash de la firma: la supresión de un anfitrión dejaría ROTA la prueba de otra familia · un caso
que apagaba las FK para ejercer el cinturón: **medido, no las apagaba**
(`Schema::withoutForeignKeyConstraints()` es no-op dentro de una transacción), se retiró en vez de
dejar una guarda que no puede fallar.
**Consecuencia**: `InvitationPrivacyTest` (5), arnés **5/5** con control **y un superviviente
DECLARADO** — el arnés gana esa tercera clase: esconderlo bajando el denominador sería mentir en el
informe. ▶ Sigue T4·6 (API).

## #578 · 2026-09-18 · T4·6 · la API de la invitación, y la hoja en blanco como objeto aparte
Spec `celebracion-e-invitacion.md` §4.10 y §10.4.6. **Cierra la T4.** Dos superficies: PÚBLICA por
token (`GET`/`POST /invitations/{token}`) y del ANFITRIÓN por la misma puerta que su formulario
(`invitation` dentro de `GuestForm`, un `PUT` para personalizar, un `DELETE` para «no lo apuntes» y
`adopt[]` en el `PUT` existente). Con ellos, el dominio que faltaba: resumen, personalización, propuesta
por emparejado, adopción, descarte y reconciliación.
**Por qué**: el enlace se reparte a un grupo de clase entero, así que la tarjeta pública es una HOJA EN
BLANCO —ni una respuesta, ni un contador, ni el token— y los cuatro «no» son **el mismo 404**:
distinguirlos diría que ese token existió. Personalizar va aparte del formulario porque escribe solo la
invitación; dentro movería el testigo de la reserva y dejaría obsoleta la página abierta del anfitrión.
**Descartado**: que `InvitationCard` sea `Invitation` recortado con banderas —un campo nuevo allí se
filtraría aquí sin que nada lo notara— · `allOf: [$ref] + nullable`, que **no valida** (`#27`, tercera
vez: copia inline con guarda de divergencia) · clavar el path de la página pública «para ir teniendo
algo»: `url` es `null` y se rellena **solo** en cuanto la T5 declare su ruta, preguntando por su nombre.
**Consecuencia**: `InvitationApiTest` (20) y arnés **14/14** con control. ⚠️ Tres mutantes sobrevivieron
a la primera y los tres eran guardas que FALTABAN, no código malo. ▶ La T4 queda cerrada; sigue la T5.

## #579 · 2026-09-18 · la revisión adversarial de la T4, y los dos defectos que encontró
Spec §10.4.7. Con permiso del owner: 8 lentes + un refutador por hallazgo, 31 agentes. **17 sobreviven,
6 refutados, 8 sin refutar por el tope declarado.** Se arreglan dos defectos reales y dos guardas
frágiles; el resto queda anotado allí.
**Los dos defectos, y los dos eran míos**: (1) `adopt()` marcaba con la clave del PADRE y
`reconcileAdopted()` compara contra las FICHAS, así que **el camino de bandera se rompía solo** —pegas
«Hugo», el padre contesta «Hugo Ruiz», emparejan por la primera palabra y la adopción se descartaba en
el mismo `PUT`, dejando sin plaza al niño que confirmó—. (2) El atajo del firmador no exigía que el
«sí» estuviera **sin consumir**: el mismo id levantaba el tope N veces.
**Por qué así**: la clave de adopción pasa a ser la de la FICHA —todo lo que la lee compara contra
fichas— y se reparte con la MISMA función que calcula la propuesta. La segunda comprobación vive en
Identity: `PartyGuests` es de Booking y los justificantes son de Identity.
**Descartado**: comparar nombres entre el «sí» y quien firma (`#328`) · tocar la IP del padre en el
rastro, que **no la introduce esta tanda** (ya graduada en `waiver-por-reserva.md` §4.14).
**Consecuencia**: dos casos vistos ROJOS, arnés **16/16**, y dos guardas arregladas (una roja sola de
19:00 a 02:00; un 500 donde el contrato promete 422). ▶ El oráculo de pertenencia lo resuelve `#520`.
❗ `#579` agota la banda 550–579; el carril del SPA sigue en **520–549**.

## #520 · 2026-09-18 · `[DECIDIDO owner]` La lista completa deja de rechazar: se cierra el oráculo
