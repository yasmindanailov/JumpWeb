...
**T5b ✅ (24-09) — el panel.** `ExperimentResource` en «Ajustes → Sistema» (`settings.manage`; rastro
`experiments.saved`/`experiments.deleted`; **con el experimento VIVO la clave y las variantes van bloqueadas**: se
apaga y se crea otro) y `ExperimentsWidget` en «Analítica → Conversión» sobre `ExperimentsReport`: por variante, los
visitantes expuestos (una vez cada uno, de sesiones limpias), cuántos compraron DESPUÉS de la primera exposición y
la conversión con su **intervalo de Wilson al 95 %**; los visitantes con dos variantes son contaminados y no cuentan
en ninguna, y las cuentas (`user_id`) con dos variantes se enseñan. ⚠️ **La compra se ata al visitante por el SELLO
del pedido** (`attribution.visitor_id`, solo con «análisis» o «anuncios»; el `Recorder` ata `order_paid` solo con
«análisis»): la cifra es la conversión ENTRE QUIENES CONSINTIERON, igual en todas las variantes —la comparación
vale, el nivel absoluto no— y el widget lo dice en su nota. Los experimentos del driver solo ven a quien consintió
(sin flags del driver hoy: fuera de alcance).
**T5c ⬜ — una prueba real.** `[DECIDIDO owner]` 24-09 (`#738`): **sin mecanismo de textos desde el panel**; cada
experimento es una tanda de código y el owner nombra la hipótesis cuando haya datos reales («medir cosas finas que
de verdad ayuden a mejorar conversión»). La candidata natural sigue siendo la carcasa (isla contra cajón): `shell` por
...
- ✅ **Lo que enseñó la T4a (24-09: la 360)**: es una SECCIÓN de la columna derecha de la ficha (`UserInfolist`),
  no una pestaña —la ficha no tiene pestañas y una sola para esto la partiría—, y sigue el molde de los menores a
  cargo: un read-model de la capa de entrega (`Filament\Resources\Users\Support\CustomerInsights`, que cruza
  Booking, Payments, Identity y Platform como `Filament\Analytics\*`) que COMPONE cada celda, y un partial que
  solo pinta, con los números en `data-*` para afirmarse sin depender de una traducción. Dos regímenes a
  propósito: lo del CONTRATO sale siempre de los pedidos cobrados (`paid`/`refunded`, las mismas tres cifras
  que `MoneyReport`: vendido, cobrado, devuelto; primera y última compra; frecuencia en compras/año solo con dos
  o más; los cinco productos más comprados por unidades) y lo de la NAVEGACIÓN solo si la cuenta tiene sesiones
  atadas (primera fuente y campaña de `users.first_attribution`, visitas antes de la primera compra, contactos
  recibidos —`contact_received` con `user_id`—); sin sesiones, la ficha lo dice y no inventa. Una cuenta
  anonimizada no enseña nada (`RGPD-01`). Nada lee `dependents`. El permiso `customers.insights` es propio (junta
  la historia comercial con la navegación de UNA persona) y no va al staff por defecto: entra en `PermissionCatalog`
  y en el seeder a la vez (`PermissionCatalogTest` exige la paridad).
- ✅ **Lo que enseñó la T4b (24-09: los segmentos)**: un segmento es un ESTADO DE HOY, no una serie del periodo:
  `SegmentsReport` (capa de entrega) no lee el filtro del cuadro y memoriza sus cuatro recuentos cinco minutos;
  la lista de personas se calcula al exportar, que es un acto auditado. Las cuatro reglas viven en SQL sobre
  los PEDIDOS cobrados (`paid`/`refunded`) y el libro: «compró una vez y no volvió» = exactamente un pedido
  cobrado y hace más de 90 días (una temporada); «fiesta hace un año» = la ÚLTIMA compra de un pack
  (`ticket_types.type = pack`) entre 10 y 12 meses atrás —quien volvió a celebrar no entra, y nunca se mira
  ...
  recuento, sin PII. 🪤 Una acción de auditoría nueva va a `AuditLog::ACTIONS` o el controlador revienta con 500;
  la lista de widgets de `AnalyticsPageTest` es EXACTA (un widget nuevo entra ahí); el `no-store` de la ruta
  llega como `max-age=0, no-store, private`.
- ✅ **Lo que enseñó la T4c (24-09: el opt-in tras comprar)**: es el MOMENTO, no un mecanismo nuevo. La casilla de
  la pantalla de «reserva creada» es la MISMA pieza y los MISMOS rótulos de «Mi cuenta → Privacidad»
  (`account.privacy.marketing_label/hint`, que ya viajan con sesión: **ni una clave más en el montaje**, el
  presupuesto de textos no se mueve) y llama al MISMO `PUT /me/marketing` (`AccountPrivacy::setMarketing()`,
  único escritor). La regla de cuándo se ofrece vive en `account/marketing-offer.js` (`CE-6`, con sus `node
  --test`): con sesión, con el perfil diciendo `marketing_opt_in === false` y **sin retirada previa** (una fila de
  `consents` de tipo `marketing` con `revoked_at` es un «no» ya dicho: no se insiste); sin la lista de
  consentimientos cargada no se decide, y solo se pide (`GET /me/consents`) en el caso en que puede cambiar la
  respuesta. Quien la marca la ve marcada y puede desmarcarla ahí mismo. 🪤 La pantalla se abre en vivo con el
  PASE de la vuelta de la pasarela (`HomeController::maybeConsumeRedsysReturn`, una entrada de un solo uso en la
  caché `database` atada al titular): la sonda lo pone por tinker y lo gasta al visitar `/?redsys=<token>`. **Y
  la sonda cazó la casilla sin pintar**: `GET /me/consents` llega con SOBRE (`{ data: [...] }`, `ApiCollection`) y
  la regla esperaba una lista plana; se normaliza en el módulo (`consentRows()`), con su caso. 🪤 **Y la suite
  cazó la clase muerta**: el envoltorio `.purchase__optin` nació sin regla y la sonda lo dio por bueno (8/8: el
...
- ✅ **Lo que enseñó la T5b (24-09: el panel de los experimentos)**: la conversión por variante NO sale del libro
  para todo el mundo —el sello del pedido lleva el visitante solo con «análisis» o «anuncios», y el `Recorder` ata
  `order_paid` al visitante solo con «análisis»—, así que se mide ENTRE QUIENES CONSINTIERON y la nota del widget lo
  dice; como el consentimiento no depende de la variante, la comparación vale. Con veinte expuestos, un «10 %» a
  pelo engaña: Wilson al 95 % (2,8–30,1 %) es lo que de verdad se sabe, y dos intervalos que se solapan no se
  distinguen. 🪤 Un `Resource` nuevo cuenta en TRES censos: la tarjeta de «Ajustes» en `AdminNavigationTest` (22 →
  23), el `WIDGETS` de `AnalyticsPageTest` y la regla «la tabla plegada cierra la pestaña». 🪤 `session()` es de
  `TestCase`: un ayudante privado con ese nombre es un fatal de PHP antes de correr nada.
...
| T6 | **✅ (24-09) la fiesta** —la lista de invitados, el justificante y la invitación, SIN cookie ni script para el invitado; hechos de la RESERVA desde los controladores, el dinero de después de reservar desde el libro, la pestaña «Fiestas» y el segmento de los invitados que vuelven—: spec propia `specs/analitica-fiesta.md` (`#739`), tres tandas allí, las tres con el ✅ del owner en vivo | `#739` | los de su §6 |
| T7 | **⬜ encuestas** (§4.10, idea del owner del 24-09): internas en la puerta al escanear el QR (el empleado pregunta en persona y marca con el dedo) y externas por correo al día siguiente; se crean en el panel (interna/externa, activa, fecha límite…) y sus resultados salen en el cuadro. **Spec antes de tocar código** (`/spec`) | | los de su spec |
...
### 4.10 Encuestas — idea del owner (24-09), pendiente de spec

Lo que pidió, con sus palabras, «para que no se me olvide»:

- **Internas**: cuando el empleado escanea el QR del cliente (la puerta), le sale un pequeño banner; lo abre y le
  hace unas preguntas al cliente EN PERSONA; el empleado marca con el dedo las respuestas.
- **Externas**: al escanear el QR de un cliente, al día siguiente se le manda un correo con una encuesta, de manera
  amigable.
- Las encuestas se crean en el panel: interna o externa, activa o desactivada, con fecha límite o sin ella, etc.
  Y sus resultados salen también en «Analítica».

Lo que su spec tendrá que decidir (nada de esto está decidido): dónde viven las preguntas y las respuestas (data-driven,
desde el panel, sin negocio quemado); el disparo interno desde la pantalla de la puerta (`GateVisits`, rol `puerta`,
tablet: el banner no puede estorbar el escaneo); el externo sobre el molde de los correos, con su UTM y su
`email_clicked`, y si una encuesta es comunicación de servicio o comercial (el opt-in); el régimen RGPD de las
respuestas (anónimas por defecto; atadas a la visita, no a la navegación); una encuesta por visita como máximo y el
plazo; y su pestaña o widget en el cuadro (resultados por pregunta, por periodo, tasa de respuesta interna y externa).
