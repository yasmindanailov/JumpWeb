...
- **Las seis tandas (T0–T6) en el árbol** (`#288` el sello · `#289` · `#294` · `#296` · `#298` con cinco
  adendas · `#299`); queda el OJO del owner. **Punto de entrada: §18** (la visión del owner, `#284`) **y §21**
  (el sello); §20 es el −X €. `MixedPartySurcharge` y `AgeFamilySealer` están en el `CRITICAL_RE` →
  `VERIFY_CONC=1` y `mixed-party:verify-concurrency` (escenarios `charge` y `credit`).
- **La regla que ordena todo: se reconcilia cuando cambia el HECHO** (edades, cantidad, producto, fecha) **y
  NUNCA cuando cambia la CONFIGURACIÓN**. Cada fiesta guarda al nacer su familia, tramos y precios
  (`order_items.age_family_seal`, `PAY-19`): solo cambia de condiciones lo que cambia de PRODUCTO; la fecha no
  cambia los tramos; el re-sello va en la MISMA transacción. Tres «no aplica» que no son intercambiables.
- **El dinero solo se mueve al guardar el formulario COMPLETO, en las dos direcciones** (dos preguntas: para el
  dinero, todas las edades declaradas; para el formulario, ninguna edad sin producto). Una edad sin producto es
  un estado CONOCIDO: no congela.
- **Un `extra_due` suelto NO cobra, MUEVE dinero ya pagado** (§8.3, medido); la forma que cobra es una LÍNEA
  (§8.4). **El portador NO puede ser el pack de destino** (`AFORO-01`): es un complemento que crea la migración.
  El descuento es el ESPEJO (`is_credit`, `chargedSubtotalCents()` pone el signo) y desde el libro (`#305`)
  entra en el saldo sin tope. Cambiar el producto de la reserva cobra por TODOS los invitados (§8.2).
- **La conexión entre productos es familia + tramo de edad** y la verdad ÚNICA de solapes es
  `TicketType::overlappingAgeSibling()`; los eventos de Eloquent no ven `Query\Builder::update()`.
- **Palabras (§25)**: «Liquidado en el parque»; la supresión con reserva por celebrar se bloquea con 409
  (`upcomingItems()`, en `AccountPrivacy`, jamás en `anonymize()`); fechas RELATIVAS en tests. La etiqueta va
  pegada al nombre (`displayProductName()`). Anexo al final (59 avisos).
...
> Lo que decía la fila **«Cumpleaños MIXTO · edad de los invitados · la conexión KIDS↔JUMP · el suplemento · etiqueta «MIXTA» · el SELLO de condiciones · una edad sin producto · el dinero solo al guardar completo»** de `CLAUDE.md` cuando el enrutador bajó a una línea por fila
> (`DECISIONES #619`). Se conserva **verbatim** porque es historia de trampas medidas: léelo
> después del §0 y no lo reescribas. Documentos que la fila citaba: `docs/specs/cumple-mixto.md` · `docs/DEUDA.md`.

- **`docs/specs/cumple-mixto.md`**
- 🟦 ▶ ❗❗❗ **T6 · EL GUARDIÁN DE SOLAPES EN EL DOMINIO — EN EL ÁRBOL (`#299`, 2026-09-01): §26 diseño fino, §26.5 ejecución. CON ELLA, EL PLAN DE `#284` QUEDA SIN TANDAS PENDIENTES (T0–T6).** El hueco G: «los tramos no pueden solaparse» era verdad SOLO dentro del panel — ahora `TicketType::overlappingAgeSibling()` es la verdad ÚNICA del criterio (nulos como 0/255, el de `coversGuestAge`) y el `saving` del MODELO revienta el solape (`OverlappingAgeRangeException`, con el hermano dentro) y el invertido; el form del catálogo DELEGA y conserva su aviso amable.
- ⚠️ **Valida SOLO al tocar los TÉRMINOS del tramo**: una fila con un solape metido por la puerta de atrás sigue editable en precio o nombre (bloquearla dejaría el catálogo ingobernable), pero tocar SUS tramos exige sanearla.
- ⚠️⚠️ **El límite, dicho**: los eventos de Eloquent NO ven `Query\Builder::update()` ni SQL crudo — cinturones: el lector resuelve por el tramo de MENOR edad y el sellador copia la realidad sin frenar ventas (la config no frena caja); con el sello de la T1, un solape ya NO mueve dinero (la tanda cierra la frase «por construcción es imposible», no un agujero de caja).
- ⚠️⚠️ **La suite escondía DOS fixtures con solapes TRANSITORIOS** (el tri-familia de la T5-J nacía como `cumple` pisando a Kids; el de cobertura de la T4 creaba Teens 12–99 ANTES de encoger Jump): se LEGALIZARON, no se excepcionó el guardián — *un fixture que necesita un estado que el dominio prohíbe prueba un mundo que no existe*.
- ⚠️ Y un grep por CLAVE i18n dijo «el form no tiene tests»: mentira del instrumento — `CatalogGuestAgeFamilyTest` asevera por CONDUCTA (10 casos). —
- ▶ ❗❗❗ **T5 · LAS PALABRAS — EN EL ÁRBOL (`#298` + 5 adendas, 2026-08-31 noche → 09-01): §25 diseño fino, §25.10 ejecución, §25.9 las tres decisiones del owner.** Y las CINCO adendas del ojo del owner sobre datos reales: el «−» del pendiente en la card (guarda N) · «Cantidad: 4 → 2» en el historial (la forma que `#145` dejó fuera, guarda O) · la hoja OPERATIVA con los hechos de la mezcla y NI UN EURO (`[DECIDIDO owner]`, REVISA la decisión «a sabiendas» de la T3 §23.2 — la corrección va DENTRO de §23.2; guarda P) · el historial a UN CLIC desde el dinero (bloque y card, solo con consecuencias que explicar; guarda Q —
- ⚠️ `hasActivity()` NO vale de condición y
- ⚠️⚠️ un botón de Filament emite `mountAction` CUATRO veces: la aguja es `wire:click=`) + el **FANTASMA de la señal** a `DEUDA.md` (la reconstrucción del online original usa la señal VIVA del catálogo — divergencia pedido-vs-card en vivo, el hermano de la señal del caso espejo) · el historial a 10/página y el NACIMIENTO del pedido online FUERA a propósito (`[DECIDIDO owner]`, registrado para que nadie lo reabra).
- ⚠️⚠️ Trampa de Blade pagada: el extractor de bloques PHP casa desde el primer `@php(` con paréntesis hasta el primer CIERRE de bloque del fichero — media pantalla sin compilar y el error señalando el FINAL del fichero; en `items-list` solo la forma con paréntesis. **D9**: las TRES claves que decían «Pagado en el parque» dicen **«Liquidado en el parque»** (es/en/fr/zh_CN; identificadores y aritmética intactos;
- ⚠️ `paid_desk` NO se toca — es un cobro REGISTRADO; `TYPE_COLLECTED_IN_PERSON` sigue declarada y SIN USO: el registro real del cobro es feature aparte; guarda por PALABRAS PROHIBIDAS, no por texto exacto). **D8**: la supresión con reserva POR CELEBRAR se bloquea en las **TRES vías** (`[DECIDIDO owner]`, panel incluido — su escape es cancelar primero) con 409 `account_has_upcoming_reservations`;
- ⚠️⚠️ **el criterio es `upcomingItems()` (el de la pantalla del titular), NO el complemento de `terminated()`**: una línea pagada SIN franja jamás «termina» y bloquearía el art. 17 PARA SIEMPRE; la puerta va en `AccountPrivacy` tras `verify()` y JAMÁS en `User::anonymize()` (censo + fronteras); el cajón no cambió NI UNA LÍNEA; `RGPD-01` ampliada.
- ⚠️⚠️ **Tres tests de `MePrivacyTest` borraban una cuenta con reserva pagada FUTURA** (fecha `2026-09-05` clavada — desde el 05-09 habrían pasado SOLOS): fechas RELATIVAS, el reloj no decide. **Correos**: las sobre-promesas eran DOS — `reduction_pending_refund` reescrita (describe las DOS salidas sin prometer canal NI correo: el registro del manual es opcional; y su puntero «Mis reservas» había caducado — no enseña importes desde `#130`) y `OrderItemRefunded`/`OrderRefunded` ganan el MODO (`MODE_MANUAL` → «se te ha devuelto en el parque; este correo es tu justificante»; el reenvío lo deriva del último reembolso con éxito); `order_item_modified.refunded` RETIRADA con su parámetro (muerta desde el D8 del desglose, con promesa de tarjeta dentro). **Panel**:
- ⚠️⚠️ **una afirmación de §18.5 resultó FALSA al medirla** («el 0,00 € de la dirección barata se resuelve solo con la T4» — NO): filtro de PRESENTACIÓN `visibleUpgrades()` —
- ⚠️⚠️ **`upgrades` es pieza de CARGA**: `creditTargets()` necesita exactamente las entradas de la dirección barata (`unit 0`, `diff < 0`) y filtrar en el lector rompe el crédito de la T4; el MISMO precio conserva su línea («es mixta y no cuesta nada») — · lo ESCRITO primero y la línea del veredicto DESPUÉS con etiqueta (`conditions_line`, clave PROPIA: `line` la comparte la HOJA, donde todo es escrito) · la cadena de estados por LADOS independientes (el único tragón real era `missing_credit_carrier` delante de `drift`; `missing_carrier` antes que `drift` es CORRECTO: es la explicación de su propio desfase) · `frozen` en NEUTRO («Importe por edades congelado»: lo congelado puede ser un descuento). **Y el «+-4,00 €»** (OJO del owner en mitad de la tanda): el `+` CLAVADO de las líneas ↳ de los DOS partials del panel — escondido tras el «Ver más» plegado por defecto, justo donde la sonda de la T4 no miró — pasa a signo consciente.
- ⚠️ Trampas pagadas (§25.10): un `git checkout` para deshacer una mutación se llevó trabajo SIN commitear (la regla de `#181` en pequeño) · la guarda del portador ausente era IMPOSIBLE con el portador vivo (**el propio cargo de la pasada cuenta como cobertura**, §24.3: debe faltar ANTES del reconcile) · `Setting::value` memoiza la tabla ENTERA (borrar un setting sin `flushMemo()` deja la guarda ciega) · el doble anónimo de `ModuleContractsTest` implementa el contrato entero y un método nuevo lo CRASHEA (fatal en la declaración → «premature end» en paratest). Queda el OJO del owner; sigue la **T6**. —
- ▶ ❗❗❗ **T4 · EL −X € — EN EL ÁRBOL (`#296`, 2026-08-31 noche): §20 diseño de producto (del owner, `#285`), §24 diseño fino, §24.10 ejecución.** El descuento es el ESPEJO del cargo: línea `is_credit` —el signo lo pone `chargedSubtotalCents()`, ÚNICO sitio, porque `unit_price` es UNSIGNED— con su `extra_due` gemelo NEGATIVO, y **tope `min(derivado, cobertura de puerta)`** (los cargos de la misma pasada cuentan como cobertura; verificado con familia de TRES tramos cerrando la puerta en cero exacto). El EXCESO no se escribe: es el «a tu favor», en TODAS las superficies incluidos «Mis pedidos» y la API (`in_favour_hint`, patrón `L6`: null ES la condición de enseñarlo; `[DECIDIDO owner]` con el coste delante).
- ⚠️⚠️ **La asimetría del silencio es distinta a la del cargo A PROPÓSITO**: el cargo crece en silencio (`#268`); el crédito NO se mueve en ninguna dirección sin veredicto que gobierne.
- ⚠️⚠️ **El `context` del ajuste del crédito JAMÁS lleva `changes.*`** — con un `quantity_change` dentro, `itemOriginalOnlineCents` reconstruye un original y el eje de caja inventa un «pendiente de devolución» (la trampa MEDIDA de §16.5.bis, hoy aseverada por guarda).
- ⚠️ El desglose ↳ emite la línea NEGATIVA con su frase (la identidad D del guardián la exige — el `continue` que las saltaba se vio en ROJO antes de tocarse, como §24.1 predijo).
- ⚠️ Las frases de `#246` («NO se descuenta solo») CADUCARON en post-form, ficha, hoja, puerta y correo.
- ⚠️ Portador PROPIO «Descuento fiesta mixta» (migración 91).
- ⚠️ `mixed-party:verify-concurrency` tiene DOS escenarios (`charge`·`credit`), y el del crédito se vio FALLAR sin el lock (12 líneas y −84,00 €). Queda el OJO del owner; sigue la T5. —
- ▶ ❗❗❗ **T3 · EL PARQUE DECIDE — EN EL ÁRBOL (`#294`, 2026-08-31 noche): §23 diseño, §23.10 ejecución.** E lo escrito del suplemento en hoja de sala y tarjeta de puerta (nunca el veredicto; del veredicto solo el caso barato y las edades sin producto —
- ⚠️⚠️ la guarda de presupuesto cazó un **N+1 preexistente de la puerta** con cualquier `extra_due`: eager anidados en el reader, techo de `GateProfileTest` 25 → 28 a cambio de lotes constantes) · F la pestaña «Invitados» por la MISMA puerta que el cliente (`OrderItemGuestDataWriter` → `submitGuestForm(guests, null, 'panel', $by)`: `via: panel`, actor operador, correo con la voz del parque; el writer es control negativo del gate —
...
- ▶ ❗❗❗ **T2 · UNA EDAD SIN PRODUCTO + EL DINERO SOLO AL GUARDAR COMPLETO (`#289`, 2026-08-31): §22 diseño, §22.9 ejecución.** «Completo» son DOS preguntas: para el DINERO, todas las edades declaradas (`GuestAgeMix::allAgesDeclared()`); para el FORMULARIO, además ninguna edad sin producto (`OrderItem::guestAgesWithoutProduct()`).
- ⚠️⚠️ **La puerta vive en `MixedPartySurcharge::reconcile()` y vale también para el panel** (`[DECIDIDO owner]`): con una edad en blanco no se escribe NADA en ninguna dirección — hasta el 31 crecía con parciales (`#268`).
- ⚠️ Una edad SIN PRODUCTO no congela: no genera línea y las demás se tarifican.
- ⚠️ Los tres textos (`below`/`above`/`gap`) viven en Ajustes → Web → «Fiestas por edad», con `:phone` y cadena de respaldo pedido → app → es → en → fr.
- ⚠️ **Hueco de contrato preexistente cazado**: `GuestFormResource` serializaba `general` vacío como `[]` (el contrato dice objeto).
- ▶ ❗❗❗ **T1 · EL SELLO ESTÁ EN EL ÁRBOL (`#288`, 2026-08-31): §21 es el diseño fino y §21.13 lo ejecutado.** Cada fiesta guarda al nacer en `order_items.age_family_seal` la familia, los tramos y los precios con los que se vendió; el veredicto deriva del sello y **ningún cambio de catálogo mueve una reserva vendida** (`PAY-19` reescrita).
- ⚠️⚠️ **Solo cambia de condiciones lo que cambia de producto, y la FECHA no cambia los tramos** (`[DECIDIDO owner]`, Q2 de §21.8): pack nuevo → sello nuevo; día nuevo → el MISMO sello re-preciado.
- ⚠️ El re-sello va en la MISMA transacción que la mutación (`OrderItemEditor::sealUpdateFor`), nunca en el post-commit.
- ⚠️ Tres «no aplica» que no son intercambiables: sin sello y sello caducado CALLAN; sello sin familia AFIRMA (y retira el suplemento).
- ⚠️ **Dos huecos preexistentes cazados al construirla**: un día sin tarifa CANCELABA el suplemento (ahora abstención, «no poder tarificar es una ausencia») y una fiesta mixta con cargo **no podía cambiar de pack** desde el panel (`orphan_addons` contaba la línea gobernada).
- ⚠️ `#283` (el aviso al mover tramos) **se retiró**: con el sello no tenía nada que avisar; queda la frase en la ayuda del campo.
- ⚠️ `nextRequest()`/`forgetScopedInstances()` **ya no hacen falta**: el lector es sin estado.
- ▶ ❗❗❗ **EMPIEZA POR §18 Y NADA MÁS: LA VISIÓN DEL OWNER, CERRADA** (`#284`, 2026-08-31) — nueve decisiones, siete huecos MEDIDOS y el plan por seis tandas. **La regla que lo ordena todo**: «el cliente compra con unas condiciones y las mantenemos; ya las siguientes reservas empiezan con las nuevas» — solo cambia de condiciones **lo que cambia de producto**.
- ❗❗ **El SELLO vive en la RESERVA, no en el producto**: cada una guarda al nacer los tramos y precios de su familia, así que **el precio viejo solo existe dentro de las reservas que lo llevan** y **no hace falta histórico de precios**.
- ❗❗ **Y hay un hueco MAYOR que el caso espejo**: el **PRIMER** cargo usa el catálogo del día en que el cliente rellena el formulario, no el del día de la compra (medido: 5,00 € pactados → **10,00 €** cobrados). No hace falta tocar tramos, basta subir un precio.
- ⚠️⚠️ **`PAY-18` NO choca con la visión, es la misma regla** («la fecha es un producto»).
- ⚠️ **Una edad sin producto es un estado CONOCIDO, no una incógnita**, y eso CORRIGE a `#268`: hoy congela el dinero (medido: un bebé de 0 años impide que el cargo baje).
- ✅ **El −X € YA ESTÁ DISEÑADO Y CERRADO: el diseño vigente es §20** (`#285`) — el ESPEJO del suplemento acotado al dinero de puerta (el clamp nunca muerde, trazado), con el exceso como línea «a tu favor» liquidada EN el parque, y **la regla del owner: el dinero solo se mueve al guardar el formulario COMPLETO, en las dos direcciones** (
- ⚠️ cambia `#268` para los cargos; el disparador va en la T2). §16 queda como pieza de la FASE 3 (todo online), que exige una feature que hoy no existe para ninguna dirección: el cobro online post-reserva.
- ⚠️ `0 LIVE · 0 PRODUCCIÓN`: no hay nada que rellenar al desplegar el sello. —
- ▶ **§17: LA REVISIÓN ADVERSARIAL** (`#249` · `#268`→`#272`) — el subsistema aterrizó en una jornada sin revisar y tenía **SEIS defectos que son UNO**: *el importe no se guardaba, se recalculaba entero desde el catálogo en cada disparo, y sin datos con que calcular escribía CERO en vez de dejarlo quieto*.
- ❗❗ **El peor lo dispara el CLIENTE**: vaciar sus casillas de edad **borraba su propio cargo** (mentir con la edad obliga a inventarse un número creíble; borrarla no). Con él, `RGPD-01` al anonimizar y cualquier tramo tocado en el catálogo.
- ⚠️⚠️ **La guarda que existía para el defecto del precio pasaba en VERDE con el defecto puesto**: aseveraba el importe sin volver a guardar, y el defecto vivía en el guardado siguiente — *una guarda que no ejercita el disparador vigila el reposo, no la regla*.
- ▶ Arreglado en cinco tandas: **una ausencia no es una corrección** (con el veredicto incompleto lo escrito CRECE pero nunca encoge) · el operador ve el cargo huérfano · **el RECIBO** (`booked_type_id` + `priced_on` en el `context`, sin migración) con el `[DECIDIDO owner]` de los **14,00 € y no 24,00 €** · **NO se construye el perdón** `[DECIDIDO owner]` —su argumento, de `#244`: «cualquier gestión de dinero post-reserva ya cobrada se hace en las instalaciones»— y el panel deja de ofrecer un gesto que deshacía **en el mismo clic** con dos correos contradictorios · la pieza entra en el **`CRITICAL_RE`** y la regla en **`PAY-19`**.
- ⚠️ **Mover el día o cambiar el pack SÍ re-tarifican** (`PAY-18`): el recibo no congela de más, y hay control para las dos cosas.
- ❗ **Lo que NO cierra (fichas en `DEUDA.md`)**: el **caso ESPEJO** —una configuración que CREA un cargo de la nada; medido, reordenar tramos convirtió una fiesta sin cargo en **40,00 €**— y su gemelo, que la etiqueta MIXTA puede **contradecir** al cargo de forma permanente. **Se cambió un fallo de dinero por uno de coherencia.** Cerrarlos exige sellar el régimen EN LA RESERVA (columna + migración), y es del owner.
- ⚠️ **Y las guardas de aquí nacen CIEGAS sin `nextRequest()`**: `GuestAgeMixReader` va en `scoped` y memoiza precios, así que un caso que cambia el catálogo y vuelve a guardar leería los valores de antes. —
- ▶ **TANDAS 1 y 2** (`#243` · `#244`, `[DECIDIDO owner]`) — **§12**: el suplemento **se COBRA y se recalcula SOLO**, sin aprobación del operador; eso CORRIGE los puntos 5 y 6 de §9.
- ⚠️⚠️ **La regla que lo ordena todo: se reconcilia cuando cambia el HECHO** (edades, cantidad, producto, fecha de la reserva) **y NUNCA cuando cambia la CONFIGURACIÓN** (precios, tramos) — lo escrito es lo que se le comunicó al cliente, y el desfase se ENSEÑA.
- ❗❗ **§12.3 SI TOCAS EL SUPLEMENTO: el portador NO puede ser el pack de destino** — medido, `PackAvailability` cuenta toda fila de producto `pack` en esa zona/día **sin mirar si es una línea HIJA**, así que consumiría una fiesta del cupo y sus plazas en silencio (`AFORO-01`). Es un COMPLEMENTO (sin zona, `seats=0`, `slot_id=null`) que **crea la migración**, con guarda ROJA si falta.
- ⚠️⚠️ **§8.3 ANTES DE TOCAR DINERO AQUÍ: un `extra_due` suelto NO cobra, MUEVE dinero ya pagado** (medido sobre un pedido real: valor +0, pagado online **−6,00 €**, puerta +6,00). `PAY-16`/`PAY-17` cierran **porque** cada `extra_due` del editor va con una subida de `unit_price`; la forma que sí cobra es una **LÍNEA** (§8.4).
- ❗ **La conexión entre productos es FAMILIA + TRAMO DE EDAD** (`guest_age_family` + `guest_age_min`/`max`, **extremos incluidos**): antes del 2026-08-29 el sistema **no tenía ninguna** —lo preguntó el owner y era cierto—.
- ⚠️ **§8.2: cambiar el producto de la reserva NO es el encargo** — cobra la diferencia por TODOS los invitados (30,00 € donde se piden 3,00 €).
- ⚠️ **§12.6: lo que NO se cierra** — el cliente puede bajar la edad la víspera; `[owner]`: «eso es trabajo en persona». Se cierra con RASTRO (`audit_logs`) y con la hoja de sala, que ya imprime la edad.
- ⚠️ **§11.2: una guarda nació CIEGA** (`assertFalse(mixed)` lo cumplen dos mundos) y un test de render cazó un **500 en el post-form de todo cliente**.
- ⚠️ Fuera de alcance `[owner]`: el **AFORO** (en el parque real son zonas distintas y el niño mayor pasa a JUMP), el resto de superficies y la API ·
- ▶ **§13: la etiqueta va PEGADA AL NOMBRE** (`#245`, idea del owner) — un solo compositor, `OrderItem::displayProductName()`, y todas las superficies de texto plano la cogen de ahí (hoja de sala, resumen del día, puerta, calendario, correos, «Mis pedidos», API).
- ⚠️ **El sitio único es la RESERVA, no el producto**: `TicketType` lo comparten todas las fiestas.
- ⚠️ La ficha del panel usa el nombre CRUDO porque pinta la pastilla aparte — el dato suelto (`isMixedParty()`) existe para quien pueda darle estilo o FILTRAR.
- ⚠️⚠️ **El precio de que todos lo cojan de ahí: `isMixedParty()` necesita `ticketType` y `slot` cargadas**, o son dos consultas por fila. Lo fija `MixedPartyLabelSurfacesTest` comparando el nº de consultas con 2 y con 6 reservas.
- ▶ ❗ **§14: el caso BARATO** (`#246`, encontrado por el owner en un pedido real) — si al invitado le toca un pack MÁS BARATO no se descuenta nada (suelo en 0), pero **se AVISA**: al cliente y al operador, con la aritmética («2 invitados corresponden a Kids, 11,00 € por invitado, en vez de Jump, 15,00 €»).
- ⚠️⚠️ **Esa cifra NO va por el desglose de dinero**: «Pendiente de devolución» es deuda REAL y cuadra con `PAY-16`/`PAY-17`; un informativo ahí lo descuadra y el cliente lee una deuda que no existe.
- ⚠️ **El aviso del post-form salía SOLO cuando había dinero**, así que una fiesta mixta sin cargo enseñaba la etiqueta y ninguna explicación. Ahora sale siempre que sea mixta.
- ⚠️ No se descuenta solo porque crearía un **incentivo para mentir a la baja** que hoy no existe.
- ▶ **§15: el régimen en el recuadro de CADA niño** (`#247`) — la pastilla sale del MISMO `walk()` que el veredicto (una copia haría que una ficha dijera «Kids» y el total otra cosa) y refleja lo GUARDADO, no lo tecleado (`CE-4`).
- ⚠️ **Dice el PACK y no la zona** aunque el encargo dijera «zona»: medido, en la demo los dos packs comparten `zone_id = 4` y el nombre de la zona no distinguiría nada.
- ⚠️ **El desglose de puerta es UNO** (`Order::pendingAtGateLines()`) y lo leen cliente (ledger), operador (partial `reservation-financials`) y hoja de sala — su frase decía «1 invitad**os**» y no nombraba el pack; corregido con `trans_choice` y con el nombre GUARDADO en el `context`.
- ⏸️ **§16: el DESCUENTO del caso barato está APARCADO** (`#248`, `[DECIDIDO owner]`) — se queda el AVISO a operador y cliente, y **no se tocan las invariantes del dinero**; se retoma en producción según las circunstancias.
- ▶ **Su diseño está escrito y VERIFICADO**: por qué no vale ninguna forma obvia (el total no existe como número · `order_items.unit_price` es **UNSIGNED** · un crédito de puerta suelto sube «pagado online»), la aritmética de los tres casos y la cota que demuestra que ningún canal queda negativo.
- ⚠️⚠️ **Y un supuesto propio que se midió y era FALSO** (§16.5.bis): el respaldo de `itemOriginalOnlineCents()` devuelve lo cobrado AHORA, así que un pack SIN SEÑAL habría perdido el descuento en silencio. Si algún día se implementa, **empieza por ahí**.
