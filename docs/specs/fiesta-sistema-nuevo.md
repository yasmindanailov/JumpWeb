...
- **FALTA**: el foco al primer campo desde el QR de la puerta, el QR de la fiesta en la puerta (lógica nueva,
  `identidad-qr-puerta.md`). ✅ **En F6a (26-09)**: la firma dentro del recibo, con la misma fuente que esta página.
...
**La F2 (26-09)**:
- **Lo vivo, en una rama APARTE**: el modo `vivo` de la tarjeta es un `@elseif` propio y no marcas repartidas por la de
  siempre; así la tarjeta pública y las piezas del banco no cambian ni un byte, y el juez lo confirma (18 de 18).
- ❗ **Un estilo en línea le gana al atributo `hidden`**: la chapa, la figura y el pie llevan `display` en línea (como el
  JSX), y `hidden` solo no los esconde; `[data-inv-vivo] [hidden] { display: none !important }` lo hace valer, también
  sin JavaScript. El `hidden` se escribe con un cierre de PHP (`$oculto()`), no con `@if` pegado a una palabra (§4.7).
- **La casilla del sistema pone sus atributos en el `label`**, no en el `input` (el campo sí): la vista previa lee la
  casilla por su `id`.
- **Lo que vio la sonda y no es de F2**: la cuenta local 436 guarda un NOMBRE en el teléfono, y el producto lo deja
  (`AccountProfile::rules`: `required|string|max:30`); la invitación pintaría «Llamar» a un no-número. Es la validación
  de la cuenta (`telefono-del-cliente.md`), no la fiesta: al owner.

**La F6a (26-09)**:
- ❗❗ **Lo que el navegador vio y la suite no**: cinco de los seis defectos de la fila de F6a salieron de la sonda y del
  banco, no de las pruebas. El 429 (el limitador), el formulario vacío tras un reenvío (dato repetido de la propia
  sonda), el foco en `body`, la cabecera que desborda y el color del botón. Cada uno lleva ahora su prueba o su par.
- ❗ **Laravel cuenta en UNA clave por IP todas las rutas con `throttle:N,M` sin prefijo** (`domain|ip`): el límite de
  una ruta lo gastan las demás. El tercer argumento (`throttle:10,1,guardian-sign`) le da cubo propio; la API ya lo hacía.
- ❗ **Volver con un ancla le quita el foco a la página**: al terminar de cargar el navegador corre los pasos de enfoque
  del ancla y, si el destino no es enfocable, deja el foco en el documento, DESPUÉS de los scripts. `firma()` enfoca
  tras `load`. (Y un `autofocus` tampoco vale: con ancla, el navegador lo ignora.)
- **En el arnés, el bolso de errores de un POST llega vacío a la petición siguiente** (la trampa de `AutorizacionPaginaTest`):
  el error del recibo se prueba con la forma real en la sesión y se midió con `curl` y cookies.
- **Un reenvío es idempotente y devuelve la firma que ya existía**, atada quizá a OTRA respuesta del mismo niño: el
  recibo, que pregunta por la atadura, no la ve. El desenlace viaja por el flash, atado a la respuesta de ESE recibo.
- **El banco de la página vio lo que el de piezas no** otra vez: `.inv a` (0,1,1) le ganaba a la clase del botón y en el
  diseño gana el estilo en línea; y un estilo en línea también le gana a la hoja de diagnóstico (`!important`).
