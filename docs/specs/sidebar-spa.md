...
| **4.1** ✅ | **HECHO 2026-08-14** (`DECISIONES #43`): dependencias (Vue 3 + Pinia), entry con `import()` diferido **y su techo de kB** (`SidebarBundleBudgetTest`, que el repo no tenía), montaje tras el flag, cliente HTTP, payload i18n, máquina de estados con `node --test` en el gate. ⚠️ **`SidebarDomContractTest` NO entra aquí y no es un olvido**: compara el árbol de los dos motores paso a paso, y con 4.1 «sin negocio» el motor SPA solo emite el andamio — no hay marcado que comparar. Nace con el primer paso transcrito (**4.2**), que es también cuando empieza a poder romperse | Sin negocio |
...
| **4.3** | Paso 4: cesta y presupuesto. ⚠️ **Partido en TRES al implementarlo** (`DECISIONES #47`), como se partió 4.2: **·1 el armazón y los cimientos de texto** ✅ (2026-08-14) · **·2 el pie y la cesta en memoria** ✅ (2026-08-14) · **·3 el aviso de reservas en pausa** ✅ (2026-08-14) · **·4 persistir la cesta** ✅ (2026-08-14) | El tramo ·1 no transcribe ningún paso: cierra lo que 4.2 dejó abierto sin que el gate pudiera verlo — el motor SPA no emitía **nada** del armazón (velo, banda, zona scrollable), la banda de progreso estaba escrita y verde pero recibía `progress: null`, y `#sidecart-spa` partía la cadena flex del panel |
| **4.4a** | Identificación de un usuario **ya autenticado**. ⚠️ **Partido en DOS al implementarlo** (`DECISIONES #51`), como 4.2 y 4.3: **·1 el CTA pregunta quién eres y si puedes reservar** ✅ (2026-08-14) · **·2 la pantalla de identificación** ✅ (2026-08-14, `DECISIONES #52`) | El corte es por DEPENDENCIA: de las cinco salidas de `checkout()` solo dos tienen pantalla hoy —el aviso de admisión y el cartel de pausa se pintan en el paso 4—; las otras tres llevan a los pasos 5 y 8, que son 4.4b y 4.5. El tramo ·1 transcribe la DECISIÓN sin navegar: navegar a un paso sin transcribir deja el cajón en blanco, que es peor que un CTA mudo |
| **4.4b** | Registro embebido + restauración de cesta. ⚠️ **Partido en DOS** (`DECISIONES #53`): **·1 el formulario de alta y el paso 7** ✅ (2026-08-14) · **·2 el widget de Turnstile** ✅ (2026-08-20, `DECISIONES #108`): el cajón monta el suyo con `turnstile.js` (módulo plano, `CE-6`) y la delegación en el modal de Livewire quedó retirada. ⚠️ El contenedor va con `v-if` y **PELADO** —sin clase ni id—: el Blade lo envuelve en `@if ($turnstileEnabled)` y la suite nunca siembra las claves, así que un `<div>` incondicional o con clase pone en rojo el diff de árbol Y el manifiesto congelado | ⚠️ **El paso más peligroso**, y no el de pagar: es el único que cruza la frontera Livewire↔Vue en los dos sentidos (`account-context` es Livewire; el modal de auth también lo era hasta `#122`), y no tiene guardián en servidor |
| **4.5** | Pasos 8–9: confirmar y auto-POST. ⚠️ **Partido en DOS** (`DECISIONES #54`): **·1 la cesta restaurada pide lo que le falta** ✅ (2026-08-14) · **·2 los pasos 8 y 9** ✅ (2026-08-14, `DECISIONES #55`) | El tramo ·1 es precondición y va aparte porque es autocontenido: sin él, una cesta de pack restaurada llega al pago y el servidor la rechaza con un 422 que el cliente no puede arreglar desde ninguna pantalla. Transcripción recta sobre invariantes ya guardados |
| **4.6** ✅ | Pasos 6, 10, 11 **y la vuelta de Redsys entera** (§4.1, costura 3). ⚠️ **Partido en DOS**: **·1 el paso 6, la reserva creada** ✅ (2026-08-14, `DECISIONES #56`) · **·2 los pasos 10 y 11** ✅ (2026-08-14, `DECISIONES #57`), denegado con su reintento y verificando con su sondeo | El corte fue por DEPENDENCIA, como todos los anteriores: el sondeo del paso 11 aterriza en el paso 6, así que el 6 fue primero. ⚠️ **Con 4.6 la transcripción está completa, pero el flag NO se despliega todavía**: falta el extremo a extremo con la pasarela en sandbox (§6) y el widget de Turnstile (4.4b·2) |
| **4.7** | Retirada: `Purchase.php`, el puente `$wire.step`↔store, el flag; congelar el manifiesto de DOM. ⚠️ **Partido en TRES** (`DECISIONES #60`): **·1 el manifiesto congelado** ✅ (2026-08-14) · **·2 el componente y el puente** · **·3 el flag** | La paridad se cierra **al final de cada paso**, no toda aquí. ⚠️ Y el orden del corte no es cosmético: **toda la red compara contra Livewire**, así que la foto del árbol hay que tomarla ANTES de retirarlo — después, un diff «los dos motores emiten lo mismo» pasaría en verde para siempre. Medido: **26 ficheros de test** ejecutan el componente |
