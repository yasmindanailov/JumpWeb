...
- **Los SEIS mecanismos del tema están en `main`** (`#192`→`#196`, `#209`, `#222`): color y superficie
  (`[data-surface]`, que PINTA) · forma (`ShapeScaleTest`: ningún canto en literal) · el hero · la ELEVACIÓN
  (§13: TRES roles —`lift` · `float` · `modal`—, no una escala) · el relleno de ACCIÓN (§15: `theme.action` en
  el panel; vacío = sigue a la superficie; `onBrand()` no vale para un botón) · el MOVIMIENTO (§16: 4 curvas y
  7 duraciones, cero literales; se revisa interactuando). ⚠️ Las cifras del sistema cambiaron con el canvas
  (`rediseno-desde-canvas.md` §3): columna 1120, táctil 48, radios, aire, curvas.
- **Dos secciones CADUCADAS y la corrección va antes que el texto**: §1.2 (papel continuo; el contraste lo dan
  las TARJETAS, `S-00`) y §7 (la tanda 2 se partió; la 2c es `armazon-y-menu.md`).
- **La columna se escribe UNA vez en `--col-max`** (§18, `ColumnIsDeclaredOnceTest`); `--wrap-gutter` solo para
  lo que va a sangre completa: un contenedor acotado usa `--col-gutter` (§17, medido a 1920 y 2560).
- **El logotipo se sirve EN LÍNEA** (`InlineSvg`, lista blanca) y de ahí cuatro trampas: una animación sobre
  `<defs>` NO pinta (`#266`) · los `px` de un `transform` en un SVG son unidades del `viewBox` · `fill: both`
  mata el `:hover` (`#265`) · sus defectos estaban en la EXPORTACIÓN (`#275`). **No lleva sombra** (`#273`).
- **La portada tiene dos puntos estáticos y un imán** (§20); el rumbo es el movimiento NETO; `--cierre-q`
  (crudo) y `--cierre-p` (suavizado) no se intercambian (§19).
- **Iconos**: rejilla 24 y `currentColor` (`IconSetAnatomyTest`); nada de otra librería; los de parque son del
  cliente y entran por el kit. **Área táctil**: `[data-tap]` y el «Lote 9»; una sonda que mide CAJAS acusa a
  controles que cumplen. Un token en `em` NO es una longitud (§25.8.1); «encendido» es un ESTADO (`--ok`).
  **Anexo al final** (127 avisos): léelo antes de tocar una pieza.
...
> Lo que decía la fila **«Capa de TEMA · fondo oscuro por sección · el 2.º gris · `--sheet` · radios · sombras · el foco · la tira · las fuentes por instalación · el color del botón de COMPRAR»** de `CLAUDE.md` cuando el enrutador bajó a una línea por fila
> (`DECISIONES #619`). Se conserva **verbatim** porque es historia de trampas medidas: léelo
> después del §0 y no lo reescribas. Documentos que la fila citaba: `docs/specs/tema-por-instalacion.md` · `docs/VERIFICACION-E2E-CAJON.md` · `docs/specs/landing-white-label.md` · `docs/INSTALACION-CLIENTE.md`.

- **`docs/specs/tema-por-instalacion.md`**
- 🟦 **LOS SEIS MECANISMOS EN `main`** (`#192`→`#196`, `#209` y **`#222`**) —
- ❗❗❗ **`#280` SI VAS A ACOTAR UN BUCLE O A TOCAR EL INTERRUPTOR** (§31): ya **no es un bucle** —salta `--switch-cycles` veces (3) y **descansa ENCENDIDO**, y vuelve a saltar cuando el hero regresa al viewport—.
- ⚠️⚠️ **El reposo de una pieza no se escribe dentro del bloque de `prefers-reduced-motion`**: ahí vivía el estado ON entero, así que acotar iteraciones la dejaba **apagada con la palabra «ON» al 100 % encima** (el rótulo no tenía `opacity` propia). *Si el reposo solo existe dentro de una excepción de accesibilidad, fuera no existe — y no lo ve nadie, porque quien revisa con movimiento reducido la ve perfecta.*
- ⚠️⚠️ **Un número ENTERO de iteraciones acaba en el fotograma APAGADO y `forwards` tampoco vale** (fija ese mismo fotograma): el corte se **trunca dentro del tramo encendido** —`calc(var(--switch-cycles) - 1 + 0.6)`, con 38–82 % en pista y bulbo pero **40–80 % en el rótulo**—, donde el valor es constante y coincide con el reposo: **0 px de salto**, medido.
- ⚠️⚠️ **El rearranque NO puede ser Web Animations API**: una animación terminada con `fill: none` **desaparece de `getAnimations()`** (medido, 1 → 0) y no hay a quién pedirle `play()` — se descarta y se recrea (`ui/hero-switch.js`), y la **lectura de disposición en medio no es superstición**: sin ella el navegador agrupa las dos escrituras y concluye que nada cambió.
- ⚠️ **Hay que haber SALIDO del viewport para volver a entrar**: con umbral 0, un temblor de un píxel devolvería el tic.
- ⚠️ Presupuesto de `#279`: `/` **5 → 2** (ya cumple), `/entradas` **6 → 4**, y **el interruptor eran 3 bucles, no 2** — *contar animaciones en un instante subestima una pieza cuyo ciclo apaga una de sus partes*. —
- ❗❗❗ **`#262` SI TOCAS EL INTERRUPTOR DEL TITULAR O VAS A TRAER UNA PIEZA ANIMADA DEL CANVAS** (§25): el interruptor **ya no es dibujo propio, es el `6d` del LOTE 6** — y eso **CADUCA el aviso de `#260`** que ocupaba este sitio (aquel relieve con sombras `inset` describía una cápsula que ya no existe).
- ⚠️⚠️ **«Idéntico» se CONSTRUYÓ, no se copió**: sus **diez** medidas (44 · 26,4 · 3,6 · 2 · 15,2 · 16,4 · 19 · −2,4 · 5,6 · 8,5) son múltiplos exactos de **1/16**, así que escritas en `em` sobre **una sola unidad** salen 1:1 *y* escalan con el titular — medido en navegador, el bulbo va a **19,00**, asienta en **16,40** y sale a **−2,40**, y el rótulo a **5,60 / 8,50**.
- ⚠️⚠️ **PERO UN TOKEN EN `em` NO ES UNA LONGITUD** (§25.8.1): una custom property se sustituye como TEXTO y el `em` lo resuelve **el elemento que la usa**, así que un descendiente que se cambia el `font-size` la ve valer otra cosa — el sangrado del rótulo salió a **1,84 en vez de 5,6** y **no falla nada, solo queda mal puesto**; no lo ve ninguna guarda de tokens ni ninguna captura. Si un descendiente cambia su cuerpo, se mide en `em` PROPIOS.
- ⚠️ El rótulo va **DENTRO** de la pista, con bucle propio (entra al 40 %, tras llenarse la pista; sale al 88 %, antes de vaciarse), y **el nombre accesible se sirve aparte en `.sr-only`**: un rótulo que parpadea cada 3,4 s no es un nombre accesible.
- ⚠️ **La familia se HEREDA** (`--font-display`), no se escribe — y ahí el cliente se contradice: su norma dice «Bungee nunca por debajo de 20 px» y su artboard lo pone a 8,5.
- ⚠️⚠️ **La CURVA no es la suya y la decisión está MEDIDA**: la del artboard y `--ease-entra` se separan **0,50 px a la talla del hero**, y estrenar una quinta curva por medio píxel deshace la tanda 2d — **además el rebote de `6d` no vive en la curva, vive en los FOTOGRAMAS**, y ésos sí están al dígito.
- ⚠️ **La duración SÍ es suya** (3,4 s) y entra como AMBIENTAL (`--dur-switch`) junto a `--dur-invite`: el techo de la escala son 620 ms y esto **no responde a un gesto**.
- ⚠️ **Los colores son ROLES, no sus literales**: su pista es Lima Bote, la nuestra `--ok` (`#254` + `ActionFillTest`: encendido es un ESTADO) — Lima Bote **sí tiene token** (`--strip-2`) pero su rol es la tira del pie.
- ⚠️ **Sin movimiento se congela en ON, no en OFF**: quieto al lado de un rótulo que dice «ON» sería un defecto **que no ve nadie**.
- ⚠️ **La copia local del canvas SIGUE sin el LOTE 6** (§25.1): `DesignSync` sin autorización, `/design-consent` da **403**, y el fichero llegó pegado **con la codificación rota** — por eso no se sobrescribió. —
- ❗ **`#260` SIGUE VIGENTE EN SU OTRA MITAD**: **el icono del CTA sale de `--cta-pair-h`**, no de un literal — con `:width` en el Blade hay dos fuentes **y gana la del marcado**. —
- ❗❗❗ **`#259` SI TOCAS EL CARGADOR O EL MARCADOR DE UN PRODUCTO**: el spinner es **«Tres botes»** y **TRES piezas, no dos** —los pseudo-elementos son los extremos y **el del medio lo pinta el fondo del elemento**, porque hacen falta tres FASES—; reposo con movimiento reducido **al 40 %**.
- ⚠️ En un `radial-gradient` las paradas se miden sobre el RAYO y por defecto llega a la ESQUINA: es `closest-side`, o el punto sale **29 % más pequeño**.
- ⚠️⚠️ **La clave del marcador viaja en el contrato** (`CatalogProduct.icon`, `openapi/v1.yaml`): **NO se deduce de `is_pack`** — el catálogo lo hacía y la guarda no lo veía porque miraba **una lista de dos ficheros escrita a mano**.
- ⚠️ Defectos: entrada → `ticket`, pack → `gift`; las seis ilustraciones siguen ofrecidas.
- ⚠️⚠️ **`.icon` dibuja a LÍNEA** (`fill: none; stroke-width: 1.6` sobre cada `path`): un glifo de masa no puede llevarla.
- ⚠️ **1:1 verificado: 47 de 47** idénticos byte a byte.
- ▶ ✅ **EL ÁREA TÁCTIL 44 YA ESTÁ** (`#264`, §26): **37 → 1** control por debajo en las siete vistas —el que queda es el enlace **en línea** del texto de cookies, que **WCAG exime**—.
- ⚠️⚠️ **Si vas a ampliar un área táctil, el mecanismo son DOS**: `[data-tap]` (pseudo centrado, **solo con puntero grueso**, no mueve el dibujo) y el bloque del **«Lote 9»** para cuatro controles del cajón, que corre en todos los punteros a propósito; el token `--tap-min` es de los dos y hay guarda de unicidad.
- ⚠️⚠️ **El del Lote 9 ENCOGÍA** desde entonces —`width: var(--tap-min)` a secas pone el área en 44 aunque el control mida 60, **sin que nada falle**—: los dos usan ya `max(100%, …)`.
- ❗ **Dentro de un carril con scroll el área invisible NO SIRVE** (un eje no visible obliga al otro a `auto` y el pseudo se recorta en silencio): por eso las dos tiras del pie crecen de verdad y **el bloque legal pasó a TIRA que se desliza** (`[DECIDIDO owner]`, el patrón de `#252`; el pie **encoge 30 px** en teléfono y queda **idéntico** en escritorio).
- ⚠️⚠️ **Y el alto de esas tiras necesita la puerta de puntero**: sin ella el pie crecía **20 px en escritorio**, donde manda el mockup.
- ⚠️⚠️ **Si mides áreas en navegador, `VERIFICACION-E2E-CAJON.md` §5.duovicies ANTES de creerte un número**: la sonda salió mal **dos veces** con cifras plausibles —recorte en coordenadas de viewport (áreas **negativas**) y el rectángulo del pseudo **sin su `transform`** (58 donde son 44)—, y **dos controles en capas distintas se solapan siempre**.
- ⚠️ **115 · 51 · 37 son tres cifras del mismo defecto**: la que vale es la del instrumento que ha demostrado estar sano. —
- ❗❗❗ **`#258` SI TOCAS UN ICONO O UNA PANTALLA DE DESENLACE** (§23): la auditoría del set está EJECUTADA —**61 componentes**, `DRAWER_OWN` **VACÍA**— y los cuatro ESTADOS ya son **PEGATINA** (círculo, keyline de tinta, sombra dura; relleno por tokens semánticos y sombra por rol, así que salen con la paleta del cliente sin una línea suya).
- ⚠️⚠️ **`SidebarIconParityTest` llevaba CIEGA a tres dibujos**: limpiaba los comentarios de HTML pero no los de **JavaScript**, y un `<svg>` citado en un docblock descarrilaba el escáner —que se tragaba el icono siguiente **sin fallar**—. Si tocas ese recorrido: **todo icono declara `viewBox` y una cita no**, que es lo único que distingue el caso (contar dibujos NO sirve: con el escáner roto salen igual).
- ⚠️ **El confeti sale de la confirmación** (la pegatina no convive con otra); `celebrate()` sigue.
- ⚠️ **`ProductIcon::CHOICES` pasa de 6 a 11 AÑADIENDO**: retirar una clave degradaría en silencio todo producto que la tenga guardada.
- ⚠️ Tres glifos los dibujamos nosotros (`chevron-down`, `eye`, `eye-off`) porque el artboard no los tiene —**no usa ni un chevron**— y eso no contradice a `#211`: aquello era un LOGOTIPO.
- ⚠️ **El PANEL queda fuera a propósito** (13 `<svg>` + los Heroicons de Filament): es otro idioma y es herramienta de operador. —
- ❗❗❗ **`#257` SI AÑADES UN ICONO** (§22): el set del artboard está EN EL PRODUCTO —**26 componentes → 55**— con su anatomía hecha ejecutable (`IconSetAnatomyTest`: `currentColor` sin excepciones · rejilla **24** · nada de línea fina **donde el trazo dibuja la forma**).
- ⚠️⚠️ **No añadas un icono de otra librería**: iría en otra rejilla y con trazo fino, y eso no falla — solo se nota mirando la web entera a la vez.
- ⚠️ **La geometría se COPIA con un guion desde el artboard**, no se transcribe; los dos que él no dibuja (`arrow-left`, `login`) se derivan **por espejo con `transform`**.
- ⚠️ **Un nombre de icono es una HIPÓTESIS sobre su dibujo**: mirarlos cazó que `ui/taquilla` es un CANDADO y `pag/pedido` una BOLSA.
- ⚠️⚠️ **El cajón comparte el set y cambia de aspecto con la landing** — es el mecanismo (`landing-white-label.md` §4.5.3), no un daño colateral: `SidebarIconParityTest` cazó las **14 copias** que se quedaron viejas.
- ⚠️ **Los 19 de parque (5 «Del parque» + 14 zonas) NO están**: son del cliente y **no existe mecanismo** para sustituir un dibujo por instalación (`DEUDA`).
- ⚠️ **`width` sin `(?<![-\w])` casa dentro de `stroke-width`** — le pasó al extractor y a `SidebarDrawerPolishTest`, que se puso ROJO con el producto sano. —
- ❗❗❗ **`#256` SI TOCAS EL CIERRE O EL MINIJUEGO** (§21): la demo del juego corre **ya en el punto estático**, como el mockup —su bucle va en modo demo siempre que el lienzo se ve, y `q > 0,985` solo decide si se puede JUGAR—.
- ⚠️⚠️ **El ANCLAJE no vale como señal aunque se llame igual que el punto estático**: medido, ahí la tarjeta está anclada a 1366, 1280 y 390 px pero **no a 1440 ni a 1920**. La señal es **que el lienzo SE VEA**.
- ⚠️ **Los dos umbrales no se tocan**: la vista enciende la ANIMACIÓN, `cierre:abierto` la hace JUGABLE — si la vista cambiara la fase, en el punto estático **el espacio dejaría de desplazar la página**.
- ⚠️ La reserva de la tira (`--salta-hueco`) vuelve a ser **constante** y eso NO contradice a `#253`: su criterio no cambia, cambia el hecho.
- ⚠️⚠️ Y destapó que **el motor cacheaba el ancho del lienzo** (62 % de estiramiento a 1920): `ResizeObserver`, no `clientWidth` por fotograma. —
- ❗❗❗ **`#275` SI TOCAS EL LOGOTIPO: LOS TRES DEFECTOS ESTABAN EN LA EXPORTACIÓN, NO EN NUESTRO CSS** (§27), y por eso cuatro tandas no los encontraron.
- ▶ **`text-shadow` NO arrastra el `-webkit-text-stroke`** —su sombra son copias del glifo DESNUDO— y el export le puso a la extrusión el trazo de la capa de color: el faldón azul medía **7,1 px contra sus 4,0**, y lo tenían **1121 de 1121** columnas contra 852 de 1137 en el suyo.
- ▶ **El velo del borde inferior** salía **α 0,377 contra 0,112** y frío bajo las dos palabras: `background-clip: text` mide sobre la **caja de línea** y `objectBoundingBox` sobre la **tinta**, y el lockup usa **un velo por palabra** (azul sobre letras cálidas **desatura**: es el «pierde color vivo»).
- ▶ **La silueta venía RESTADA de las letras**: a la «A» le faltaba el **16,8 %** del área, invisible en reposo y a la vista en cuanto `brand-hop` mueve la figura. Reconstruida desde Lilita One con la afín recuperada del propio trazado (control: **0,001 %**; la L, validación independiente, cae con **0,52 %** de área).
- ⚠️⚠️ **La misma geometría vive en DOS sitios** —`#u1` y el `<path>` del relleno—: arreglar solo uno la deja **BLANCA** y no falla nada.
- ⚠️ **`logo-contorno.php` SE RETIRA** y con él el 65 % de `#274`: las bandas del export ya eran las suyas al dígito (cian 1,00 · tinta 1,50 · blanco 0,875) y aquel factor **creó** un anillo marino de 1,14 px por fuera del cian.
- ▶ Guiones `scripts/logo-sombra.php` y `logo-letra-a.php`, **idempotentes**; la letra reconstruida es del CLIENTE (`client-logo-a.path`, tres piezas del patrón de marca).
- ⚠️ **El BRILLO superior se midió y NO diverge**: no se toca.
- ⚠️⚠️ **Siete trampas de instrumento**, todas con números creíbles — entre ellas el **subrayado por defecto del `<a>`** de su lockup (inflaba su caja de 60,75 a 65,5) y que **dos SVG en un documento comparten `id`**, así que el arreglo salía idéntico al original. —
- ❗❗ **`#274` queda CORREGIDO por `#275`**: el contorno y el alto del hero eran **un número nuestro puesto encima de uno suyo**, y el diagnóstico del hero sigue bueno; el del contorno no.
- ⚠️⚠️ **La aritmética decía que NO había defecto** —las bandas visibles calculan idénticas (0,90 · 1,45 · 0,90 px) y el stroke base sale **6,49 contra 6,50**—: tenía razón, y lo que faltaba era medir el MECANISMO, no el dibujo otra vez.
- ⚠️ **`-webkit-text-stroke` son px ABSOLUTOS: NO escalan con el `font-size`** — escalar su lockup para compararlo fue una de las cuatro trampas de esta medición.
- ▶ **El HERO de móvil** llevaba `--hero-h-end: min(72vh, **520px**)`, un segundo tope que **no sale del mockup** (el suyo es UNA fórmula, `Math.min(vh * 0.78, 660)`, para todas las ventanas): mordía por encima de **722 px** de ventana y dejaba **252 px vacíos a 390×844 y 340 a 390×932**. Retirado → 114 y 200, con escritorio intacto.
- ⚠️⚠️ Y era **la única de las tres expresiones de ventana del hero sin el par `vh` → `svh`** — justo la que fija el punto estático. Guarda con 2 mutaciones. —
- ❗❗❗ **`#273` SI TOCAS LA SOMBRA DEL LOGOTIPO: NO LLEVA NINGUNA** (`[DECIDIDO owner]`, elegido mirando una tira de cinco niveles, y hay guarda). El SVG **ya trae su relieve horneado** —**26 pasos** de extrusión por palabra, frente a los **6** del `text-shadow` del mockup—, así que cualquier `drop-shadow` encima se le suma; su lockup sí lo necesita porque su extrusión es fina.
- ⚠️⚠️ **Costó CUATRO vueltas del owner**, y la trampa está en `#267`: **midió bien y concluyó mal** —densidad total y halo daban 4-5 % de diferencia, pero *una métrica agregada puede decir «equivalente» sobre dos cosas que el ojo separa al instante*—. Eso **devuelve la razón a `#253`**, que diagnosticó bien y corrigió a medias.
- ❗ **La lección de método**: *cuando alguien dice varias veces que algo se ve mal y cada corrección falla, lo que falta no es precisión, es la pregunta* — enseñar opciones y dejar elegir.
- ⚠️ Y **descartar lo trivial primero**: staging servía el filtro viejo.
- ⚠️ Si un cliente quiere sombra, la pone su `client.css`. —
- ❗❗ **`#267`**:
- ⚠️ corregida por `#273`, pero su método sigue valiendo — son **los números del mockup** (`0 6px 16px` al **45 %** + `0 1px 0` al **25 %**) y **no se ajustan a ojo** — hay guarda. Costó **tres vueltas del owner**: `#253` bajó la tinta razonando que «copiar un filtro no es copiar un resultado si el sujeto es otro» y `#263` corrigió el radio comparando **cuatro variantes NUESTRAS entre sí**; el original no entró en ninguna de las dos.
- ▶ Medido con su lockup delante: **su lockup 1.193.218 · nuestro SVG con SU filtro 1.252.968 · lo que teníamos 645.997** — o sea que el mismo filtro sobre nuestro sujeto SÍ da su sombra (5 %) y **la nuestra era la mitad**.
- ⚠️⚠️ *Cuando el owner dice tres veces que algo se ve distinto, lo que falta no es otro ajuste: es la comparación que nadie ha hecho.*
- ⚠️ **Se copian los NÚMEROS, no el color**: lee `--paper-fg` o dentro del menú de tinta la sombra se vuelve luz.
- ⚠️ La geometría del asset está a **2-3 %** de su mockup: el dibujo no era el problema. —
- ❗❗❗ **`#266` SI TOCAS CUALQUIER ANIMACIÓN DE UN SVG EN LÍNEA**: **una animación CSS sobre un elemento de `<defs>` NO PINTA NADA** —lo que se dibuja son los `<use>` que lo referencian, y el clon no recibe la animación (el atributo `style` en línea sí)—. El salto del logotipo llevaba **tres tandas sin verse** (`#254` lo puso, `#263` lo «arregló», `#265` le dio la física) y las tres midieron lo mismo: que la animación **existe** y que su valor **computa**.
...
- ⚠️⚠️ **Y dentro de un SVG los `px` de un `transform` NO son píxeles: son unidades del `viewBox`** (factor 0,1249 en este logotipo), así que la amplitud estaba **7,5× corta** — va en **% con `transform-box: fill-box`**, que se mide contra la figura y es **más white-label que el mockup**.
- ⚠️ El **«0,000 px» de `#265` era ADIMENSIONAL** (el comparador normalizaba la escala fuera): validaba la forma y era ciego al tamaño; estaba en seis sitios.
- ⚠️ Guarda en `VERIFICACION-E2E-CAJON.md` **§5.quatervicies** (con control); §5.tervicies queda corregida. —
- ❗❗❗ **`#265` SI TOCAS EL SALTO DEL LOGOTIPO O EL CTA FLOTANTE DE MÓVIL**: el movimiento puede vivir en los **FOTOGRAMAS** o en la **CURVA**, y hay que saber cuál antes de tocar ninguno — `brand-hop` tenía los ocho fotogramas del mockup al dígito y **una sola curva con overshoot aplicada a todos** (`--ease-cae`, 1.56), o sea rebotando **dentro de cada tramo**. Ahora declara **siete, una por tramo**, que es la GRAVEDAD (sube desacelerando, cae acelerando) y no el estilo; `MotionScaleTest` las admite **solo dentro de un `@keyframes` declarado como COREOGRAFÍA**, con su caso de que la lista no apunte al vacío.
- ⚠️ Entran también el **asentamiento** del lockup (se hunde 2 px al recibir al saltador) y el **tempo** (`v = 0.9`: sus números de diseño son `--dur-cae` y `--dur-espera`, pero su código los divide). **Medido contra su fórmula: 0,000 px en 21 muestras.**
- ⚠️⚠️ **`fill: both` en una animación de `transform` MATA cualquier `:hover` que use `transform`** —deja el último fotograma fijado y una animación gana a la cascada—, y no falla nada; **`matrix(1, 0, 0, 1, 0, 0)` no es «no hay transform», es la identidad**, así que una sonda que pregunte «¿tiene transform?» sale verde con el hover muerto.
- ❗ **El RELEVO DE LA Y está BLOQUEADO por una exportación del owner**: el SVG dibuja «PLA» y la Y la hace la silueta, así que hay **475 ms** de logotipo incompleto al cargar — qué tiene que traer el fichero está en `INSTALACION-CLIENTE.md` §4.a.sexies, y eso **CORRIGE a `#263`**, que lo declaró imposible.
- ⚠️ **El asset se comprobó ANTES de buscar el defecto**: `client-logo.svg` es **byte a byte** el del owner.
- ⚠️⚠️ **Y el CTA flotante de móvil**: 48 → **56** con token propio (`--book-bar-h`, porque arriba el racimo baja a 48 al compartir fila con el logotipo), y su sombra era el **ROL EQUIVOCADO** —`--shadow-float`, que con este paquete vale `5px 5px 0`, dura y desplazada— cuando el cuarto rol de elevación **es la familia `--shadow-nav-*`**: el arreglo fue **RETIRARLA**, porque pisaba la que el componente ya traía. —
- ❗❗❗ **`#263` SI TOCAS EL LOGOTIPO**: su salto estaba **MUERTO en ONCE de las doce vistas** — lo disparaba `body.nav--live` y **esa clase la pone el componente del HERO, que solo existe en la portada** (medido en `/servicios`: `#fig` en el árbol y `getAnimations()` a CERO). El marcador bueno es `body[data-has-hero]`, y van **dos ramas**.
- ⚠️⚠️ **Ninguna guarda lo vio con una mirando al lado**: se comprobaba que `id="fig"` llega al documento, y eso pasa en verde con la animación muerta — *que la pieza llegue no es que se mueva*.
- ⚠️ **El ASSET está bien**: 27 pasos de extrusión por palabra, 16 en la silueta y los dos degradados; es la receta del mockup en contornos, **no hay que rehacerlo**.
- ⚠️⚠️ **La sombra: el RADIO hace la SUAVIDAD y la OPACIDAD la DENSIDAD** — `#253` bajó el radio 16→7 para quitar peso y lo que consiguió fue un borde DURO; queda **5 · 18 · 18 %**, comparadas cuatro combinaciones en navegador.
- ⚠️ El relevo completo del mockup (la silueta se queda como Y y la Y se desvanece) **no se puede**: en nuestro logotipo la silueta YA ES la Y. —
- ❗❗❗ **`#254` SI TOCAS LA MARCA**: el logotipo **se sirve EN LÍNEA** para poder animarlo, y eso cambia el modelo de amenaza — **dentro de un `<img>` un SVG es inerte y en línea NO**. Pasa por `InlineSvg`, **lista blanca y todo o nada**: con un `<script>`, un `on*=`, un `javascript:`, un `<foreignObject>` o una referencia externa **no se sirve** (la plantilla cae a su suelo de texto). Cuesta **~64 KB de marcado** en las doce vistas.
- ⚠️ Y al incrustarlo aparecen **dos nombres accesibles anidados**: el dibujo va `aria-hidden` y el nombre lo pone el envoltorio.
- ⚠️ **La tarjeta del cierre ocupa el hueco que le deja el pie** (`min-height` contra `--foot-h`), y **el armazón se retira en cuanto se ancla**.
- ⚠️ **El hero tiene UN solo CTA** —el par del armazón, debajo del titular—: tres piezas de compra en una pantalla eran dos de más.
- ⚠️⚠️ **El interruptor «ON» usa `--ok` y no `--action`, y lo dijo `ActionFillTest`**: acción es el control que hace AVANZAR, y «encendido» es un ESTADO — el chip de «Abierto ahora» ya lo había resuelto así. —
- ❗❗❗ **`#253` SI TOCAS LA PORTADA**: la tarjeta del cierre estaba en `z-index: 210` y **abría el cajón DETRÁS del juego, con el scroll ya bloqueado** — la escala por rol la fija `LayerOrderTest` y esa tarjeta es PÁGINA (88, y **no 90 para no empatar con `.book-bar`**).
- ⚠️ **El punto estático del cierre no cabía en la ventana** (921 px necesarios, solo por encima de ~1500): se ganó espacio quitando el selector de idioma del pie y dejando de reservar en reposo una tira de juego **que en reposo está vacía**.
- ⚠️ **El armazón nunca se retiró al bajar en la portada**: la lógica era correcta y **la regla del hero pisaba a la de `landing.css`** — la dirección es ahora un TERCER FACTOR del mismo producto, no una regla que pelea.
- ⚠️⚠️ **`.lang-dd--up` llevaba cinco días sin existir** y el prop `:up` no hacía nada: guarda nueva y general —**ningún modificador que un componente emite puede quedarse sin regla**— que **nació ciega TRES veces** (mutación incompleta · el nombre vivo en un comentario · el modificador es la CLAVE del array).
- ⚠️ Y `justify-content: center` en un contenedor con scroll **corta por arriba y no se alcanza**: es `safe center`. —
- ❗❗❗ **§20 SI TOCAS EL SCROLL DE LA PORTADA** (`#252`, `[DECIDIDO owner]`): la portada tiene **DOS PUNTOS ESTÁTICOS** —el hero encogido con el racimo colocado y la tarjeta del cierre en reposo con el pie entero— y un **IMÁN** que encaja en ellos (`ui/scroll-magnet.js`, mitad pura con 19 casos de `node --test`). **No es el `freno` del mockup**: el suyo bajando por el cierre te lleva a pantalla completa y aquí te RETIENE hasta que insistes.
- ⚠️⚠️ **Si tocas cualquier detector de dirección de scroll**: el rumbo **NO se puede sacar del último evento**. La portada **crece 34 px al llegar al final** —55 imágenes, 51 perezosas y **ninguna declara proporción** (`DEUDA`)— y el anclaje del navegador compensa: eso llega como un evento hacia abajo. **Pasó los 16 casos unitarios y falló en el navegador.** Es el movimiento **NETO desde la última parada**.
- ⚠️ **El hueco que el hero deja al racimo se CALCULA** (`--nav-pad-block + max(--nav-logo-h, --nav-btn-h) + --hero-top-air`): iba en `vh`, y como el racimo no cambia con la altura de ventana, **a 900 px el logotipo se metía 5 px dentro del hero**. Ahora, 12 px de aire en once ventanas.
- ⚠️ De paso apareció un **token FUERA DE ALCANCE** —la hamburguesa leía `--cta-pair-h`, declarado dentro de `.cta-pair`, y **nunca bajaba a 48 en teléfono**—, y **la guarda escrita para cazarlo nació CIEGA** por resolver la cadena de `var()`: **cuarta vez en tres días**.
- ⚠️ El pie va a **UNA fila que se DESLIZA** (el número de destinos lo manda la instalación) y la **marquesina de palabras está retirada entera** (siguen la de `/servicios` y la de la galería). —
- ❗❗ **§19 SI TOCAS LA COREOGRAFÍA DEL CIERRE** (`#251`): **publica DOS progresos y no son intercambiables** — `--cierre-q` (CRUDO) manda la retirada del armazón, el umbral de «ya llena» y el arranque del minijuego; `--cierre-p` (SUAVIZADO) manda la geometría, que es lo único que el mockup mete en sus `lerp`.
- ⚠️ Usar el suavizado para la retirada fue un fallo REAL y MUDO: al 7 % del crecimiento el armazón del mockup valía **0,19 de opacidad y el nuestro 0,82**, y no lo veía ni la suite ni una captura —solo la comparación **fotograma a fotograma** con su fórmula—. La retirada además es **CÚBICA**. Guarda: `CierreChoreographyTest`, 6 mutaciones.
- ⚠️ **Y una guarda propia se puso ROJA con el producto sano** por aseverar el NOMBRE de un token: se re-apuntó a resolver la cadena de `var()`. *Aseverar el texto literal ata la guarda a una implementación.* —
- ❗❗ **EMPIEZA POR §18 SI VAS A ESCRIBIR CSS** (`#250`, `[DECIDIDO owner]`): **la columna del sitio es la del mockup — 1176 px, no 1380 —, y su ancho se escribe UNA VEZ en `--col-max`**. De ahí salen las TRES formas en que el sitio expresa la misma columna: `width` (`.wrap`) · SANGRADO (`--wrap-gutter`, para lo que va a sangre completa) · caja EXTERIOR (`--hero-w-end`, `.reserve`, `.menu__inner`). **No escribas un ancho de columna a mano**: por encima de 1000 px ya no es una medida tipográfica y `ColumnIsDeclaredOnceTest` te lo dirá (6 mutaciones).
- ⚠️ **No se ensanchó: se ESTRECHÓ** — y había prueba interna de que la del mockup era la buena: el hero ya acababa en 1240 (su número) y las secciones iban a 1380, **dos columnas que nadie decidió**.
- ⚠️⚠️ **Dos lecciones de INSTRUMENTO que ahorraron dos cambios equivocados**: un comparador que mide con `getBoundingClientRect()` un elemento **girado** devuelve la envolvente del giro y miente; y un barrido de roturas **sin pasada de CONTROL** inventa roturas (marcó 31 «fuera de ventana»… las mismas 31 que antes del cambio).
- ❗ El reposo del hero del cierre está medido contra el artboard: **27 de 28 idénticas**; la única real es el canto de sus CTA (10 vs 14) y es la QUINTA contradicción del cliente consigo mismo (`DEUDA`). — §17 (`#238`): **`--wrap-gutter` es el sangrado de un elemento A SANGRE COMPLETA y solo sirve ahí**. Su `100%` es un porcentaje: **mide el CONTENEDOR, no el elemento**, así que dentro de una caja con `max-width` propio el sangrado crece con la ventana mientras la caja no puede. Medido: la tarjeta del hero del cierre en **700 px a 1920** y **160 a 2560**, y **`.menu__inner` con el MISMO fallo desde `#201`** — a 2560 su columna eran **60 px** — sin que lo viera nadie.
- ▶ Un contenedor acotado usa **`--col-gutter`** (longitud fija), y lo vigila `CappedContainerGutterTest`.
- ⚠️⚠️ **La lección de método es la cuarta de este carril: no basta con medir, hay que medir DONDE el fallo puede aparecer** — todas las sondas corrieron a **1280 y 390**, justo los dos anchos donde este defecto no existe.
- ⚠️ **Y dos reservas del mismo hueco no se ven por separado: se ven SUMADAS** (el sitio del minijuego estaba apartado dos veces: 220 px de aire muerto, y eso era lo que tapaba el pie).
- ❗ **§17.4 deja UNA decisión al owner**: la tarjeta del cierre ya va en la columna del mockup (1176) pero el pie sigue en `.wrap` (1380) — a 1920 hay un escalón de 102 px por lado, y igualarlos es bajar la columna de las DOCE vistas. —
- ▶ **§16 es el SEXTO y último: el MOVIMIENTO** (tanda 2d). Un paquete retempla el tacto de la web entera con **once tokens**: 4 curvas con contrato (`entra` · `cae` · `sale` · `bucle`) y 7 duraciones (120 → 900), cada una con su uso.
- ⚠️⚠️ **Medido antes: 239 declaraciones, 53 duraciones, 20 curvas — y 200 de los 220 usos de curva eran `ease`**, o sea que **el 90 % del movimiento de la web no lo decidía nadie**. Después: **608 usos de token, CERO literales fuera de la escala**.
- ⚠️ **La conversión se hizo a MEDIAS la primera vez, otra vez** (§16.3): el filtro de «esto es ambiente» buscaba palabras en el CONTEXTO y saltó **once** declaraciones —`--shadow-float` en un comentario vecino salvó a `.lang-dd__panel`—. *Un filtro que mira el contexto acierta hasta que el contexto cambia.*
- ⚠️ **El escondite de los literales eran las `custom properties`** (§16.4): `--cta-pair-swap: 0.46s` no lo veía ningún inventario de `transition`.
- ❗ **Y esta tanda NO se revisa con una captura: se revisa interactuando.** —
- ▶ **§15 es el QUINTO: el relleno de ACCIÓN**, y con él la landing ya puede ser 1:1 con el mockup en color. El botón de comprar deja de estar atado a `var(--fg)` y es un **ROL** de cuatro tokens con **un solo dato en el panel** (`theme.action`).
- ⚠️ **Vacío es una RESPUESTA, no una falta**: sin color de acción el botón *sigue a la superficie* (conducta histórica); con él es *idéntico en los dos fondos* (lo que exige el sistema del cliente). Las dos conductas salen de UNA línea —`var(--action-brand, var(--fg))` declarada en `:root` **y en las dos superficies**—.
- ⚠️ **Medido: 52 reglas rellenan con `var(--fg)` y solo 13 son acción**; las otras 39 se quedan en tinta a propósito.
- ⚠️⚠️ **`onBrand()` NO vale para un botón** y lo dijo la guarda con el color del propio cliente: prefiere blanco y su umbral es 3,0 —el de texto GRANDE—, así que sobre `#D56319` daba **3,73**.
- ⚠️ **El hover se DERIVA ×0,88**, que es el factor exacto del cliente Y del `--err-hover` que el producto ya tenía.
- ⚠️ Con luminancia ≈ 0,19 **no existe texto que pase AA** (empate en 4,31): avisar de eso está `[PENDIENTE: owner]`. — Antes, los cuatro primeros (`#192` → `#196`: color y superficie · forma · el hero · la ELEVACIÓN) — los MECANISMOS que `landing-white-label.md` §4.5 dio por supuestos y **no existían**.
- ▶ **EMPIEZA POR §10 si quieres saber cómo quedó la FORMA, y por §1.7 si vas a la 2b.**
- ⚠️⚠️ **DOS SECCIONES DE ESTA SPEC ESTÁN CADUCADAS Y LA CORRECCIÓN VA ANTES QUE EL TEXTO**: **§1.2** —«el sistema alterna dos superficies, nunca dos papeles seguidos»— **YA NO ES CIERTA** (el hallazgo `S-00` del owner, Alta y Aplicado, la sustituye por **papel continuo y el contraste lo dan las TARJETAS**; verificado: **cero** fondos a sangre en los 218 KB del mockup) —
- ▶ y eso **no tira la tanda 1: la hace más útil**, porque `[data-surface]` vale igual en una tarjeta que en una sección; y **§7**, porque la tanda 2 se partió en **2a** (forma+pie, hecha), **2b** (el hero + la escala de sombra), **2c** (el menú → **spec propia**, toca 12 vistas) y **2d** (el movimiento: 237 declaraciones, 48 duraciones, 20 curvas).
- ⚠️ **§1.3 es aritmética, no gusto**: nuestro `--fg-mute` daba **3,28 sobre tinta** (falla AA) y **ningún gris único pasa en los dos fondos**.
- ⚠️⚠️ **§13: la ELEVACIÓN son TRES ROLES, no una escala — y eso CORRIGE a §10.3**, que midió solo el difuminado. Con las cuatro dimensiones: **53 sombras, 42 formas distintas**, y la mejor escala de 5 escalones movía **47 de 53**. No era una escala con ruido: **no había ninguna** — y el sistema del cliente **tampoco tiene**, declara DOS formas. La pregunta era «¿para qué sirve cada sombra?»: `lift` · `float` · `modal`, y **19 pierden la sombra** porque una tarjeta quieta no está elevada.
- ⚠️ Leen **`--paper-fg`**, no `--fg`: dentro del hero `--fg` vale CLARO.
- ⚠️ **§10.6, la lección que más cara sale**: **dos instrumentos propios salieron rotos y las dos veces parecía lo contrario** —el clasificador de sombras no veía un `0` sin unidad, y el arnés de mutación dio «0 de 12 muerden» con el test perfecto porque aquí `grep` es **ugrep en ERE** y `\|` es un pipe literal—. **Cuando un instrumento dice que NADA funciona, la primera hipótesis es el instrumento.**
- ❗ **§12: el HERO ya NO lleva CTA** (`[DECIDIDO owner]`: «en el mockup el CTA sale DESPUÉS del hero») — queda eslogan → titular → estado, es una **tarjeta** con margen y **encoge al bajar**; comprar se ofrece en el nav y en la barra de móvil.
- ▶ **§12.3: el JS publica UNA custom property y NO decide diseño** — los dos estados los define el CSS con `calc()`, porque si los números vivieran en el `.js` serían la única parte del tema que un cliente no puede tocar.
- ⚠️ **§12.4: el test re-apuntado NO fijaba nada** —`assertSee('cta-prime')` casaba con `cta-prime__ico` y el texto del precio lo decía también el NAV—: **acota al elemento antes de creerte un test verde**.
- ⚠️ **§11.1: la auditoría del cliente EXCLUYE el hero por indicación suya** — sus COLORES están revisados, su forma y su coreografía **no**.
- ⚠️⚠️ **§11.4, la lección más cara de la tanda**: la conversión estaba **a medias en tres sitios** (el scrim tiene CUATRO paradas y se convirtieron dos) y **la guarda escrita para cazarlo nació ciega** —su diccionario ciclaba, devolvía `null` y pasaba sin mirar nada—.
- ▶ **Una guarda no se sabe si sirve hasta que se muta con el fallo REAL que la motivó.**
- ⚠️ **`ShapeScaleTest` prohíbe escribir un canto en literal**: usa un token de `5·8·10·14·16·28·999` o justifica la familia — las listas de excepción **solo encogen**. ·
- ⚠️⚠️ **El canvas del cliente NO está en el repo y no puede estarlo** (`DECISIONES #1`): se baja a `mockup_playjumppark/`, **gitignorada y excluida del `rsync`**, así que en tu máquina puede no existir — receta en §1.
- ❗ **Y CADUCA SIN AVISAR**: la copia del 27 a las 07:30 ya no valía a las 11:00 (**386 líneas de diff** en `Landing PJP Modos`, y tocaban el hero). **`DesignSync · list_files` + diff ANTES de implementar nada.** **No todos sus artboards están migrados**: de los sin migrar se saca la FORMA, nunca el color.
