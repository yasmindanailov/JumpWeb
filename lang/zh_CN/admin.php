...
return [
    ...
    'puerta' => [
        ...
        'validar' => [
            ...
            'card_query' => '已扫描会员卡',
            'card_revoked' => '会员卡已失效',
            'card_revoked_cta' => '此会员卡已轮换或撤销。请通过电子邮件或电话查找客户。',
            'card_unknown' => '无法识别的会员卡',
            'card_unknown_cta' => '该代码形似会员卡，但不在系统中。请通过电子邮件或电话查找。',
            'lookup_limited' => '一小时内手动查询次数过多。扫描仍可使用；若需按电子邮件或电话查找，请稍候或联系负责人。',
            'profile' => [
                'title' => '入口档案',
                'waiver_signed' => '免责声明已于 :date 签署',
                'waiver_outdated' => '旧版本 — 放行',
                'waiver_missing' => '未签署免责声明',
                'card_active' => '会员卡有效',
                'card_revoked' => '会员卡已撤销',
                'card_none' => '无会员卡',
                'today' => '今天',
                'today_empty' => '今天没有预订。可在门口购买。',
                'window' => '其他日期（±:days）',
                'window_note' => '有预订，但在其他日期：不是“没有预订”。',
                'entries' => '{1}:n × 门票|[2,*]:n × 门票',
                'guests' => '{1}:n 位来宾|[2,*]:n 位来宾',
    ...
    'access' => [
        ...
        'permissions' => [
            ...
            'orders_edit_item_below_minimum' => '允许套餐低于最少来宾数,创建与编辑时均适用(留有记录)',
    ...
    'catalog' => [
        ...
        'field_guest_age_family' => '年龄家族',
        'guest_age_family_hint' => '把本套餐与同一服务、不同年龄段的套餐关联起来(例如在儿童场和青少年场都填「cumple」)。仅限小写字母、数字和连字符。留空 = 本产品不区分年龄,也不会提出补差价。已售出的派对会保留购买时的家族、年龄段和价格:此处的更改只影响之后的预订。',
        'field_guest_age_min' => '最小年龄',
        'guest_age_min_hint' => '本套餐覆盖的第一个年龄,含本数。留空 = 下限不限。',
        'field_guest_age_max' => '最大年龄',
        'guest_age_max_hint' => '本套餐覆盖的最后一个年龄,含本数:填 6 时,6 岁属于本套餐,7 岁属于下一个套餐。留空 = 上限不限。',
    ...
    'settings' => [
        ...
        'section_mixed_party' => '按年龄的派对(混合生日)',
        'section_mixed_party_hint' => '当客户在来宾表单中填写的年龄在其预订条件下没有对应产品时,客户看到的文字。三种情况,按语言编辑;留空 = 默认文字。在需要显示联系电话的位置写 :phone。',
        'mixed_party_below' => '年龄低于最低年龄段',
        'mixed_party_above' => '年龄高于最高年龄段',
        'mixed_party_gap' => '年龄落在两个年龄段之间的空档',
        'mixed_party_text_hint' => '例如:「该年龄暂无生日套餐,请致电 :phone 与我们联系。」',
    ...
    'zones' => [
        ...
        'section_schedule' => '本区专属营业时间',
        'section_schedule_hint' => '留空 = 该区使用公园的营业时间。适用于在其他时段运营的区域，例如学校郊游（工作日上午到访）。⚠️ 官网公布的仍是公园的时间，而非本区的。',
        'field_opens_at' => '开放时间',
        'field_closes_at' => '结束时间',
        'field_schedule_hint' => '留空 = 继承公园的时间。',
        'field_ignores_venue_closure' => '公园休息日也开放',
        'field_ignores_venue_closure_hint' => '允许在公园休息或标记为关闭的日期销售本区。⚠️ 需要为本区设置专属时间：没有专属时间，关闭日仍然关闭。⚠️ 若要关闭某一天，请在「时段」中手动关闭其时段（重新生成时会保留手动关闭）。',
