...
// ⚠️ Se instancia AQUÍ, fuera de `alpine:init`, desde F4 · T2: lo comparten el controlador del cajón
// —que tiene que existir aunque Alpine no llegue nunca— y los superpuestos de Alpine de más abajo.
...
document.addEventListener('alpine:init', () => {
    ...
    // ⚠️⚠️ **Aquí vivía el store `purchase` ENTERO, y se mudó el 2026-09-18** (F4 · T2): su lógica es ahora
    // `cajon/controller.js`, un objeto sin framework publicado en `window.JumpWeb.cajon`, para que una página
    // que no cargue Alpine —que aquí llega dentro de Livewire— pueda abrir el cajón. Lo que queda es el
    // REGISTRO: el mismo objeto, como store de Alpine, para que los 23 usos de `$store.purchase` de la
    // landing y los `:class` de la carcasa sigan reaccionando.
    // ⚠️ Y `window.JumpWeb.cajon` pasa a ser el PROXY reactivo: Alpine solo se entera de un cambio si la
    // escritura pasa por él. Sin esta línea, `window.JumpWeb.cajon.open()` abriría «por dentro» y la carcasa
    // no se movería.
