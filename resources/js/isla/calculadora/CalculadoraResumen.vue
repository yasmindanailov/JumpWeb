 ...
 * RECIBO —lo elegido, sus líneas y el total del servidor—, «Reservar y pagar» apagado hasta tener día y hora (con el
 * enlace a lo que falta), la línea de la cuenta y el descargo antes del botón, y compartirlo por WhatsApp. PINTA:
...
<template>
    <ResumenPrecio :selection="v.resumen.seleccion" :lines="v.resumen.lineas" :total="v.resumen.total" :incomplete="v.resumen.falta" :incomplete-href="v.resumen.faltaHref" :cta-note="v.resumen.junto">
        ...
        <template #cta>
            <BotonSistema variant="primary" size="xl" full :disabled="Boolean(v.resumen.falta)" @click="$emit('reservar')">{{ v.resumen.boton }}</BotonSistema>
        </template>
    </ResumenPrecio>
