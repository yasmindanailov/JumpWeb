/**
 * **LA SECUENCIA DE COMPRA**, del catálogo al desenlace: lo que el embudo PIDE y a dónde LLEVA
 * (`specs/isla-y-landing-nueva.md` §4.10, T3d; `DECISIONES #691`).
 *
 * Vivía dentro de `sections/PurchaseSection.vue` (1.391 líneas, 464 de guion con su excepción de `CE-6`) y
 * bajó aquí, sin cambiar una conducta, porque la necesitan DOS carcasas: el cajón y la isla (`#682`). Con dos
 * copias habría dos sitios donde recordar cada ⚠️ de abajo —el orden del montaje, el sondeo que se para, la
 * cesta que no se purga por un corte de red, el pay-first del alta—, que es justo la familia de fallos que
 * este cajón ya ha pagado. Cada carcasa llama a `usePurchaseFlow(props)` dentro de su `setup` y pinta con lo
 * que devuelve; lo que es solo SUYO (la banda de progreso, el pie, el aviso de pausa y «Volver» del cajón, o
 * las pantallas de la isla) se queda en ella.
 *
 * `props` son los de la sección de compra, y de ellos se leen `messages` (el grupo `tickets`), `auth` y
 * `orderCode`. ⚠️ `loadOutcome()` lee además `props.locale`, que la sección NO declara: llega `undefined`
 * y la hora de retención del paso 10 se formatea siempre en `'es'` (el valor por defecto de
 * `holdUntilLabel()`), también para quien compra en inglés o en francés. Se mudó tal cual —esto es una mudanza,
 * no un arreglo— y está anotado en el buzón del carril del cajón.
 *
 * ⚠️⚠️ **Todos los stores se declaran ARRIBA, antes de cualquier `watch`** (`DECISIONES #210`): un `watch`
 * que lee una constante declarada debajo lanza un `ReferenceError` que Vue TRAGA y deja al observador sin
 * dependencias. En el `<script setup>` de la sección lo vigilaba `SidebarSetupBindingsTest`; aquí lo vigila la
 * misma prueba, que exige este orden en este fichero.
 */
...
export function usePurchaseFlow(props) {
     ...
     *
     * El puente de señales hacia fuera se rinde en cuanto no hay anfitrión —es lo correcto para lo que
     * hace: publicar señales—, así que colgar de él la parada dejaría el temporizador vivo en cualquier
     * página sin Alpine. Un intervalo suelto no rompe nada visible: sigue preguntando a la API cada cinco
