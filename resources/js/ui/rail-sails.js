 ...
 * hubiera más», y es FALSO.** El CSS de los dos carriles las APAGA cuando falta el atributo
 ...
 *
 * ▶ **Desde `#522` publica el hecho para TRES carriles, no uno**: el de complementos y las dos tiras del
 * pie (destinos y legal). Las dos del pie tenían el mismo problema y otro peor: su línea de tiempo
 * era `scroll(nearest inline)` sobre el `::after` del ENVOLTORIO, que busca el contenedor de scroll
 * ANTECESOR —el documento— y no la fila, que es su hermana. **No se apagó nunca**: estuvo siempre
 * visible, que es por lo que nadie lo notó.
 */

/** Los carriles con vela: `[envoltorio, carril]`. El envoltorio recibe la marca; el carril es el que desliza. */
const CARRILES = [
...
function marcar(wrap, carril) {
    wrap.toggleAttribute('data-rail-scroll', desborda(wrap, carril));
}

export function initRailSails(raiz = document) {
    const pares = CARRILES.flatMap(([envoltorio, carril]) =>
        [...raiz.querySelectorAll(envoltorio)].map((wrap) => [wrap, carril]));

    if (pares.length === 0) return;

    pares.forEach(([wrap, carril]) => marcar(wrap, carril));
    ...
    const ro = new ResizeObserver((entradas) => {
        for (const e of entradas) {
            ...
            if (par) marcar(par[0], par[1]);
        }
    });

    for (const [wrap, carril] of pares) {
        const track = wrap.querySelector(carril);

        if (track) {
            observados.set(track, [wrap, carril]);
            ro.observe(track);
        }
    }
}
