@props(['theme' => 'confeti', 'name' => '', 'age' => '', 'title' => '', 'when' => '', 'date' => '', 'time' => '', 'place' => '', 'host' => '', 'words' => '', 'gifts' => '', 'phone' => '', 'phoneHref' => '', 'art' => null, 'logo' => null, 'variant' => 'card', 'titleTag' => 'h3', 'mini' => false, 'animate' => false, 'labels' => [], 'vivo' => false, 'telefonoVisible' => true])
@php
    /*
     ...
     *
     * `vivo` (F2 de `fiesta-sistema-nuevo.md`): la VISTA PREVIA de «Personalizar» en la lista, que se reescribe al
     * teclear (`lista.js`). Pinta lo mismo que la tarjeta de siempre y además lo que HOY no se ve pero puede verse al
     * teclear —la chapa sin edad, la burbuja sin palabras, las dos formas del pie, «Llamar» sin marcar, la línea del
     * regalo—, oculto (`hidden`, que la hoja hace valer con `[data-inv-vivo] [hidden]`) y con sus marcas `data-inv-*`.
     * En modo `vivo`, `phone` es el teléfono que HAY y `telefonoVisible` si se enseña. ⚠️ La tarjeta pública y la de
     * siempre NO llevan nada de esto: su marcado es el de antes, byte a byte (lo guarda `InvitacionPaginaTest`).
     */
    ...
    // La banda acaba en ondas; la máscara va en su capa y la chapa de edad asoma por debajo sin recortarse. En `vivo` la
    // chapa se pinta SIEMPRE (oculta sin edad) con sus marcas, para que la edad tecleada aparezca y desaparezca.
    $banda = static function (bool $miniBanda, bool $animar, ?string $unidad, bool $vivoBanda = false) use ($t, $age, $conEdad, $art, $decor, $num): string {
        ...
        if ($conEdad || $vivoBanda) {
            $out .= '<span'.($vivoBanda ? ' data-inv-chip'.($conEdad ? '' : ' hidden') : '').' style="position: absolute; left: '.($miniBanda ? 12 : 20).'px; bottom: '.($miniBanda ? -$chip / 2 + 4 : -$chip / 2 + 6).'px; display: inline-flex; flex-direction: column; align-items: center; justify-content: center; width: '.$chip.'px; height: '.$chip.'px; border-radius: 50%; background: '.$t['chip'].'; color: '.$t['chipFg'].'; box-shadow: 0 0 0 '.($miniBanda ? 3 : 4).'px var(--fiesta-nieve)'.($miniBanda ? '' : ', var(--shadow-sm)').'; animation: '.($animar ? 'fiesta-chip-pop 560ms var(--ease-spring) 220ms both' : 'none').';">';
            $out .= '<span'.($vivoBanda ? ' data-inv-edad' : '').' style="font-family: var(--font-display); font-weight: 900; font-size: '.($miniBanda ? 17 : 32).'px; line-height: 0.9; font-variation-settings: &quot;wdth&quot; 104; font-variant-numeric: tabular-nums;">'.e((string) $age).'</span>';
    ...
    // En `vivo`: el atributo `hidden` como cadena (sin directivas pegadas a una palabra, spec §4.7), y «Llamar».
    $oculto = static fn (bool $ocultar): string => $ocultar ? ' hidden' : '';
    $telTag = $phoneHref !== '' ? 'a' : 'span';
    $telHref = $phoneHref !== '' ? ' href="'.e($phoneHref).'"' : '';
@endphp
@if ($mini)
<div{!! $vivo ? ' data-inv-vivo' : '' !!} style="border-radius: var(--r-md); overflow: hidden; background: var(--fiesta-nieve); box-shadow: inset 0 0 0 1px var(--border-subtle); padding-bottom: 22px;{{ $estiloExtra }}" {{ $attributes->except('style') }}>{!! $banda(true, $animate, null, $vivo) !!}</div>
...
@elseif ($vivo)
<article data-inv-vivo data-inv-tema="{{ $theme }}" data-resto-con="{{ __('fiesta.invitacion.rest', ['age' => ':age']) }}" data-resto-sin="{{ __('fiesta.invitacion.rest_sin_edad') }}" style="display: flex; flex-direction: column; min-width: 0; border-radius: var(--invite-radius, var(--r-lg)); overflow: hidden; background: var(--fiesta-nieve); color: var(--fiesta-tinta-600); box-shadow: inset 0 0 0 1px var(--fiesta-tinta-200), var(--shadow-sm);{{ $estiloExtra }}" {{ $attributes->except('style') }}>{!! $banda(false, $animate, $L['unit'], true) !!}<div style="display: grid; gap: 14px; padding: 46px 20px 20px;"><{{ $H }} style="margin: 0; display: grid; gap: 4px; font-family: var(--font-display); font-weight: 900; letter-spacing: -0.02em; color: var(--fiesta-tinta-900); text-wrap: balance;"><span data-inv-nombre style="font-size: clamp(2rem, 9vw, 2.5rem); line-height: 0.95; letter-spacing: -0.035em; color: {{ $t['accent'] }}; overflow-wrap: anywhere;">{{ $name }}</span><span data-inv-resto style="font-size: clamp(1.125rem, 4.2vw, 1.3rem); line-height: 1.12;">{{ $resto }}</span></{{ $H }}>@if ($detalles !== [])<ul style="margin: 0; padding: 0; list-style: none; display: grid; gap: 7px;">@foreach ($detalles as [$ic, $tx])<li style="{{ $txt }} display: flex; align-items: flex-start; gap: 9px; color: var(--fiesta-tinta-900); font-weight: {{ $ic === 'calendar-days' ? 700 : 500 }};"><span aria-hidden="true" style="display: inline-flex; flex: 0 0 auto; margin-top: 1px; color: {{ $t['accent'] }};"><x-lucide :name="$ic" :size="16" /></span><span style="min-width: 0;">{{ $tx }}</span></li>@endforeach</ul>@endif{{ $slot }}<figure data-inv-figura{!! $oculto($words === '' && $host === '') !!} style="margin: 0; display: grid; gap: 8px; padding-top: 14px; border-top: 1px solid var(--fiesta-tinta-100);"><span data-inv-con-palabras{!! $oculto($words === '') !!} style="font-family: var(--font-ui); font-size: var(--fs-caption); font-weight: 700; letter-spacing: 0.02em; color: var(--fiesta-tinta-500);">{{ $L['words'] }}</span><div data-inv-con-palabras{!! $oculto($words === '') !!} style="display: flex; align-items: flex-end; gap: 10px; min-width: 0;"><span data-inv-inicial aria-hidden="true" style="flex: 0 0 auto; display: inline-flex; align-items: center; justify-content: center; width: 36px; height: 36px; border-radius: 50%; background: {{ $t['chip'] }}; color: {{ $t['chipFg'] }}; box-shadow: 0 0 0 3px var(--fiesta-nieve); font-family: var(--font-display); font-weight: 900; font-size: 16px; line-height: 1;">{{ $inicial }}</span><blockquote data-inv-palabras style="margin: 0; min-width: 0; padding: 11px 15px; border-radius: 18px 18px 18px 6px; background: {{ $t['tint'] }}; font-family: var(--font-ui); font-weight: 600; font-size: 1.0625rem; line-height: 1.4; color: var(--fiesta-tinta-900); text-wrap: pretty; overflow-wrap: anywhere; transform-origin: 0 100%; animation: {{ $animate ? 'fiesta-bubble-in 460ms var(--ease-spring) 1100ms both' : 'none' }};">{{ $words }}</blockquote></div><figcaption data-inv-pie="con"{!! $oculto(! ($host !== '' && $words !== '')) !!} style="padding-left: 46px; display: flex; flex-wrap: wrap; align-items: baseline; gap: 2px 12px; font-family: var(--font-ui); font-size: var(--fs-caption); line-height: 1.45; color: var(--fiesta-tinta-500);"><span>{{ $L['host'] }}: <strong data-inv-invita style="font-weight: 700; color: var(--fiesta-tinta-900);">{{ $host }}</strong></span>@if ($phone !== '')<{{ $telTag }} data-inv-llamar{!! $oculto(! $telefonoVisible) !!}{!! $telHref !!} style="display: inline-flex; align-items: center; gap: 4px; font-weight: 700; color: var(--fiesta-agua-700); text-decoration: {{ $phoneHref !== '' ? 'underline' : 'none' }}; text-underline-offset: 3px; white-space: nowrap;"><x-lucide name="phone" :size="13" />{{ $L['call'] }}</{{ $telTag }}>@endif</figcaption><figcaption data-inv-pie="sin"{!! $oculto(! ($host !== '' && $words === '')) !!}><p style="{{ $txt }} margin: 0; display: flex; flex-wrap: wrap; align-items: center; gap: 4px 12px; color: var(--fiesta-tinta-900);"><span><span style="color: var(--fiesta-tinta-500);">{{ $L['host'] }}: </span><strong data-inv-invita style="font-weight: 700;">{{ $host }}</strong></span>@if ($phone !== '')<{{ $telTag }} data-inv-llamar{!! $oculto(! $telefonoVisible) !!}{!! $telHref !!} style="display: inline-flex; align-items: center; gap: 5px;{{ $phoneHref !== '' ? ' min-height: 44px;' : '' }} font-weight: 700; color: var(--fiesta-agua-700); text-decoration: {{ $phoneHref !== '' ? 'underline' : 'none' }}; text-underline-offset: 3px;"><x-lucide name="phone" :size="15" />{{ $L['call'] }}</{{ $telTag }}>@endif</p></figcaption></figure><p data-inv-pistas-linea{!! $oculto($gifts === '') !!} style="margin: 0; display: flex; align-items: flex-start; gap: 7px; padding-top: 10px; border-top: 1px solid var(--fiesta-tinta-100); font-family: var(--font-ui); font-size: var(--fs-caption); line-height: 1.45; color: var(--fiesta-tinta-500); overflow-wrap: anywhere;"><span aria-hidden="true" style="display: inline-flex; flex: 0 0 auto; margin-top: 1px;"><x-lucide name="gift" :size="14" /></span><span data-inv-pistas data-rotulo="{{ $L['gifts'] }}">{{ $L['gifts'] }}: {{ $gifts }}</span></p></div></article>
@else
