...
@php
    ...
    $cola = $external || $arrow;
@endphp
@if ($href && ! $disabled)<a href="{{ $href }}" @if ($external)target="_blank" rel="noopener noreferrer" @endif{{ '' }}{{ $attributes->class($clases) }}>@else<button type="button" @disabled($disabled) {{ $attributes->class($clases) }}>@endif{{ '' }}@unless ($block)<span aria-hidden="true" class="pz-enlace__toque"></span>@endunless<span class="pz-enlace__cuerpo">@if ($icono)<span class="pz-enlace__icono">{{ $icono }}</span>@endif<span class="pz-enlace__texto">{{ $slot }}</span></span>@if ($cola)<span class="pz-enlace__cola">@if ($external)<x-lucide name="arrow-up-right" :size="$iconoTam" />@endif @if ($arrow)<x-lucide :name="$block ? 'chevron-right' : 'arrow-right'" :size="$iconoTam" />@endif</span>@endif{{ '' }}@if ($external)<span class="pz-sr">(se abre en otra pestaña)</span>@endif{{ '' }}@if ($href && ! $disabled)</a>@else</button>@endif