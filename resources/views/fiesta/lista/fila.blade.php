...
@endphp
<x-fiesta.fila-invitado :id="$n['id']" :name="$n['nombre']" :age="$n['edad']" :allergies="$n['alergias']" :state="$estado" :birthday="$n['origen'] === 'cumple'" :viaInvite="$n['origen'] === 'invitacion'" :signed="$n['firmada']" :editable="$n['editable']" :last="$ultima" :campos="$n['campos']" :vacia="$n['vacia']" :quitar="$n['editable'] && $n['origen'] === 'mano'" :omitir="$n['pendiente'] && $n['editable']" :omitirForm="$descartar" :omitirValue="$n['reply_id']" :volver="$n['no_reply_id'] !== null && $n['editable']" :volverValue="$n['editable'] ? $n['no_reply_id'] : null" data-indice="{{ $n['indice'] }}" data-completa="{{ $n['completa'] ? '1' : '0' }}" data-origen="{{ $n['origen'] }}" data-respuesta="{{ $n['respuesta'] ?? '' }}" :data-regimen="$n['regimen']" :data-sin-producto="$n['sin_producto'] ? '1' : null">
    {{-- Una fila que NO abre ficha (un «no», o solo lectura) sigue siendo una posición del formulario: sus tres
         ...
         nombre y no es seguro —su «Pablo» puede no ser el «Pablo Ortiz» que dijo que no—, así que lo decide él. --}}
