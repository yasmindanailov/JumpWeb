...
RSYNC_EXCLUDES=(
    ...
    --exclude='/phpunit.xml'
    --exclude='/phpstan.neon'            # análisis estático (#625): dependencia de DESARROLLO; con
    --exclude='/phpstan-baseline.neon'   # `composer install --no-dev` allí no hay nada que los lea
    --exclude='/eslint.config.js'        # la otra mitad de #625: ESLint del cajón, solo desarrollo
    --exclude='/eslint-suppressions.json'  # (no hay node en el servidor)
