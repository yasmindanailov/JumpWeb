...
class UsedTokenIsDeclaredTest extends TestCase
{
    ...
    /**
     * **Tokens que declara el PAQUETE DEL CLIENTE, no una hoja del repo** (`#468`).
     *
     * ⚠️⚠️ **Sin esta lista la guarda dependía de un fichero GITIGNORADO y decía cosas distintas en
     * cada máquina.** `declared()` considera declarado lo que haya en `glob(public/css/*.css)`, y
     * `public/css/client.css` —el paquete de instalación, `.gitignore:27`— es justo donde vive
     * `--deco-tag` (`INSTALACION-CLIENTE.md` §4.e). Con el paquete puesto, verde; en un clon limpio o
     * en un carril que no lo tenga, **roja sin que nadie haya tocado nada**. Es la trampa de `#302`
     * («un test así pasa en tu máquina y falla en un clon limpio»), aquí por la puerta del tema.
     *
     * ⚠️ **Y su uso SIN fallback es deliberado, no un descuido**: el hueco de ilustración por
     * instalación no tiene suelo del producto —`hueco-ilustracion.md`: «sin paquete **no se pinta
     * nada**»—, porque un dibujo genérico versionado es por donde se cuela el arte de un cliente en
     * el producto. Ponerle un fallback sería inventar ese suelo.
     *
    ...
    public function test_every_used_token_without_fallback_is_declared(): void
    {
        ...
        foreach ($this->usedWithoutFallback() as $token => $sheets) {
            // Lo declara el paquete de instalación, que no está en el repo: no es un token roto.
            if (in_array($token, self::DECLARED_BY_INSTALLATION, true)) {
                continue;
            }
     ...
     */
    /**
     * **La disciplina de la lista del paquete** (`#468`), y con una diferencia que importa: se
     * comprueba contra las hojas **VERSIONADAS**, nunca contra `declared()`.
     *
     * ⚠️⚠️ `declared()` globa `public/css/*.css`, así que incluye el `client.css` del paquete **si la
     * máquina lo tiene**. Usarlo aquí devolvería el problema que esta lista viene a cerrar: la guarda
     * diría una cosa con el paquete puesto y otra en un clon limpio. Lo que se afirma es del repo:
     * que el token **se sigue usando sin fallback** (tiene sujeto) y que **ninguna hoja del repo lo
     * declara** (si alguien lo declara, la entrada sobra).
     */
    public function test_the_installation_tokens_keep_their_subject(): void
    {
        $usados = array_keys($this->usedWithoutFallback());

        $declaradosEnElRepo = [];
        foreach (self::SHEETS as $sheet) {
            preg_match_all(
                '/(--[A-Za-z0-9_-]+)\s*:/',
                $this->stripComments((string) file_get_contents(base_path($sheet))),
                $m,
            );
